...
5-55/5 8 * * 1-5 bash $PROJECT_DIR/deploy/run_runtime_release.sh holding-exit-sentinel \$(TZ=Asia/Seoul date +\%F) >> $PROJECT_DIR/logs/run_holding_exit_sentinel_cron.log 2>&1 # HOLDING_EXIT_SENTINEL_PREMARKET_0805_0855
5-55/5 9 * * 1-5 bash $PROJECT_DIR/deploy/run_runtime_release.sh holding-exit-sentinel \$(TZ=Asia/Seoul date +\%F) >> $PROJECT_DIR/logs/run_holding_exit_sentinel_cron.log 2>&1 # HOLDING_EXIT_SENTINEL_KRX_0905_0955
*/5 10-14 * * 1-5 bash $PROJECT_DIR/deploy/run_runtime_release.sh holding-exit-sentinel \$(TZ=Asia/Seoul date +\%F) >> $PROJECT_DIR/logs/run_holding_exit_sentinel_cron.log 2>&1 # HOLDING_EXIT_SENTINEL_KRX_1000_1455
0-30/5 15 * * 1-5 bash $PROJECT_DIR/deploy/run_runtime_release.sh holding-exit-sentinel \$(TZ=Asia/Seoul date +\%F) >> $PROJECT_DIR/logs/run_holding_exit_sentinel_cron.log 2>&1 # HOLDING_EXIT_SENTINEL_KRX_1500_1530
*/5 16-18 * * 1-5 bash $PROJECT_DIR/deploy/run_runtime_release.sh holding-exit-sentinel \$(TZ=Asia/Seoul date +\%F) >> $PROJECT_DIR/logs/run_holding_exit_sentinel_cron.log 2>&1 # HOLDING_EXIT_SENTINEL_AFTERMARKET_1600_1855
0-50/5 19 * * 1-5 bash $PROJECT_DIR/deploy/run_runtime_release.sh holding-exit-sentinel \$(TZ=Asia/Seoul date +\%F) >> $PROJECT_DIR/logs/run_holding_exit_sentinel_cron.log 2>&1 # HOLDING_EXIT_SENTINEL_AFTERMARKET_1900_1950
