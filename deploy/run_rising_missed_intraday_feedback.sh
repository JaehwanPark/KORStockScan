...
PYTHON
  ...
  exec bash "$REVIEWED_ROOT/deploy/run_rising_missed_intraday_feedback.sh" "$@"
fi
...
if ! flock -n 8; then
  echo "[SKIP] rising missed intraday feedback heavy analysis busy target_date=${TARGET_DATE}" | tee -a "$LOG_FILE"
  exit 0
fi
