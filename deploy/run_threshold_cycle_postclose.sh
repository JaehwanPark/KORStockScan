...
RUN_UPPER_LIMIT_WATCH_REPORT="${THRESHOLD_CYCLE_RUN_UPPER_LIMIT_WATCH_REPORT:-true}"
...
if [ "$RUN_UPPER_LIMIT_WATCH_REPORT" = "true" ] || [ "$RUN_UPPER_LIMIT_WATCH_REPORT" = "1" ]; then
  wait_for_postclose_resources "upper_limit_watch_report"
  run_postclose_cmd env PYTHONPATH=. "$VENV_PY" -m src.engine.monitoring.upper_limit_watch_report \
    --target-date "$TARGET_DATE"
  wait_for_json_artifact \
    "$PROJECT_DIR/data/report/upper_limit_watch/upper_limit_watch_report_${TARGET_DATE}.json" \
    "upper_limit_watch_report.json"
  wait_for_json_artifact \
    "$PROJECT_DIR/data/report/upper_limit_watch_counterfactual/upper_limit_watch_counterfactual_${TARGET_DATE}.json" \
    "upper_limit_watch_counterfactual.json"
  wait_for_json_artifact \
    "$PROJECT_DIR/data/threshold_cycle/bounded_live_candidates/upper_limit_watch_bounded_live_candidate_${TARGET_DATE}.json" \
    "upper_limit_watch_bounded_live_candidate.json"
fi
