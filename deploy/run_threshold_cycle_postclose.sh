...
# Current compact finalization owns its optimizer/consumer publication below.
...
# Claude/scalping pattern analysis is permanently retired. Shared reviews
# belong only to explicitly enabled Swing; legacy Main flags cannot restore it.
if [[ "$RUN_SWING_POSTCLOSE" != "true" && "$RUN_SWING_POSTCLOSE" != "1" ]]; then
  RUN_PATTERN_LAB_CURRENTNESS_AUDIT=false
  RUN_PATTERN_LAB_AI_REVIEW=false
  RUN_PATTERN_LAB_PROPAGATION_AUDIT=false
fi
