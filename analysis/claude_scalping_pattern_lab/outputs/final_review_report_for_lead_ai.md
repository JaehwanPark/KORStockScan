...
- `WAIT65~79 total_candidates=38`, `recovery_check=0`, `promoted=0`, `submitted=0`
- `blocked_ai_score_share=42.1%`, `gatekeeper_eval_ms_p95=0ms`, `budget_pass_to_submitted_rate=0.0%`

- `Budget pass without submit`: 경고 — `budget_pass=578`인데 `submitted=0`라 제출 전 병목이 기대값 회복을 끊고 있다.
...
**#1** — `AI threshold miss`
- 차단 건수 합계: 84240건 | 차단 비율: 99.4% | 관찰 일수: 49일

**#2** — `overbought gate miss`
- 차단 건수 합계: 12200건 | 차단 비율: 96.3% | 관찰 일수: 49일

**#3** — `latency guard miss`
- 차단 건수 합계: 12038건 | 차단 비율: 96.3% | 관찰 일수: 49일
