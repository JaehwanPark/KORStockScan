...
## 1.1 현재 상태 기준 (`2026-05-21 KST`)

- 현재 단계는 `Plan Rebase`의 자동화체인 튜닝 단계이며, 목적은 손실 억제가 아니라 기대값/순이익 극대화다. `2026-05-21` preopen runtime env 기준 selected runtime family는 `soft_stop_whipsaw_confirmation`, `score65_74_recovery_probe`, `scalp_sim_candidate_window_expansion`, `scalp_sim_ai_budget_manager`, `lifecycle_decision_matrix_runtime`, `scalp_sim_scale_in_window_expansion`다.
...
- `lifecycle_decision_matrix_runtime`은 ADM 확장 umbrella owner다. 기존 Entry ADM, Holding/Exit ADM, submit 관찰, scale-in bias adapter를 `entry`, `submit`, `holding`, `scale_in`, `exit` arm으로 감싸며 기본 OFF, auto-bounded micro canary 후보로만 열린다. 2026-05-21부터 LDM `entry_bucket_attribution`과 `scale_in_bucket_attribution`은 bucket별 `runtime_approval_candidates`와 `code_improvement_workorders`를 source-only로 자동 생성하고, downstream 누락은 postclose verifier FAIL로 본다.
- 기존 fixed threshold는 삭제하지 않고 역할을 바꾼다. broker submit guard, stale quote submit block, price freshness, hard/protect/emergency stop, account/order/cooldown/qty guard는 `hard_safety`; `BUY_SCORE_THRESHOLD`, VPW/strength/momentum, entry score cutoff는 `baseline_prior`; latency caution, score65_74 probe, soft stop/holding flow, scale-in price guard는 `bounded_tunable`; fallback scout/main, fallback single, latency fallback split-entry, legacy latency composite, closed shadow axes는 `legacy_archive`다.
- lifecycle matrix와 ADM의 우선순위는 `hard safety veto -> account/order/broker guard -> lifecycle matrix runtime policy -> 기존 ADM adapter -> baseline fixed threshold fallback`이다. matrix/ADM은 broker submit guard나 stale/price freshness/stop/qty guard를 우회할 수 없다.
- sim-first lifecycle은 새 독립 체인이 아니라 기존 threshold-cycle 자동화체인의 입력/해석 범위다. bucket 분류, 유형화, sim-only 후보/workorder 생성은 자동화체인이 무인 수행하며, 사용자가 명시할 지점은 code workorder의 `implement_now` 실행과 runtime env approval artifact 승인이다.  스캘핑과 스윙의 BUY/selection 가능 후보 전체를 `selection -> entry -> holding -> scale_in -> exit` virtual lifecycle로 넓게 실행해 최적 threshold 후보와 기능개선 workorder를 찾지만, 실주문 enable/cap 해제/provider 변경/bot restart의 단독 근거로 쓰지 않는다. `scalp_sim_candidate_window_expansion`은 2026-05-19 장중 사용자 override로 score 55~100 WAIT/blocked 후보까지 sim-only로 넓힌 관찰 owner이며, `scalp_sim_ai_budget_manager`는 확대된 sim holding의 OpenAI 호출을 상태변화/예산 기반으로 제어하는 sim-only owner다. 둘 다 `actual_order_submitted=false`, `broker_order_forbidden=true`, `decision_authority=sim_observation_only`를 유지한다. sim-only 후보는 BUY 분석/주문 텔레그램 알림을 보내지 않고 provenance만 남기며, sim AI budget은 provider route, BUY/submit threshold, broker guard를 변경하지 않는다.
...
- 스캘핑 entry/price/holding은 자동화체인 attribution으로 판정한다.
  ...
  - score bucket은 baseline prior와 feature일 뿐 score 단독으로 BUY/WAIT/DROP을 확정하지 않는다.
  ...
  - `soft_stop_micro_grace`, selected `soft_stop_whipsaw_confirmation`, `holding_flow_override`, Entry/Holding ADM, lifecycle matrix는 hard/protect/emergency/order safety를 우회하지 않는다.
...
- 스윙은 dry-run self-improvement 체인이다. `swing_lifecycle_audit`, `swing_threshold_ai_review`, `swing_improvement_automation`, `swing_runtime_approval`이 장후 source bundle을 만들 수 있지만 별도 approval artifact 없이는 env apply, one-share real canary, scale-in real canary, 전체 실주문 전환 금지다. `swing_strategy_discovery_sim`은 safe pool 후보를 sim-only candidate/8-arm으로 확장하고, `swing_strategy_discovery_label_builder`와 `swing_strategy_discovery_ev_report`가 label/EV/source-only 분석을 수행한다. 이 체인은 `actual_order_submitted=false`, `broker_order_forbidden=true`, `runtime_effect=false`, `decision_authority=swing_sim_exploration_only`를 유지하며 기존 스윙 runtime이나 `recommendation_history`를 대체하지 않는다.
...
- 코드베이스를 수정하는 모든 작업은 `구현 -> 자체 코드리뷰 -> 수정보완 -> 재검증 -> 결과 보고` 순서로 닫는다.
- 자체 코드리뷰는 변경한 파일만 훑는 형식으로 끝내지 않고, 변경 경로의 producer/consumer 계약, silent fail 가능성, runtime/order/provider/threshold 권한 누출, 기존 테스트가 놓치는 회귀를 함께 확인한다.
- 리뷰에서 발견한 문제는 사용자가 별도로 "보완하라"고 말하기 전에도 즉시 수정한다. 단, 수정 범위가 원 요청을 벗어나거나 runtime/order/provider/bot/threshold 변경으로 이어지면 먼저 범위를 분리해 보고한다.
- 수정보완 후에는 관련 targeted test, 문법/컴파일 검증, `git diff --check`를 기본으로 실행한다. 문서/checklist/parser 대상 변경이 있으면 parser 검증까지 포함한다.
- 테스트를 실행하지 못했거나 일부만 실행했다면 최종 답변에 미실행 사유와 남은 위험을 명시한다.
- 단순 조회, 설명, 코드 변경이 없는 리뷰 요청은 예외지만, 코드 또는 자동화 파일을 수정한 순간 이 루프를 적용한다.
