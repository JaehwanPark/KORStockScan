...
- `lifecycle_decision_matrix_runtime`은 ADM 확장 umbrella owner다. 기존 Entry ADM, Holding/Exit ADM, submit 관찰, scale-in bias adapter를 `entry`, `submit`, `holding`, `scale_in`, `exit` arm으로 감싸며 기본 OFF, auto-bounded micro canary 후보로만 열린다. 2026-05-21부터 LDM `entry_bucket_attribution`은 entry bucket별 `runtime_approval_candidates`와 `code_improvement_workorders`를 source-only로 자동 생성하고, downstream 누락은 postclose verifier FAIL로 본다.
