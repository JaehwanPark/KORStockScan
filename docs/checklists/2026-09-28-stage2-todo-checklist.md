...
- [ ] `[DirectFamilySourceRepairCompactAuxiliary] 보조 AI 판정·장후·정책 소비 폐루프 결손 수리` (`Due: 2026-09-28`, `Slot: POSTCLOSE`, `TimeWindow: 16:30~21:40`, `Track: RuntimeStability`)
  ...
  - 9/26 추가 장전 검증: 실제 프롬프트 호출 중 발견된 장후 token ceiling 오류를 수리하고 계약 보완 v4를 장후 후보로 등록했다. 이전 정책 v3은 유지한다. 9/28 bootstrap을 선택 릴리스 기준으로 재생성하여 holding vote·trailing 누락 영수증을 해소했다. 이어 기존 9/23 발행 기계정책 v4가 현재 v5 점수 기준으로 소급 거절되던 loader를 역사적 계약 범위에서 수리했다. 새 v5 후보 심사 기준은 유지하며 실제 메인 PID 소비는 9/28 기동 후 대사한다.
