...
- [ ] `[AIDecisionQualityPromptV2Followup0729] exact_v2 Prompt V2 paired replay 품질보완 및 stage 표본 재확인` (`Due: 2026-07-29`, `Slot: POSTCLOSE`, `TimeWindow: 16:35~17:00`, `Track: ScalpingLogic`)
  ...
  - 현재 결과: Prompt V2.4는 KRX entry fresh exact_v2 `49`건·`17`종목에서 provider/schema/missing result `0`건, provider none `0`건, comparable `49/49`를 충족했다. Candidate는 `EDGE 28`, `NO_EDGE 21`, `WAIT 17`, `DROP 32`, dominant action ratio `0.6531`로 V2.1의 WAIT collapse를 해소했다. 459550은 `EDGE + pullback_recovery + WAIT`, 058610은 `EDGE + blocking risk + DROP`, 459510 overextension 사례는 `EDGE + blocking risk + DROP`으로 positive edge와 adverse risk가 분리됐다.
  - 품질 판정: Control source-quality-adjusted EV `-0.051096%`, Candidate EV `0.0%`, delta `+0.051096%p`, missed-upside reduction `0`, new missed-upside `0`, adverse-first exposure 증가 `0`이다. action collapse·전체 비교·EV 개선 조건은 통과했지만 Candidate BUY가 없어 EV 양수와 missed-upside 감소 조건을 충족하지 못했으므로 `prompt_candidate_quality_rejected_refine`, runtime 승격 금지 상태다.
  - recovery 판정: `EDGE+WAIT 17건/11종목`의 5분 completed-bar recovery 원장은 sample floor를 통과했지만 recovery/adverse first=`8/9`, next-bar-open 기준 Candidate EV=`-0.165957%`, Control EV=`-0.074356%`, delta=`-0.091601%p`, negative exposure=`1→6`으로 `recovery_counterfactual_quality_rejected`다. missed-upside 감소 `2`만으로 내일 live prompt 적용을 열지 않는다.
  ...
  - 다음 판정 기준: BUY를 결과에 맞춰 강제하거나 당일 유리 사례만 고르지 않는다. 신규 natural exact_v2 cohort를 추가해 동일 Prompt V2.4와 recovery 원장을 매일 version-fixed replay하고, rolling/누적 candidate EV 양수·EV 개선·missed-upside 감소·negative/adverse exposure 비증가를 동시에 통과한 뒤 별도 PREOPEN live-prompt apply review를 연다.
...
- [ ] `[AIPromptV2DailyReplay0730] Prompt V2.4 신규 자연표본·recovery outcome 누적 replay` (`Due: 2026-07-30`, `Slot: POSTCLOSE`, `TimeWindow: 16:35~17:10`, `Track: ScalpingLogic`)
  - Source: [ai_prompt_paired_replay_2026-07-29.json](/home/ubuntu/KORStockScan/data/report/ai_prompt_paired_replay/ai_prompt_paired_replay_2026-07-29.json), [ai_prompt_recovery_trigger_2026-07-29.json](/home/ubuntu/KORStockScan/data/report/ai_prompt_recovery_trigger/ai_prompt_recovery_trigger_2026-07-29.json), [ai_decision_quality.py](/home/ubuntu/KORStockScan/src/engine/scalping/ai_decision_quality.py)
  - 판정 기준: 2026-07-30 자연 exact_v2 호출의 10분 outcome 성숙 후 동일 prompt/model/provider·전체 eligible cohort로 paired replay와 next-bar-open recovery 원장을 재생성한다. 일별·rolling/누적 결과에서 candidate EV 양수, Control 대비 EV 개선, missed-upside 감소, negative/adverse exposure 비증가, provider/schema/missing 0을 모두 확인한다.
  - 현재 적용 상태: 2026-07-29 V2.4와 recovery 후보는 모두 quality reject이므로 2026-07-30 PREOPEN live prompt 적용은 `blocked_keep_current_hot_v1`이다.
