...
- [ ] `[EntryPromptV215ProspectiveRecheckCollection0813] V2.15 fresh recheck·passive probe 전이 표본 수집` (`Due: 2026-08-13`, `Slot: INTRADAY`, `TimeWindow: KRX_REGULAR`, `Track: AIPrompt`)
  - Source: [V2.15 KRX detailed replay](/home/ubuntu/KORStockScan/data/report/ai_prompt_detailed_paired_replay/ai_prompt_detailed_paired_replay_2026-08-11_decision_quality_v2_15_bounded_recovery_venue_krx_session_krx_regular.json), [decision quality producer](/home/ubuntu/KORStockScan/src/engine/scalping/ai_decision_quality.py)
  - 실행: 신규 natural Exact V2에서 `soft_distribution_micro_program_divergence`와 `recovery_liquidity_tape_confirmation`을 outcome-blind로 관측하고, 최초 arm 이후 300초 안의 fresh exact snapshot·submit-guard 판정·post-probe 상태를 같은 symbol/venue/session으로 연결한다. arm 1건부터 cumulative observation은 갱신하되 고정 성능 floor는 `10건/3종목`으로 유지한다.
