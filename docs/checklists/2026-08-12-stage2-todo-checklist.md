...
- [x] `[FourSymbolEpisodeMachines0812] 미래에셋·제주반도체·두산·한화 독립 2-leg 실기계 구현` (`Due: 2026-08-12`, `Slot: MAINTENANCE`, `TimeWindow: 12:35~14:00`, `Track: ScalpingLogic`)
  ...
  - 기동 경계: profile별 same-day preflight와 exact-date applied hash가 통과한 뒤 제주 09:09, 두산 09:19, 미래에셋 09:34, 한화 10:04 timer가 service를 시작하도록 repo unit을 구현한다. 이번 작업은 설치 unit 갱신·timer enable·현재 PID 재기동·실주문 제출을 포함하지 않으며 review gate 종료 후 별도 사용자 지시가 있어야 runtime에 반영한다.
  - 리뷰/검증: review gate에서 미래에셋 `-2틱` leg의 재조정 누락, 오전 profile 동시 polling의 API 과호출 위험, 삼성 공통 executor에 대한 과도한 entry-state 검증, 2026-08-12 기존 3-profile applied artifact 호환 결손을 보완했다. 독립기계·삼성 오전/midday/afternoon 회귀 `190 passed`, 7-profile 연구 evidence/preopen applied 계약, Black, Ruff, compile, shell syntax, systemd verify, checklist parser, `git diff --check`를 통과했고 최종 finding은 0이다.
