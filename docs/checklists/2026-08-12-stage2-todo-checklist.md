...
- [x] `[EpisodeMachinePostcloseOpportunityExpansion0812] 에피소드 기계 장후 종목·시간·로직·보유위험 탐색 보완` (`Due: 2026-08-12`, `Slot: MAINTENANCE`, `TimeWindow: 14:15~14:45`, `Track: ScalpingLogic`)
  ...
  - 판정: 고정 신규 검토군에 target date 이하 최신 완료 `daily_recommendations_v2.csv` 스냅샷의 10만원 이하·기존/기검토 제외·score rank 상위 5개 dynamic seed를 더하고 선택 원천일을 artifact/Telegram에 고정한다. 신규종목 4개 regular session, 기존종목 미구현 session, active 7-profile logic-improvement를 분리 평가한다. logic lane은 current plan과 entry offset/3·5bar validity/+2·+4 target 대안을 clean-baseline expanding calibration과 최근 16거래일 untouched holdout으로 비교한다.
...
- [x] `[WidgetPostcloseEvExplorationAndSafety0812] 위젯 장후 EV 탐색·독립 holdout·실행귀속 보완` (`Due: 2026-08-12`, `Slot: MAINTENANCE`, `TimeWindow: 15:10~16:20`, `Track: ScalpingLogic`)
  ...
  - 검증: 저가 2-leg·widget·postclose 통합 회귀 `708 passed`, Ruff, Black, compile, shell syntax, systemd verify, checklist parser 및 diff check를 통과했다. Kiwoom 공식 SHA `69642586f7d84ba9fd8a6faf1f1537c7fda6568b`의 주문·체결잔량 계약으로 successor-order reconciliation을 재검증했고, 부분체결 root와 후속 주문의 중복 합산 방지 회귀를 추가했다. 서비스·봇 재기동과 장후 산출물 재생성은 수행하지 않았다.
