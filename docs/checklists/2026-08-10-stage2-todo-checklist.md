...
- [ ] `[ScalpMicroReversionForwardCollector0810] non-blocking forward path collector canary 및 Gate B 검증` (`Due: 2026-08-10`, `Slot: POSTCLOSE`, `TimeWindow: 20:30~21:00`, `Track: ScalpingLogic`)
  ...
  - 다음 검증: `latest.json` freshness와 auto-stop을 장중 감시하고 explicit item venue가 확인되는 유효 envelope, writer/path/reference, manual-control hard veto, restart/recovery 증적을 누적한다. Gate B의 5거래일·성숙 event 200건·필수 path field coverage 90%를 충족하기 전에는 P2 discovery·ranking·sim/runtime/order를 열지 않는다. Rollback은 observer/path=false, discovery=false 고정 후 `restart.sh` graceful restart다.
