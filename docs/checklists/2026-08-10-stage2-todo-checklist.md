...
- [ ] `[ScalpMicroReversionForwardCollector0810] non-blocking forward path collector canary 및 Gate B 검증` (`Due: 2026-08-10`, `Slot: POSTCLOSE`, `TimeWindow: 20:30~21:00`, `Track: ScalpingLogic`)
  - Source: [forward_collector.py](/home/ubuntu/KORStockScan/src/engine/scalping/micro_reversion/forward_collector.py), [kiwoom_websocket.py](/home/ubuntu/KORStockScan/src/engine/kiwoom_websocket.py), [integration manifest](/home/ubuntu/KORStockScan/docs/audit-reports/2026-08-08-scalp-micro-reversion-producer-canary-integration-manifest.json), [구현 결과](/home/ubuntu/KORStockScan/docs/audit-reports/2026-08-08-scalp-micro-reversion-producer-canary-result.md)
  ...
  - 구현 결과: 최종 감리 HOLD finding을 source-only remediation commit `24b6d04b`에서 보완했다. 동적 file-backed 수동관리 제외가 ring/detector/parent-wave/active segment/sequence/clock을 폐기하고 old-version queue row를 재차 veto한다. collector는 one-shot lifecycle과 all-writer cleanup을 사용하며 type-specific 0B item만 venue provenance로 인정한다. series explained/unexplained gap, per-series persisted watermark, reference/path shutdown reconciliation, timestamp/source validation rate와 detector-clock metric을 추가했고 source/test/deployment manifest 재검산과 관련 테스트 `169 passed`를 닫았다.
  - 현재 상태: `source_remediated_reaudit_pending_observer_off`; verified symbol/tax metadata 미확인은 관찰 blocker가 아니라 경제성 headline/P2-C/sim 승격 blocker다. observer/path/discovery flag는 기본 OFF이고 bot 재기동·runtime env 변경·장중 수집은 수행하지 않았다. Gate B·P2·sim/runtime/order는 미승인이다.
  - 다음 액션: 새 commit/manifest와 producer hook 원문 범위를 감리 재제출한다. 재승인 뒤에만 정상 거래일 observer/path canary를 열고 `producer_canary_observation_pending`, `collector_health_pass_research_data_only`, `path_coverage_insufficient`, `journal_degraded`, `manual_control_leak_blocked` 중 하나로 닫는다.
