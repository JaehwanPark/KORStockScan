...
- [ ] `[ScalpMicroReversionForwardCollector0810] non-blocking forward path collector canary 및 Gate B 검증` (`Due: 2026-08-10`, `Slot: POSTCLOSE`, `TimeWindow: 20:30~21:00`, `Track: ScalpingLogic`)
  ...
  - 현재 상태: 최종 재감리에서 `observer_path_canary_activation=CONDITIONAL_GO`를 받았으나 2026-08-08 preflight는 `conditional_go_preflight_blocked`다. 핵심 파일 hash·clean tree·승인 commit ancestor는 통과했고 `origin/main` push로 승인 commit의 원격 fetchability는 닫았다. 다만 현재 HEAD는 승인 commit의 문서-only 후속 descendant라 exact `746c015b` 조건과 다르고, 배포 서버 DNS 접근 실패로 원격 HEAD/tree/hash/PID를 확인하지 못했으며, 배포 서버 callback p95/p99 baseline·숫자 허용폭도 미동결이다. 토요일이고 local bot도 미기동이라 observer/path/discovery flag, runtime env, bot 상태는 변경하지 않았다. Gate B·P2·sim/runtime/order는 미승인이다.
  - 다음 액션: 배포 서버 접근을 복구한 뒤 exact commit/tree/hash/clean preflight와 canary 전 callback p95/p99 baseline·절대 또는 상대 허용폭을 숫자로 고정한다. 둘 다 통과한 정상 거래일에만 observer/path=true, discovery=false로 시작하고 즉시 중단 metric을 감시해 `producer_canary_observation_pending`, `collector_health_pass_research_data_only`, `path_coverage_insufficient`, `journal_degraded`, `manual_control_leak_blocked` 중 하나로 닫는다.
