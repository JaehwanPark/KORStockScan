...
- [ ] `[ScalpMicroReversionForwardCollector0810] non-blocking forward path collector canary 및 Gate B 검증` (`Due: 2026-08-10`, `Slot: POSTCLOSE`, `TimeWindow: 20:30~21:00`, `Track: ScalpingLogic`)
  ...
  - 현재 상태: 최종 재감리에서 `observer_path_canary_activation=CONDITIONAL_GO`를 받았다. 폐기된 원격 서버를 배포 대상으로 본 최초 preflight는 사용자 확인으로 정정했으며 현재 main 서버가 유일한 배포 대상이다. 핵심 파일 hash·clean tree·승인 commit ancestor는 통과했고 `746c015b..HEAD`의 deployment source diff는 0건이다. 후속 HEAD 변경은 감리보고서·manifest·checklist 증적 문서뿐이다. 현재 상태는 `conditional_go_latency_preflight_pending`이며 main 서버 callback p95/p99 baseline·숫자 허용폭만 미동결이다. 토요일이고 bot도 미기동이라 observer/path/discovery flag, runtime env, bot 상태는 변경하지 않았다. Gate B·P2·sim/runtime/order는 미승인이다.
  - 다음 액션: main 서버에서 canary 전 callback p95/p99 baseline과 절대 또는 상대 허용폭을 숫자로 고정한다. 승인 source/hash/clean과 latency 조건을 모두 통과한 정상 거래일에만 observer/path=true, discovery=false로 시작하고 즉시 중단 metric을 감시해 `producer_canary_observation_pending`, `collector_health_pass_research_data_only`, `path_coverage_insufficient`, `journal_degraded`, `manual_control_leak_blocked` 중 하나로 닫는다.
