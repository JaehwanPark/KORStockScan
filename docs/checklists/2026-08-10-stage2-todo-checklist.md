...
- [ ] `[ScalpMicroReversionForwardCollector0810] non-blocking forward path collector canary 및 Gate B 검증` (`Due: 2026-08-10`, `Slot: POSTCLOSE`, `TimeWindow: 20:30~21:00`, `Track: ScalpingLogic`)
  - Source: [forward_collector.py](/home/ubuntu/KORStockScan/src/engine/scalping/micro_reversion/forward_collector.py), [kiwoom_websocket.py](/home/ubuntu/KORStockScan/src/engine/kiwoom_websocket.py), [canary monitor](/home/ubuntu/KORStockScan/src/engine/scalping/micro_reversion/canary_monitor.py), [latency baseline](/home/ubuntu/KORStockScan/docs/audit-reports/2026-08-08-scalp-micro-reversion-callback-latency-baseline.json.txt), [canary guard](/home/ubuntu/KORStockScan/configs/scalp_micro_reversion_canary_guard.toml), [integration manifest](/home/ubuntu/KORStockScan/docs/audit-reports/2026-08-08-scalp-micro-reversion-producer-canary-integration-manifest.json), [구현 결과](/home/ubuntu/KORStockScan/docs/audit-reports/2026-08-08-scalp-micro-reversion-producer-canary-result.md)
  - 판정 기준: producer integration commit과 source/test/deployment manifest를 고정하고 기존 구독 범위 안에서 수동관리 hard veto 뒤 immutable envelope만 bounded queue에 넣는다. unknown symbol/tax metadata는 관찰할 수 있지만 경제성 headline/P2-C/sim 승격에서 차단한다. post-session compression/retention dry-run, process restart/last-sequence recovery, producer latency 악화 없음, 정상장 drop 0, 5거래일·성숙 event 200건·필수 path field coverage 90%·gap/restart/recovery는 collector 건강성만 판정한다.
  ...
  - 현재 상태: main 서버 동일 합성 0B observer OFF/ON preflight를 5회 실행해 collector internal 최대 p95=`0.026090ms`, p99=`0.032335ms`, queue drop=`0`, worker error=`0`을 확인했다. 장중 동결 한계는 callback 1,000건 이후 p95=`1.0ms`, p99=`2.0ms`다. producer/dashboard와 분리된 observer 전용 10초 monitor loop, atomic runtime snapshot, stop metric 평가, observer-only auto-stop, clean-close final reconciliation snapshot과 stop latch를 구현했다. monitor/collector targeted test `38 passed`, micro-reversion 전체와 Kiwoom WebSocket `191 passed in 4.31s`로 리뷰했다. 이 변경은 `kiwoom_websocket.py` 실행 소스를 바꾸므로 기존 `746c015b` exact-source 조건부 승인은 monitoring delta 재감리 전 활성화 근거로 쓰지 않는다. 토요일이고 bot도 미기동이라 observer/path/discovery flag, runtime env, bot 상태는 변경하지 않았다. Gate B·P2·sim/runtime/order는 미승인이다.
  - 다음 액션: monitoring delta를 commit/manifest로 고정하고 감리 재승인을 받는다. 재승인과 source/hash/clean을 모두 통과한 정상 거래일에만 observer/path=true, discovery=false로 시작하고 `latest.json` stop metric과 freshness를 감시해 `producer_canary_observation_pending`, `collector_health_pass_research_data_only`, `path_coverage_insufficient`, `journal_degraded`, `manual_control_leak_blocked` 중 하나로 닫는다.
