...
- [x] `[RisingMissedDirectPairedEconomics0919] TP1 직접 paired 경제평가·장전 소비 구조 종결` (`Due: 2026-09-19`, `Slot: POSTCLOSE`, `TimeWindow: 00:00~23:59`, `Track: ScalpingLogic`)
  ...
  - Result: source 20일/paired 2,861건. `positive_support_min 2→1` 결정 변화 342건 중 calibration 252건/12일 EV `-0.76333333%`, holdout 50건/4일 EV `-0.85%`로 `measured_no_edge`; 실제 수량·자본 제약이 없어 원화 일별 순익·최악일 원화 손익은 null이다. 나머지 5개 후보는 결정 변화 0의 identical policy다. effective `2026-09-21` 정책은 임계값 변경 없는 `incumbent_preserved`다. 실제 PID 소비·자연 decision/fill·COMPLETED 비용 성과는 다음 정상 PREOPEN 이후 별도 확인한다.
