...
- [ ] `[PostcloseSourceQualityGateReview0908] 장후 source-quality gate 결과 및 튜닝 입력 허용/제외 확인` (`Due: 2026-09-08`, `Slot: POSTCLOSE`, `TimeWindow: 21:40~21:55`, `Track: RuntimeStability`)
  - 2차 보완 자연 수용: 최종 verifier의 `validated_preflight`가 EV/PREOPEN과 같은 source generation을 판정하는지, WS 독립 finalize가 episode별 receipt 결손을 native workorder에 남기는지 확인한다. snapshot의 `*_at_snapshot`을 현재 fresh로 오인하지 않는다. `rolling_hotset_economics`는 소목표별 비용 차감 EV·관측 보유시간·빈도를 분리하고 source-only/비합산 권한을 유지해야 한다. 기존 95% floor 미달 subset은 제한된 진단일 뿐 전체 비교·실적용 성공이 아니다. 과거 원천 결손 복원이나 당일 실현 수익 증가로 판정하지 않는다.
  - 9/8 #74/#89 추가 수용: [보완 리뷰](../audit-reports/2026-09-08-ws-freshness-source-quality-remediation-review.md)의 audit v2 날짜·boolean·원천 generation 및 압축 identity receipt→EV/PREOPEN 품질 소비를 확인한다. WS `--finalize`에서 `diagnostic_acceptance`와 `rolling_prune_economics`를 분리하고, 근거 결손이 같은 장후 재확인으로 되돌아가지 않고 native workorder에 전달되는지 확인한다. 거래 공백·정상 stale 차단은 BUY/재등록 권한으로 바꾸지 않는다. 신규 self-hash receipt가 없는 과거일은 합성·반복 재생성하지 않으며, 최근 10개 source-report 날짜 안에서 finalized·동일 경제 계약의 그룹만 누적한다. 결함 없는 계측 완료에 20개 경제 outcome을 추가 요구하지 않는다. 코드 수리와 자연 소비·실현 순이익 개선은 별도 상태다.
