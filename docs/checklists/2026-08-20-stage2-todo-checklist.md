...
- [ ] `[PostProbeWinnerRecoveryNaturalAttribution0821] 새 코드 load 이후 winner-recovery 자연 receipt·rolling EV 귀속 확인` (`Due: 2026-08-21`, `Slot: POSTCLOSE`, `TimeWindow: 20:35~20:50`, `Track: ScalpingLogic`)
  ...
  - 판정 기준: 새 PID 자연행에서 `post_probe_terminal_abort_recovery_observed`의 configured/active/date/venue/cohort/rollback provenance와, 발생 시 `scale_in_executed -> sell_completed`의 entry venue/session·ADD/SELL 실제 체결 venue·완전한 quantity/economics provenance를 확인한다. exact blocker는 source-quality preflight 통과 날짜만 누적하고 venue별 10건 source bundle과 실제 1주 유효 종결 20건 비용 차감 EV를 분리한다.
