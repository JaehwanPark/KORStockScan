...
- [ ] `[EngineRefactorSafeSliceNext0526] src.engine Phase 2+ 다음 safe slice 선정 및 Phase Final gate 확인` (`Due: 2026-05-26`, `Slot: POSTCLOSE`, `TimeWindow: 18:55~19:10`, `Track: Plan`)
  ...
  - 금지: 초기 slice에서 runtime/order/provider/threshold/bot restart 경로를 이동하지 않는다. bulk move, `MetaPathFinder` shim, preopen/postclose 안정 검증 전 wrapper 제거, retired/offline/backup 파일 재편입을 금지한다. `src/trading` 전체 일괄 이동, direct import rewrite, `src.engine.scalping.__init__` side effect, broker submit/qty/cooldown/stale quote semantics 변경을 금지한다.
  - 다음 액션: `next_slice_selected`, `trading_to_scalping_inventory_required`, `defer_dirty_worktree`, `consumer_migration_ready_keep_wrappers`, `wrapper_keep_required`, `blocked_by_runtime_path` 중 하나로 닫고, 구현 시 각 slice마다 `구현 -> 코드리뷰 -> 수정보완 -> 재검증` 결과를 남긴다.
