...
- [ ] `[ScannerAsyncEvalCommitRuntimeActivation0727] scanner_async_eval_commit_v1 별도 승인·우아한 재기동·전 venue 귀속` (`Due: 2026-07-27`, `Slot: INTRADAY`, `TimeWindow: operator-approved`, `Track: RuntimeStability`)
  ...
  - 현재 상태: `async_v1_fresh_cohort_attribution_started`(scanner WATCHING/analyze_target 핵심 async bridge). PID `224372`, commit `17f4d61e`, `source_dirty=false`, effective startup mode=`async_v1`에서 시장자료 준비 worker 1개와 loaded OpenAI key 수 이내 AI dispatcher, generation별 중복 병합, deadline 이후 observation-only 폐기, main-thread generation·venue·source·fresh quote·position/order/cooldown 재검증이 로드됐다. 이번 activation owner는 SCALPING scanner continuation으로 한정한다. holding score/flow, pre-submit retry, entry-price, gatekeeper는 기존 lifecycle owner를 유지하며 scanner async 결과나 submit 권한을 공유하지 않는다.
  ...
  - 잔여 판정: 이전 PID의 boot WATCHING 13건은 initial precheck를 직렬 소모하며 `334970`의 attach→first precheck를 `69.229초`까지 지연시켰다. PID `216476`의 새 KRX `017670`·`048550`은 attach→first precheck `0.006초`·`0.080초`로 통과했지만, `399720`의 Rising Missed `entry_ai_not_evaluated` 재시도가 REST·캔들·AI를 main thread에서 약 `50.48초` 점유해 이후 attach와 recurring work를 지연시켰다. immutable generation hydration 보완은 반영됐으나 이 동기 재시도는 별도 결함이었다. `17f4d61e`는 기존 async coordinator로 preparation/AI를 보내고, validated main-thread commit 결과만 Rising Missed 후보 재판정에 적용하며 pending 상태에서는 backoff·submit을 만들지 않게 했다. PID `224372` 재기동 직후 KRX boot generation 14건은 attach→initial precheck p95 약 `2.16초`, max `2.161초`, deadline-expire `0`건이었고, 새 `017670`·`000660` generation은 `0.0045초`·`0.0046초`였다. provider=none/fatal 및 broker reconciliation 변화는 없었다. 다만 Rising Missed `not_evaluated`의 자연 async dispatch/commit 표본은 아직 0건이므로 전 venue full-cycle pass는 보류한다.
  - 적용 기준: SCALPING scanner continuation의 generation/state/version commit guard, stale/superseded result submit 차단, duplicate/silent-return test, 16-symbol stress, targeted test·compile·parser·diff PASS와 order/receipt/fast-exit cadence 비악화, 현재 broker 보유·미체결 snapshot/reconciliation PASS가 필요하다. hydration 보완 리뷰·커밋 후 user standing approval에 따라 우아하게 재기동하고, boot backlog는 별도 cohort로 분리한 뒤 신규 attach의 attach→first precheck p95 `≤2초`, max `≤5초`와 async result/commit 지연을 KRX·PREMARKET_KRX_LIKE·NXT별로 귀속한다. holding score/flow, pre-submit retry, entry-price, gatekeeper의 dispatcher migration은 scanner 결과/submit authority와 분리한 후속 code-improvement owner이며 scanner activation을 차단하지 않는다.
  ...
  - 다음 액션: `reviewed_rising_retry_async_fix_restart_async_v1`, `async_v1_fresh_cohort_attribution_started`, `async_v1_full_cycle_pass`, `rollback_to_deadline_v1` 중 하나로 닫는다.
