...
- [ ] `[ScannerAsyncEvalCommitRuntimeActivation0727] scanner_async_eval_commit_v1 별도 승인·우아한 재기동·전 venue 귀속` (`Due: 2026-07-27`, `Slot: INTRADAY`, `TimeWindow: operator-approved`, `Track: RuntimeStability`)
  ...
  - 현재 상태: `implemented_not_runtime_reflected`(scanner WATCHING/analyze_target 핵심 async bridge). 시장자료 준비 worker 1개와 loaded OpenAI key 수 이내 AI dispatcher, generation별 중복 병합, deadline 이후 observation-only 폐기, main-thread generation·venue·source·fresh quote·position/order/cooldown 재검증이 구현됐다. 이번 activation owner는 SCALPING scanner continuation으로 한정한다. holding score/flow, pre-submit retry, entry-price, gatekeeper는 기존 lifecycle owner를 유지하며 scanner async 결과나 submit 권한을 공유하지 않는다. 현재 PID의 effective startup mode는 `deadline_v1`이며 다음 검증·재기동 후 `async_v1` scanner attribution을 시작한다.
  ...
  - 잔여 판정: 재기동 boot backlog initial은 여전히 attach→first precheck `20초+` 사례가 있어 전 venue acceptance는 통과하지 못했다. 만료-refresh 순환 결함은 닫혔지만 main-thread synchronous REST/AI/heavy blocking과 복원 backlog 압력은 구조적 병목으로 남으므로 `deadline_v1_validation_pass`가 아니라 `complete_hot_path_endpoint_migration`을 유지한다.
  - 적용 기준: SCALPING scanner continuation의 generation/state/version commit guard, stale/superseded result submit 차단, duplicate/silent-return test, 16-symbol stress, targeted test·compile·parser·diff PASS와 order/receipt/fast-exit cadence 비악화, 현재 broker 보유·미체결 snapshot/reconciliation PASS가 필요하다. 이 조건을 충족하면 scanner activation gate를 리뷰로 해제하고 사용자 standing approval에 따라 `KORSTOCKSCAN_SCANNER_SCHEDULER_MODE=async_v1`로 우아하게 재기동한다. holding score/flow, pre-submit retry, entry-price, gatekeeper의 dispatcher migration은 scanner 결과/submit authority와 분리한 후속 code-improvement owner이며 scanner activation을 차단하지 않는다. KRX·PREMARKET_KRX_LIKE·NXT 결과는 분리 귀속한다.
