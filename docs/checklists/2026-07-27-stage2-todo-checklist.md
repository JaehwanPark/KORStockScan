...
- [ ] `[ScannerAsyncEvalCommitRuntimeActivation0727] scanner_async_eval_commit_v1 별도 승인·우아한 재기동·전 venue 귀속` (`Due: 2026-07-27`, `Slot: INTRADAY`, `TimeWindow: operator-approved`, `Track: RuntimeStability`)
  ...
  - 잔여 판정: 이전 PID의 boot WATCHING 13건은 initial precheck를 직렬 소모하며 `334970`의 attach→first precheck를 `69.229초`까지 지연시켰다. PID `213155`에서는 `006910`·`008700` 등 boot generation이 각각 약 7초를 점유해 직후 새 promotion들의 attach가 약 12초 지연됐고, generation 내 fast-precheck만 막아서는 부족함을 확인했다. 직접 원인은 precheck 직전 rising-relief 계산이 generation에도 persisted promotion hydration을 호출한 것이다. `scanner_generation_id`가 있는 scheduler generation은 immutable 현재 payload만 사용하고, provenance repair는 attach/registration owner에만 남기는 보완을 리뷰·검증했으나 현재 PID에는 아직 미반영이다. 따라서 현 상태는 `async_v1_full_cycle_pass`가 아니며 이 보완 반영 재기동 뒤 새 attach cohort를 다시 귀속해야 한다.
  ...
  - 다음 액션: `reviewed_hydration_fix_restart_async_v1`, `async_v1_fresh_cohort_attribution_started`, `async_v1_full_cycle_pass`, `rollback_to_deadline_v1` 중 하나로 닫는다.
