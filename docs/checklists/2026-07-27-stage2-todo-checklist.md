...
- [ ] `[ScannerAsyncEvalCommitRuntimeActivation0727] scanner_async_eval_commit_v1 별도 승인·우아한 재기동·전 venue 귀속` (`Due: 2026-07-27`, `Slot: INTRADAY`, `TimeWindow: operator-approved`, `Track: RuntimeStability`)
  ...
  - 잔여 판정: 새 PID에서 boot WATCHING 13건이 initial precheck를 직렬 소모하며 `334970`의 attach→first precheck가 `69.229초`로 deadline-expired 됐다. 반면 새 attach `017670`은 `0.055초`였다. 원인은 fast-precheck가 promotion provenance 불일치 시 persisted context hydration을 호출해 pipeline-event I/O를 수행한 구조 결함이다. hydration을 attach/registration owner로 제한하는 보완은 source에 구현·검증됐으나 현재 PID에는 아직 미반영이다. 따라서 현 상태는 `async_v1_full_cycle_pass`가 아니며 기존의 non-scanner endpoint migration과 별개로 fresh cohort 재귀속이 필요하다.
