...
- [ ] `[MultiTimeframeAIContextPremarketFinalValidation0727] 신규 다중분봉 AI 문맥 PREMARKET 최종검증 및 전 장 전면 승격 판정` (`Due: 2026-07-27`, `Slot: PREOPEN`, `TimeWindow: 08:20~08:40`, `Track: ScalpingLogic`)
  ...
  - 판정 기준: review finding=`0`, 관련 tests/compile/diff=`pass`, exact payload `request_capture_status=captured`, 응답 provenance `provider!=none`, 고정 표본별 exact request와 core endpoint별 `analyze_target`·`entry_price`·`holding_score`·`holding_flow` actual exact request가 각각 1건 이상, 같은 symbol/venue/session/completed-bar 기준 비교 가능한 필수 필드 `MISMATCH=0`, 결측·중복·시장경로 충돌=`0`, 별도 PREMARKET promotion artifact, global runtime hook/env mapping, rollback 및 post-apply attribution hook이 모두 확인돼야 한다. 기존 `AIInputExternalValidation*` 보고서는 source evidence이며, 사용자 지시와 PREMARKET promotion artifact가 전면 승격 권한을 소유한다.
