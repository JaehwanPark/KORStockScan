...
- [ ] `[CodeImprovementWorkorderReview0918] code improvement workorder 구현 필요 여부 및 Codex 지시 대상 확인` (`Due: 2026-09-21`, `Slot: ALL`, `TimeWindow: 07:45~20:35`, `Track: ScalpingLogic`)
  - 생산자 역추적 후속 intake: [통합 계획 §12 PR0–PR5](../proposals/postclose-integrated-verification-recovery-and-next-preopen-readiness-plan-2026-09-20.md#12-런타임-생산자-역추적-결함의-최소-보완계획). 사후 자본 false PASS→설치 생산자 release→생성 가능한 scanner 비교/SOR 순서로 보완한다. 현재는 계획이며 실행/배포 완료 아님. 지원 원천 생성·소비 성공과 실패 경계를 함께 검증하고, 미호출 AI의 별도 운영 계약은 자연 대기로 숨기지 않는다. 기존 기동 준비/자연 소비 owner의 완료 이력과 일정을 유지한다.
