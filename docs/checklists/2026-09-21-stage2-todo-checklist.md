...
- [ ] `[CodeImprovementWorkorderReview0918] code improvement workorder 구현 필요 여부 및 Codex 지시 대상 확인` (`Due: 2026-09-21`, `Slot: ALL`, `TimeWindow: 07:45~20:35`, `Track: ScalpingLogic`)
  - WS producer 재개방: [정정된 WS 리뷰](../audit-reports/2026-09-19-ws-freshness-conditional-economics-implementation-review.md). capacity-pruned incoming 후보가 장중 추가 주문·provider 호출 없이 prospective source-only compact 판정/entry recipe/요청 수량/guard/terminal 또는 동등하게 검증된 실행 모델 입력을 생산하고, exact `scanner_promotion_id`로 양측 selection과 join되는 계약을 구현·검증한다. 빈 ID 추가만으로 닫지 않으며 historical 결손을 합성하지 않는다. 최소 한 changed-selection 자연 generation의 `execution_inputs` 양측 도달 가능성과 후행 Daily/EV 소비를 closure로 삼는다.
