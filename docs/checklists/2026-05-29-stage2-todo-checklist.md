...
- [x] `[OpenAIWSPreopenConfirm0529] OpenAI WS 유지 설정 및 entry_price/analyze_target provenance 확인` (`Due: 2026-05-29`, `Slot: PREOPEN`, `TimeWindow: 08:55~09:00`, `Track: RuntimeStability`)
  ...
  - 운영 override 보강(2026-05-30 KST): 사용자 지시로 `entry_price` transport를 Bedrock Qwen3 32B primary + Nova Lite v2 failback으로 전환하는 코드/런타임 export를 반영한다. Nova Lite v2도 실패하면 OpenAI로 3차 failback하지 않고 기존 defensive entry-price fallback으로 닫는다. `holding_flow`는 Nova Lite v2 primary + OpenAI failback을 유지한다. 이 변경은 provider route/provenance 전환이며 threshold, 주문가/수량 guard, bot restart 권한이 아니다.
