...
- [ ] `[MicroReviewedCostSymbolOwner0815] reviewed 비용·종목분류 artifact owner 구현` (`Due: 2026-08-15`, `Slot: POSTCLOSE`, `TimeWindow: 18:30~19:30`, `Track: ScalpingLogic`)
  - Source: [micro bridge](/home/ubuntu/KORStockScan/src/engine/scalping/micro_reversion/ai_quality_bridge.py), broker fee/tax source, effective-dated verified symbol master
  - Acceptance: 적용일·venue·instrument/listing 범위와 원문 artifact SHA를 검증하는 canonical producer를 만들고, 실제 bridge CLI가 동일 artifact를 로드해 entry after-cost·notional EV를 생성한다. 비용 또는 종목분류가 없거나 범위가 맞지 않으면 계속 research-only로 fail closed한다.
...
- [ ] `[MainScalpingMicroLifecyclePairedProducer0825] 메인 스캘핑 전 lifecycle fast-turnover source-only paired producer 설계` (`Due: 2026-08-25`, `Slot: POSTCLOSE`, `TimeWindow: 18:00~20:00`, `Track: ScalpingLogic`)
  ...
  - Acceptance: 이미 AI에 도달한 호출이나 widget/episode만이 아니라 main bot 전체 lifecycle을 동일 parent에서 비교한다. 비용 반영 EV·p10·실제 exit duration·session exposure 기반 거래빈도·자본시간당 순수익·reconciled fill/exit·5% depth capacity를 공통 paired denominator로 검증하고 rolling candidate producer까지 scheduled R0→R3를 닫는다. 최초 bounded runtime family/consumer와 R4→R6는 별도 rollback·receipt·사용자 명시 승인 뒤에만 연다.
