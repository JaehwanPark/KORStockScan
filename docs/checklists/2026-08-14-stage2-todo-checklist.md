...
- [x] `[MainAIQualityPostcloseR0R3Automation0814] 장후 Exact-V2 micro A/B/C·lifecycle rolling source-only 자동화 구현·리뷰` (`Due: 2026-08-14`, `Slot: POSTCLOSE`, `TimeWindow: 20:20~21:10`, `Track: ScalpingLogic`)
  ...
  - Provider 예산: 기본 일 1 parent, 12 network attempts, USD 1.00 ceiling이며 모든 schema retry를 호출 전에 보수적으로 예약한다. Bedrock budget lane은 internal key rotation을 끄고 예약 1건당 network call을 최대 1회만 허용하며 실패/unknown usage 예약은 환불하지 않는다. request cap은 A/B/C parent를 쪼개지 않는다. reviewed pricing source가 없거나 hash/effective scope가 맞지 않으면 호출 0으로 fail closed한다.
...
- [ ] `[MicroReviewedCostSymbolOwner0815] reviewed 비용·종목분류 artifact owner 구현` (`Due: 2026-08-15`, `Slot: POSTCLOSE`, `TimeWindow: 18:30~19:30`, `Track: ScalpingLogic`)
  - Source: [economic reference producer](/home/ubuntu/KORStockScan/src/engine/scalping/micro_reversion/economic_reference.py), [micro bridge](/home/ubuntu/KORStockScan/src/engine/scalping/micro_reversion/ai_quality_bridge.py), broker fee/tax raw source, effective-dated official symbol master, reviewed Provider pricing raw source
  - 현재 상태: canonical producer·consumer·scheduled wiring과 hash/scope 검증은 구현됐다. repository에는 실제 reviewed raw broker fee/tax/symbol/provider-pricing source와 manifest가 아직 없으므로 자연 postclose는 durable `source_only_blocked_or_deferred`로 닫히고 Provider 호출은 0이다.
  - Acceptance: 공식 raw source bytes·SHA·size·effective date·venue·instrument/model scope를 검토해 `economic_reference_sources.json`과 `provider_pricing.json`을 생성하고, 실제 bridge/source-bundle/Provider budget이 동일 hash를 소비해 eligible row를 1건 이상 생성한다. 비용 또는 종목분류가 없거나 범위가 맞지 않으면 계속 research-only로 fail closed한다.
...
- [ ] `[MainAIQualityFirstBoundedRuntimeFamily0826] 최초 AI 판단품질 후보 exact-bound PREOPEN 적용·지속튜닝 family 구현` (`Due: 2026-08-26`, `Slot: POSTCLOSE`, `TimeWindow: 18:00~20:30`, `Track: ScalpingLogic`)
  - Source: [R3 source-only candidates](/home/ubuntu/KORStockScan/src/engine/scalping/micro_reversion/ai_quality_cycle.py), [standing intent matcher](/home/ubuntu/KORStockScan/src/engine/automation/main_ai_quality_standing_authorization.py), [policy approval control plane](/home/ubuntu/KORStockScan/src/engine/automation/machine_microstructure_policy_approval.py)
  - 현재 상태: 사용자의 자동적용 방향은 standing intent로만 보존한다. 아직 exact candidate prompt SHA, trusted runtime family registry entry, same-stage PREOPEN consumer가 없으므로 승인·handoff·runtime apply 권한은 0이며 R3 artifact는 source-only다.
...
- [x] `[MicroReversionStorageRetentionAutomation0814] 0B/0D/path 원천 자동 압축·삭제권한 분리` (`Due: 2026-08-14`, `Slot: POSTCLOSE`, `TimeWindow: 19:25~20:05`, `Track: RuntimeStability`)
  ...
  - 판정: 2026-08-14 점검 시 root disk 77GiB 중 66GiB 사용(86%, free 약 12GiB), micro forward 원천 약 4.6GiB이며 dry-run에서 2026-08-10~13 closed-date 43 JSONL·2.895GB가 즉시 압축 대상으로 확인됐다. 기존 storage module은 scheduled caller가 없어 gzip 0건이었고, 이를 기존 21:00 cleanup에 기본 활성으로 연결하되 삭제 권한은 제거했다.
  - 보존 계약: 실제 KST 오늘과 caller as-of date는 항상 보호한다. 자동 경로는 1일 이상 closed-date JSONL만 gzip 복원 SHA-256이 원문과 일치한 뒤 원자 교체한다. 14일 초과 partition은 purge candidate census로만 남기며 기본 `MICRO_REVERSION_STORAGE_PURGE_ENABLED=false`에서는 삭제하지 않는다. 삭제는 별도 운영 승인 뒤 wrapper env=true와 CLI `--purge-expired`가 함께 있는 explicit opt-in에서만 가능하다. target 생성 후 source unlink 또는 manifest refresh 전에 중단돼도 다음 실행이 기존 gzip을 원문과 재검증한 뒤 안전하게 마무리한다.
  - 운영 경계: 이번 리뷰에서는 실제 data apply나 purge를 실행하지 않았다. 첫 21:00 run에서 `micro_reversion_storage_purge_enabled=false`, `purge_status=disabled_no_deletion_authority`, `purged=0`과 압축 census를 확인한다. lock 경합은 skip하고 검증 실패는 원본 보존·wrapper fail이다. 이는 storage-only이고 runtime/order/provider/threshold/bot/cap 권한이 없다.
...
- [ ] `[LogCleanupArchiveRaceNaturalAcceptance0815] 21:00 growing archive 격리·micro storage 독립 완료 자연 검증` (`Due: 2026-08-15`, `Slot: POSTCLOSE`, `TimeWindow: 21:00~21:15`, `Track: RuntimeStability`)
  ...
  - 현재 상태: 2026-08-14 자연 실행은 `kiwoom_utils_info.log.9`가 압축 중 계속 쓰여 `file size changed while zipping`으로 START 직후 중단됐고, `[LOG_CLEANUP]/[FAIL]`와 micro maintenance까지 도달하지 못했다. 동시에 8/13 valid `.9.gz`와 8/14 growing `.9`가 다른 세대로 공존했다. wrapper를 micro-first 독립 lane과 generic archive 실패 집계 방식으로 보완하고 기본 300초 quiet age, pre/post-publish metadata·hash recheck, existing gzip content-conflict guard를 추가했다.
  - Acceptance: 다음 21:00 자연 실행에서 micro lane이 `status=pass|lock_busy`, `purge_enabled=false`, `purged=0`로 먼저 닫히고 closed-date 압축 census가 기록돼야 한다. concurrent-growing/quiet 미충족이면 `[ARCHIVE_COMPRESSION_FAIL] archive/reason/source_preserved=true` 뒤 다른 cleanup이 계속되고 최종 `[LOG_CLEANUP]`와 명시 `[FAIL]`가 남아야 한다. quiet/stable한 다른 세대 valid gzip conflict는 기존 gzip byte를 유지한 채 source SHA 기반 unique generation gzip을 atomic publish·roundtrip 검증하고 `[ARCHIVE_COLLISION_RECONCILED]`, `archive_collision_reconciled=1`, `[DONE]`으로 terminal 처리해야 한다. 기존 gzip과 plain hash가 같으면 source finalize만 기록한다. invalid gzip, publish 경계 변경은 양쪽 보존과 같은 실행 retention protection으로 fail-close하고 `.tmp` residue는 0이어야 한다.
...
- [ ] `[LogRotationWriterOwnershipRepair0818] kiwoom_utils numeric backup 지속 writer·세대 충돌 root-cause 제거` (`Due: 2026-08-18`, `Slot: POSTCLOSE`, `TimeWindow: 20:35~21:00`, `Track: RuntimeStability`)
  ...
  - 현재 상태: `kiwoom_utils_info.log.9`는 `fuser/lsof` writer 미검출 상태에서도 수초 간격으로 증가하고 전일 `.9.gz`와 다른 세대로 공존했다. content-hash unique generation reconciliation은 lossless storage containment이며 multi-process numeric RotatingFileHandler ownership 자체를 고친 것은 아니다.
  - Acceptance: 실제 writer PID/handler/rotation config를 식별하고 동일 numeric backup을 여러 process가 다시 여는 경로를 제거하거나 single-writer/gzip-aware 세대 계약으로 통합한다. 다음 2회 자연 21:00 실행에서 같은 archive의 반복 `source_not_quiet`, content collision, invalid gzip이 없고 archive retention이 unique generation gzip까지 30일 census로 관리되는지 확인한다. 지속 writer가 남으면 source를 삭제하지 말고 exact owner/config와 재현 test를 가진 code-improvement workorder로 유지한다.
...
- [ ] `[AIJournalAndMicroTerminalLifecycleP20815] exact journal·micro terminal artifact bounded archive 계약 구현` (`Due: 2026-08-15`, `Slot: POSTCLOSE`, `TimeWindow: 19:30~20:30`, `Track: RuntimeStability`)
  - Source: `data/ai_decision_{payloads,trace,requests,prompts,outcomes}`, `data/report/ai_micro_reversion_materialized_replay_requests`, [AI quality](/home/ubuntu/KORStockScan/src/engine/scalping/ai_decision_quality.py), [JSONL IO](/home/ubuntu/KORStockScan/src/utils/jsonl_io.py)
  - Acceptance: 모든 journal 소비자가 logical `.jsonl`과 verified `.jsonl.gz`를 동일하게 읽고 stored/content hash·row census를 검증한다. 그 후에만 closed-date gzip과 clean-baseline/감사 증거 보존 manifest를 연결한다. source bundle·materialized·result·checkpoint는 terminal hash 검증 뒤 중복 checkpoint를 제거하고 날짜별 bounded archive/retention 상태를 남긴다.
