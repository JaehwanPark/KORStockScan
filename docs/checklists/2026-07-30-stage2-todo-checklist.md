...
- [ ] `[ScannerWarmParkRisingCross0730] 장전 warm-park 후보의 정규장 fresh 상승 교차 재평가 계약 반영 확인` (`Due: 2026-07-30`, `Slot: INTRADAY`, `TimeWindow: 09:05~09:30`, `Track: ScalpingLogic`)
  ...
  - 10시 자연 표본 재판정: 에스피지 `058610`은 attach 83,300원 뒤 fresh WS 84,500원(+1.44%)과 86,000원(+3.24%)에서 warm-park 재활성화·FAST_PRECHECK 통과까지 도달했으나 opening-rotation이 후보를 소유한 채 `pullback_not_observed`로 종료되어 rising-missed 평가·AI·submit은 호출되지 않았다. 단일 당일 표본으로 owner/threshold를 변경하지 않고, 같은 generation의 attach anchor/current WS delta·venue·freshness와 no-pullback continuation 후보를 `runtime_effect=false`, `allowed_runtime_apply=false`, `broker_order_forbidden=true`로 계측한다.
  - 검증: 기존 reactivation 회귀 `8 passed`, scheduler 단위 테스트 `33 passed`에 더해 no-pullback continuation·fast-precheck provenance·source-quality 연결 회귀 `372 passed`, Ruff/compile/Black changed-range/`git diff --check`를 통과했다. 현재 PID에는 미반영이므로 반영 후 `scanner_rising_cross_warm_reason`, reactivation 1회 제한, 후속 no-pullback candidate 및 AI/TP1 귀속을 자연 표본으로 확인한다.
  - 금지: warm-park 전체 무조건 재실행, 동일 generation 반복 재활성화, stale quote 사용, threshold 완화, 주문·broker guard 우회.
  - 다음 액션: 무결함 최종 리뷰 후 별도 우아한 재기동 승인/요청 시 계측을 반영한다. clean baseline 이후 KRX/NXT를 분리해 generation당 candidate ID로 중복 제거하고 1·3·5·10·20·30·60분 MFE/MAE와 target/adverse first-hit을 성숙시킨 뒤, venue별 source-quality 통과 표본 20건 이상과 rolling EV에서 opening-rotation owner 유지 대 rising-missed handoff를 재판정한다. post-apply 자연 표본에서 `warm_park_reactivated → FAST_PRECHECK → opening_rotation_no_pullback_continuation_candidate → 이후 결과` 흐름을 확인하고 회귀 시 변경을 되돌린다.
