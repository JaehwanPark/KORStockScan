...
- [ ] `[KiwoomCommonHealthOpportunityCostAcceptance0917] 공통 health 부분 릴리스·미진입 전체 모집단 구현 잔여 확인` (`Due: 2026-09-17`, `Slot: INTRADAY`, `TimeWindow: 07:35~20:00`, `Track: RuntimeStability`)
  ...
  - 후속 U9 chronological 선정 증거: [부분 구현 리뷰 §14](../audit-reports/2026-09-17-common-health-opportunity-cost-scoped-implementation.md#14-수량leg-4군-chronological-증거와-최종-소비자-검증-보완). 서명 source-date 기반 이전일 calibration/최신일 holdout·동일 4군/분모/digest 보존식과 Daily→PREOPEN→runtime 독립 검증, 신규 발급 legacy downgrade 차단. 전체30건/80%와0.10%는 유지하되 partition별30건/0.10%를 추가하지 않음. 작업본1079건 통과; 배포/PID 소비는 별도 후속 receipt로 확인. 자연 quartet 생성/서명 date 직접 전달은 미입증이며 전체 U9 완료 아님.
