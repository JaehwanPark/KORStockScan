...
- [ ] `[SamsungWidgetAdvisoryPreopen0803] 삼성전자 전 세션 위젯 수집기·조언 API 최초 운영 귀속 확인` (`Due: 2026-08-03`, `Slot: PREOPEN`, `TimeWindow: 07:55~08:05`, `Track: RuntimeStability`)
...
- [ ] `[ExactV2DailyQualityMaterialization0803] Exact V2 일별 판단품질 생산체인 최초 장후 귀속 확인` (`Due: 2026-08-03`, `Slot: POSTCLOSE`, `TimeWindow: 20:05~20:10`, `Track: ScalpingLogic`)
  ...
  - 판정 기준: 동일 target date의 `ai_decision_quality_control`, `ai_decision_outcome_labels`, `ai_decision_quality_baseline`, `ai_prompt_paired_replay`가 `control -> mature -> baseline -> paired request preparation` 순서로 생성되고 네 산출물 모두 `runtime_effect=false`, `actual_order_submitted=false`, `broker_order_forbidden=true`를 유지하는지 확인한다. Control은 버전명만이 아니라 당일 마지막 eligible Exact V2의 prompt hash/provider/model/request 설정 전체 서명을 고정해야 하며, 동일 버전의 과거 hash는 `control_signature_not_selected`로 제외한다. Eligible exact mature 표본 1건부터 cumulative learning request에 포함하면서 runtime 승격용 `promotion_evidence_floor`는 별도 유지하고, paired report의 `candidate_execution_performed=false`를 확인한다. 성과 기준은 무조건적 DROP/WAIT가 아니라 missed-upside 감소·adverse-first 비증가·positive exposure EV 및 후단 보호장치 결합이다.
