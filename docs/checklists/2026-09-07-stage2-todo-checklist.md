...
- [ ] `[MainAIQualitySourceGapMicroReversionForwardCollectorContinuity0907] micro observer 저장공간·연속수집 source gap 복구 확인` (`Due: 2026-09-07`, `Slot: PREOPEN`, `TimeWindow: 08:40~08:45`, `Track: RuntimeStability`)
...
- [ ] `[ThresholdEnvAutoApplyPreopen0907] threshold env 자동 apply 산출물 및 사용자 개입 여부 확인` (`Due: 2026-09-07`, `Slot: PREOPEN`, `TimeWindow: 08:50~08:55`, `Track: RuntimeStability`)
...
- [ ] `[EpisodeRecommendationRuntimeAcceptance0907] Episode 53-profile exact-date 정책·timer 자연 적용 확인` (`Due: 2026-09-07`, `Slot: PREOPEN`, `TimeWindow: 08:50~14:45`, `Track: ScalpingLogic`)
  ...
  - 판정 기준: applied policy가 exact target date, 53개 full inventory, frozen evidence/source hash, `profile_revision_transition` 13건(로직 8·시간확장 5), policy hash를 통과하고 신규 timer 10개가 enabled 상태인지 확인한다. 각 신규 profile authority가 main bot/shared token/manual owner/evidence/applied hash를 모두 통과한 경우에만 자연 기동한다.
...
- [ ] `[RisingMissedScoutRuntimePreopen0907] rising_missed_scout_workorder 후속 구현 및 귀속 확인` (`Due: 2026-09-07`, `Slot: PREOPEN`, `TimeWindow: 08:55~09:00`, `Track: ScalpingLogic`)
