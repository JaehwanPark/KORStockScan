...
- [x] `[KiwoomAuth8005TokenHandoff0805] 갱신 후 장수 caller의 stale token 반복 사용 해소` (`Due: 2026-08-05`, `Slot: PREOPEN`, `TimeWindow: 08:47~09:20`, `Track: RuntimeStability`)
  ...
  - 검증 결과 (`2026-08-05 09:01 KST`): auth cache/retry·REST/order/account·WS·detector·radar fallback·read-only advisory 회귀 `274 passed`, checklist parser `60 passed`, full detector dry-run `summary_severity=pass`, Black, targeted Ruff, compile, `git diff --check`를 통과했다. 자체리뷰에서 공통 helper를 우회하던 radar fallback과 read-only advisory 소비자를 찾아 동일 handoff resolver에 연결했고 재리뷰 finding=`0`이다.
