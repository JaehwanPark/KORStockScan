...
- [ ] `[MachineExecutableMicroEntryConfirmationV2Acceptance0903] 위젯·에피소드 executable micro 진입확인 v2 장후 산출물·다음-session handoff 확인` (`Due: 2026-09-03`, `Slot: POSTCLOSE`, `TimeWindow: 21:15~21:35`, `Track: ScalpingLogic`)
  ...
  - 판정 기준: 21:15 final refresh가 `machine_entry_timing_tuning_report_v2`와 다음 거래일 `machine_entry_timing_policy_applied_v2`를 생성하고 verifier가 exact source hash를 검증했는지 확인한다. 후보가 있으면 표본 전부가 `supportive_confirmation_candidate`이고 signal/due exact-route `ka10004` BBO·비악화 bid/ask·정책 고정비용 차감 양의 modeled owner-target edge 계약이 scope에 결속됐는지 확인한다. 이 값은 broker receipt exact 비용이나 미래 target 체결이 아니다. 후보가 없으면 빈 scope와 즉시진입 `0초` baseline을 정상 carry-forward로 분리한다. 함께 생성되는 `machine_dynamic_micro_confirmation_replay_v1`은 actual signal과 owner 가격·비용 계약, 각 `신호+0/1/3/5초` 직전 1초의 past-only same-epoch BBO·0B/0D, checkpoint별 5분 same-epoch executable target/adverse first-hit을 exact 결속하고 `REJECT`를 0 exposure로 EV denominator에 남겼는지 확인한다. 일부 checkpoint 결손은 만료 거부가 아니라 `INSUFFICIENT_DATA`여야 한다. 이 replay와 `source_only_candidate_ready`는 applied-v2 policy나 다음 기동 권한이 아니며 `runtime_effect=false`, `allowed_runtime_apply=false`를 유지해야 한다.
  ...
  - 금지: v1/변조 policy, `adverse_veto_candidate|recheck_required|source_quality_blocked`, report 존재만으로 signal 생성·수량·가격·target·holding/exit·provider/bot/cap·broker/hard-safety 권한을 열지 않는다. 이 항목은 재기동 권한을 만들지 않는다.
  - 다음 액션: `v2_empty_scope_baseline_verified`, `v2_selected_scope_handoff_verified`, `source_quality_or_sample_floor_blocked`, `producer_or_verifier_contract_failed` 중 하나로 닫고, selected scope가 있으면 다음 거래일 OPEN acceptance를 생성한다.
