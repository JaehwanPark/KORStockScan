...
- [x] `[MachineFinalRefreshBoundedTerminalRepair0903] 21:15 source wait와 전체 service timeout·재시도 계약 정합화` (`Due: 2026-09-03`, `Slot: POSTCLOSE`, `TimeWindow: 21:15~21:40`, `Track: RuntimeStability`)
  - Source: [run_machine_microstructure_final_refresh.sh](/home/ubuntu/KORStockScan/deploy/run_machine_microstructure_final_refresh.sh), [widget expansion service](/home/ubuntu/KORStockScan/deploy/systemd/korstockscan-widget-expansion-recommendation.service), [machine attribution](/home/ubuntu/KORStockScan/data/report/machine_microstructure_attribution/machine_microstructure_attribution_2026-09-03.json)
  - 판정: 설치 unit의 `TimeoutStartSec=1200`은 expansion label wait 900초 뒤 attribution·policy·checklist fallback을 끝내기에 부족해 2026-09-03 자연 run을 attribution 도중 timeout으로 종료했다. missing label은 main postclose 실패의 downstream evidence gap이며 수익 0이나 정상 empty로 보간하지 않는다.
  - 보완: repository unit timeout을 3600초로 늘리고, expansion producer가 bounded source exhaustion에만 전용 exit 42를 반환하며 fallback을 끝낸 wrapper는 그 코드를 보존한다. `RestartPreventExitStatus=42`는 같은 대용량 체인의 무의미한 반복만 막고 다른 expansion/단계 오류의 원래 exit code·실패 우선순위·재시도는 유지한다. runtime/order/provider/quantity/cap/threshold authority는 추가하지 않는다.

- [ ] `[MachineFinalRefreshUnitInstallAndNaturalAcceptance0904] 21:15 final-refresh unit 설치 정합화와 자연 run terminal 검증` (`Due: 2026-09-04`, `Slot: POSTCLOSE`, `TimeWindow: 21:15~22:20`, `Track: RuntimeStability`)
  - Source: [repository service](/home/ubuntu/KORStockScan/deploy/systemd/korstockscan-widget-expansion-recommendation.service), [installed service](/etc/systemd/system/korstockscan-widget-expansion-recommendation.service), [runbook](/home/ubuntu/KORStockScan/docs/time-based-operations-runbook.md)
  - 판정 기준: reviewed repository unit을 별도 운영 절차로 설치·daemon-reload한 뒤 `TimeoutStartSec=3600`, `RestartPreventExitStatus=42`, 단일 final-refresh `ExecStart`가 byte-equivalent인지 확인한다. 다음 자연 run에서 source-ready면 6단계 rc와 unit success를, expansion-only source exhaustion이면 fallback completion·exit 42·restart 0회를 확인한다. 설치·process 재시작은 이 source-only 코드 보완의 자동 권한이 아니다.

- [x] `[WidgetEvaluationEODTerminalGateRepair0903] 20:10 위젯 평가와 EOD 키움 조회 경합 복구` (`Due: 2026-09-03`, `Slot: POSTCLOSE`, `TimeWindow: 20:10~21:20`, `Track: RuntimeStability`)
  - Source: [run_widget_evaluation.sh](/home/ubuntu/KORStockScan/deploy/run_widget_evaluation.sh), [update_kospi_2026-09-03.json](/home/ubuntu/KORStockScan/data/runtime/update_kospi_status/update_kospi_2026-09-03.json), [widget_symbol_signal_policy_research_2026-09-03.json](/home/ubuntu/KORStockScan/data/report/widget_symbol_signal_policy_research/widget_symbol_signal_policy_research_2026-09-03.json), [widget_symbol_runtime_policy_2026-09-04.json](/home/ubuntu/KORStockScan/data/runtime/widget_symbol_runtime_policy/widget_symbol_runtime_policy_2026-09-04.json)
  - 판정: 최초 20:10 run은 EOD `ka10080` 조회와 겹쳐 `shared_read_rate_wait_budget_exhausted`로 fail-closed했다. EOD `completed`, latest quote date=`2026-09-03`, rows=`2629` 뒤 실패 producer와 직접 consumer만 재실행해 source-only 연구와 `2026-09-04` observation-only policy를 복구했다.
  - 보완: wrapper가 exact target-date EOD terminal, same-date latest quote와 positive row count를 확인한 뒤에만 widget signal research를 실행한다. EOD fail·date/row mismatch·bounded timeout은 연구 전에 fail-closed하며 runtime/order/provider/quantity/cap/threshold 권한은 바꾸지 않는다.
  - 검증: shell syntax, wrapper EOD ready/fail-closed 회귀 테스트와 widget wrapper targeted test를 통과해야 하며 다음 거래일 20:10 자연 run에서 wait receipt와 terminal `[WIDGET_EVALUATION] completed`를 재확인한다.
