...
- [ ] `[ScalpPresetTpTrailingUnifiedRuntime0709] 사용자 지시 SCALP preset TP 제거 및 trailing 통일 코드 반영 준비` (`Due: 2026-07-09`, `Slot: INTRADAY`, `TimeWindow: 10:30~15:20`, `Track: ScalpingLogic`)
  ...
  - 판정 기준: 신규 SCALPING 체결은 +1.5% preset TP 지정가 주문을 생성하지 않는다. 기존 hard-stop 호환은 유지하되, `SCALP_PRESET_TP` 보유는 추적 중인 preset 주문번호가 있으면 취소한 뒤 preset AI/protect-profit 청산 분기를 거치지 않고 `scalp_trailing_take_profit` 평가로 내려간다.
