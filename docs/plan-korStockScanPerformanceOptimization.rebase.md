...
| sim-first lifecycle threshold scope | 신규 산출물 owner가 아니라 기존 `R0_collect -> R1_daily_report -> R2_cumulative_report -> R3_manifest_only -> R4_preopen_apply_candidate -> R5_bounded_calibrated_apply -> R6_post_apply_attribution` 체인의 범위 규칙이다. 스캘핑과 스윙의 BUY/selection 가능 후보 전체를 `selection -> entry -> holding -> scale_in -> exit` virtual lifecycle로 넓게 실행해 최적 threshold 후보와 기능개선 workorder를 찾는다. 2026-05-19 장중 사용자 override로 `scalp_sim_candidate_window_expansion`은 score 55~100 WAIT/blocked 후보까지 sim-only로 열어 후보 창을 확장하고, `scalp_sim_ai_budget_manager`는 확대된 sim holding의 OpenAI 호출을 상태변화/예산 기반으로 제어한다 | 실주문/예수금/cap/selected runtime family는 sim exclusion 사유가 아니라 provenance tag로만 남긴다. 별도 standalone report가 threshold-cycle consumer나 apply authority가 되지 않는다. `scalp_sim_candidate_window_expansion`과 `scalp_sim_ai_budget_manager`는 `actual_order_submitted=false`, `broker_order_forbidden=true`, `decision_authority=sim_observation_only`를 유지하며 BUY 분석 텔레그램 알림과 real 주문 경로를 건드리지 않는다 |
