...
| scale-in/position sizing | Keep scale-in price resolver and dynamic quantity safety. `position_sizing_dynamic_formula` remains the upstream affordability owner for every active `SCALPING/SCALP` new buy, Rising Missed Scout, AVG_DOWN/PYRAMID, and scalping simulation. Source-count/time/venue classification selects fixed tiers `10%/15%/20%/25%/25%` under `entry_type_5stage_cap25_v1`; NXT, unknown venue, invalid/missing source, and unrecoverable original-entry context fail closed to tier 1 (10%). Scale-in reuses the initial tier. The post-probe winner-recovery lane starts at one share; for each venue, 20 source-quality-valid closed one-share outcomes and cost-adjusted EV `>=0.1%` let the next PREOPEN delegate its first residual leg to the exact-date validated central sizing policy. Falling below that EV floor returns the venue to one share, non-positive EV disables it, and a missing/invalid central policy fails closed to one share. The 95% safe budget and downstream position/stage/broker caps remain. | Score-linear 10-30%, retired Opening Rotation margin/private-ratio authority, Rising Missed 400,000 KRW cap, sim 100% default, and `entry_armed_ratio` have no current sizing authority. Winner-recovery evidence cannot create an independent quantity formula or bypass central policy validation. Sim/probe real-order authority and hard-safety bypass are forbidden. An unapproved bot restart remains forbidden. |
