...
| lifecycle decision matrix | `lifecycle_decision_matrix_runtime`은 ADM 확장 umbrella owner다. 기존 Entry ADM/Holding-Exit ADM과 submit/scale-in adapter, sim/counterfactual source를 감싸 `entry/submit/holding/scale_in/exit` stage별 weighted ADM policy를 만들고, selected될 때만 다음 PREOPEN env로 policy file/version/promote cap을 연결한다. `lifecycle_ai_context`는 이 policy와 postclose attribution을 OpenAI entry/holding/exit prompt context로만 공급하며, context contribution은 bounded auxiliary weight로 LDM에 되먹임된다 | hard safety, account/order/broker guard, stale quote/price freshness/stop/qty guard 우회 금지. score bucket 고정 정책이나 score 단조 EV 가정으로 쓰지 않는다. context-only 운영에서는 deterministic runtime effect와 ADM/Holding runtime bias를 OFF로 둔다 |
