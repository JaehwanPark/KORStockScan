...
8. 스윙은 dry-run self-improvement 체인이다. `selection -> db_load -> entry -> holding -> scale_in -> exit -> attribution` lifecycle을 매 장후 감사하고, approval request는 생성할 수 있지만 별도 approval artifact 없이는 runtime env/live order 전환으로 보지 않는다. 비용/성능 보호를 위해 `SWING_THRESHOLD_AI_REVIEW_PROVIDER` 기본값은 `none`이며, 명시적으로 `openai`를 주지 않으면 스윙 threshold AI review는 OpenAI 호출 없이 proposal artifact만 생성한다.
...
11. `latency_classifier_runtime_profile`은 `latency_pass_events=0`/`order_bundle_submitted_events=0` 제출 고갈(submit drought)의 primary bounded-tunable owner다. `latency_classifier_recommendation`은 grid/quantile 후보를 runtime `EntryPolicy` semantics 기준으로만 평가하고, `SAFE pass`, `CAUTION runtime reject`, 제한 복구 후보(bounded recovery candidate), `hard reject`를 분리한다. `latency_fallback_deprecated`는 would-pass가 아니라 runtime reject/복구 후보로만 해석하며, 다음 PREOPEN env 반영은 `recommended_action=bounded_apply`와 `allowed_runtime_apply=true`가 동시에 닫힌 경우로 제한한다.
...
| 영역 | 현재 상태 | 금지선 |
| --- | --- | --- |
| scalping entry | Entry ADM은 `BUY_NOW/WAIT_REQUOTE/SKIP_STALE/BUY_DEFENSIVE/NO_BUY_AI/SKIP_SOURCE_QUALITY/SKIP_PRE_SUBMIT_SAFETY` bucket과 hypothesis fallback으로 AI `BUY`를 `WAIT`/`DROP`으로 보정할 수 있다. score 50 fallback/neutral은 `blocked_ai_score`로 보류한다. 기존 score cutoff는 matrix feature와 baseline prior로만 본다. `latency_classifier_runtime_profile`은 제출 고갈(submit drought)을 다루는 별도 bounded-tunable owner이며, `latency_classifier_recommendation`이 runtime SAFE pass와 CAUTION reject/recovery candidate를 분리한 뒤 PREOPEN 제한 카나리(bounded canary) 후보만 만든다 | threshold/provider/broker submit guard 우회 금지. stale/liquidity/overbought/latency/price freshness pre-submit safety는 ADM보다 우선. latency recovery canary도 hard safety, stale quote, broker/account/order/qty/cooldown guard를 우회할 수 없다 |
...
| entry funnel | `BUY Funnel Sentinel` + `wait6579_ev_cohort` + `score65_74_recovery_probe` + `scalp_entry_adm_runtime_bias_p1` + `latency_classifier_runtime_profile` | BUY/submitted drought와 forced WAIT/DROP은 daily/rolling/cumulative EV, blocker attribution, missed-upside/avoided-loss로 판정. `latency_classifier_runtime_profile`은 latency block을 reason/profile별로 쪼개 SAFE pass, CAUTION runtime reject, 제한 복구 후보(bounded recovery candidate), hard reject를 분리하고 counterfactual EV가 음수이거나 label/source-quality가 부족하면 `hold_sample`/`reject`로 닫는다 |
