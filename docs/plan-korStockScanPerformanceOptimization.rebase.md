...
| swing strategy discovery sim | `swing_strategy_discovery_sim_v1`은 safe pool 전체를 대상으로 기존 ML을 낮은 가중치의 feature/cohort로만 쓰고, lifecycle rank 60% + diversity exploration 30% + legacy ML 10% bundle로 후보를 뽑은 뒤 후보당 8개 가상 arm을 생성한다. DB table이 source of truth이고 `data/report/swing_strategy_discovery_sim` JSON/Markdown은 감사 artifact다 | `actual_order_submitted=false`, `broker_order_forbidden=true`, `runtime_effect=false`, `decision_authority=swing_sim_exploration_only` 고정. broker 주문, Telegram BUY 알림, recommendation_history 대체, threshold-cycle runtime apply, real execution 품질 주장 금지 |
...
| swing strategy discovery sim | 1차 구현 상태. `swing_strategy_discovery_candidates`, `swing_strategy_discovery_arms`, `swing_strategy_discovery_labels` schema와 safe-pool candidate builder, bounded 8-arm builder, JSON/Markdown report가 있다. 기존 ML은 feature/comparison cohort로만 들어가며 sector/theme은 v2 필수 확장 input으로 수집한다 | 다음 단계는 label builder와 postclose source bundle 편입이다. 현재 table/report는 sim exploration source이며 approval artifact 없이 real order, runtime env, threshold apply로 연결하지 않는다 |
