...
| lifecycle decision matrix | `lifecycle_decision_matrix_runtime`은 ADM 확장 umbrella owner다. 기존 Entry ADM/Holding-Exit ADM과 submit/scale-in adapter, sim/counterfactual source를 감싸 `entry/submit/holding/scale_in/exit` stage별 weighted ADM policy를 만들고, selected될 때만 다음 PREOPEN env로 policy file/version/promote cap을 연결한다. `entry_bucket_attribution`은 entry row를 `score_band/source_stage/stale/liquidity/strength/overbought/time_bucket/combo_entry_spot`으로 자동 bucket화하고 `runtime_approval_candidates`와 `code_improvement_workorders`를 source-only로 생성한다. `lifecycle_ai_context`는 이 policy와 postclose attribution을 OpenAI entry/holding/exit prompt context로만 공급하며, context contribution은 bounded auxiliary weight로 LDM에 되먹임된다 | hard safety, account/order/broker guard, stale quote/price freshness/stop/qty guard 우회 금지. score bucket 고정 정책이나 score 단조 EV 가정으로 쓰지 않는다. bucket 후보는 `approval_required=true`, `allowed_runtime_apply=false`에서 시작하고, context-only 운영에서는 deterministic runtime effect와 ADM/Holding runtime bias를 OFF로 둔다 |
...
| lifecycle matrix runtime | `lifecycle_decision_matrix_runtime`은 Full runtime용 구조가 구현됐지만 기본 OFF이며 auto-bounded micro canary 후보로만 열린다. fixed threshold는 `hard_safety`, `baseline_prior`, `bounded_tunable`, `legacy_archive` contract에 따라 재해석한다. 2026-05-21부터 LDM `entry_bucket_attribution`이 bucket별 EV/label, runtime approval candidate, code-improvement workorder를 자동 생성하며, downstream handoff 누락은 postclose verification fail 사유다 | postclose `lifecycle_decision_matrix`, `threshold_cycle_ev`, `runtime_approval_summary`, `code_improvement_workorder`, 다음 PREOPEN `threshold_cycle_preopen_apply`; 장중 mutation 없음 |
