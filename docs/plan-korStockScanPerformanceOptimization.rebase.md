...
8. LDM/ADM이 bucket, runtime approval candidate, code-improvement workorder를 생성한 뒤 downstream consumer가 받지 못하면 조용히 통과하지 않는다. `threshold_cycle_ev`, `runtime_approval_summary`, `code_improvement_workorder` 중 하나라도 LDM entry bucket handoff를 누락하면 postclose verification은 `FAIL`로 닫는다.
...
| lifecycle decision matrix | `lifecycle_decision_matrix_runtime`은 ADM 확장 umbrella owner다. 기존 Entry ADM/Holding-Exit ADM과 submit/scale-in adapter, sim/counterfactual source를 감싸 `entry/submit/holding/scale_in/exit` stage별 weighted ADM policy를 만들고, selected될 때만 다음 PREOPEN env로 policy file/version/promote cap을 연결한다. `entry_bucket_attribution`은 entry row를 `score_band/source_stage/stale/liquidity/strength/overbought/time_bucket/combo_entry_spot`으로 자동 bucket화하고, `scale_in_bucket_attribution`은 `AVG_DOWN/PYRAMID` arm과 blocker namespace/reason을 분리해 source-only EV bucket, `runtime_approval_candidates`, `code_improvement_workorders`를 생성한다. `lifecycle_ai_context`는 이 policy와 postclose attribution을 OpenAI entry/holding/exit prompt context로만 공급하며, context contribution은 bounded auxiliary weight로 LDM에 되먹임된다 | hard safety, account/order/broker guard, stale quote/price freshness/stop/qty guard 우회 금지. score bucket 고정 정책이나 score 단조 EV 가정으로 쓰지 않는다. bucket 후보는 `approval_required=true`, `allowed_runtime_apply=false`에서 시작하고, context-only 운영에서는 deterministic runtime effect와 ADM/Holding runtime bias를 OFF로 둔다 |
...
| lifecycle matrix runtime | `lifecycle_decision_matrix_runtime`은 Full runtime용 구조가 구현됐지만 기본 OFF이며 auto-bounded micro canary 후보로만 열린다. fixed threshold는 `hard_safety`, `baseline_prior`, `bounded_tunable`, `legacy_archive` contract에 따라 재해석한다. 2026-05-21부터 LDM `entry_bucket_attribution`과 `scale_in_bucket_attribution`이 bucket별 EV/label, runtime approval candidate, code-improvement workorder를 자동 생성하며, downstream handoff 누락은 postclose verification fail 사유다 | postclose `lifecycle_decision_matrix`, `threshold_cycle_ev`, `runtime_approval_summary`, `code_improvement_workorder`, 다음 PREOPEN `threshold_cycle_preopen_apply`; 장중 mutation 없음 |
...
| sim-first lifecycle threshold scope | 신규 산출물 owner가 아니라 기존 `R0_collect -> R1_daily_report -> R2_cumulative_report -> R3_manifest_only -> R4_preopen_apply_candidate -> R5_bounded_calibrated_apply -> R6_post_apply_attribution` 체인의 범위 규칙이다. 스캘핑과 스윙의 BUY/selection 가능 후보 전체를 `selection -> entry -> holding -> scale_in -> exit` virtual lifecycle로 넓게 실행해 최적 threshold 후보와 기능개선 workorder를 찾는다. 2026-05-19 장중 사용자 override로 `scalp_sim_candidate_window_expansion`은 score 55~100 WAIT/blocked 후보까지 sim-only로 열어 후보 창을 확장하고, `scalp_sim_ai_budget_manager`는 확대된 sim holding의 OpenAI 호출을 상태변화/예산 기반으로 제어한다. 다음 scale-in 확대는 고정 조건 추가가 아니라 `lifecycle_decision_matrix_runtime`의 `scale_in` arm을 채우는 sim-only actuator로만 열며, `scalp_sim_scale_in_order_assumed_filled/unfilled`는 matrix scale-in row의 source adapter로 귀속한다. postclose는 `scalp_sim_scale_in_window_expansion_YYYY-MM-DD.json`을 기본 `approved=false`로 생성하고, 사용자가 승인한 경우에만 다음 PREOPEN env hook을 쓴다 | 실주문/예수금/cap/selected runtime family는 sim exclusion 사유가 아니라 provenance tag로만 남긴다. 별도 standalone report가 threshold-cycle consumer나 apply authority가 되지 않는다. `scalp_sim_candidate_window_expansion`, `scalp_sim_ai_budget_manager`, 향후 `scalp_sim_scale_in_window_expansion`은 `actual_order_submitted=false`, `broker_order_forbidden=true`, `decision_authority=sim_observation_only`를 유지하며 BUY 분석 텔레그램 알림과 real 주문 경로를 건드리지 않는다 |
