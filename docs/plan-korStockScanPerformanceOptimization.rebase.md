...
| Sentinel/panic | BUY/HOLD/EXIT/panic sell/panic buying은 기본 report-only source bundle이다. 단 `scalp_sim_panic_lifecycle`은 `actual_order_submitted=false`, `broker_order_forbidden=true`인 스캘핑 sim-only actuator로 risk-regime context를 소비해 entry/scale-in block, holding partial/full exit, level-1 bottoming probe entry를 기록/적용할 수 있다 | real 자동매도, TP/trailing/threshold/provider/bot restart 변경 금지. level-1 bottoming probe는 sim-only이며 broker submit/Telegram BUY 알림/real entry 완화 권한 없음 |
