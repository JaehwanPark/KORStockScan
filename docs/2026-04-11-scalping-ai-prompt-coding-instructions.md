...
- 단, `작업 3 HOLDING hybrid override 조건 명세`는 문서화만 끝났고 구현은 아직 시작되지 않았다. 구현 기준으로는 `Done`이 아니라 `In Progress(문서완료/구현대기)`로 본다.

## 예정 일정 (2026-04-14 재고정)

1. `2026-04-14 POSTCLOSE`
   - `P1` 착수 범위 확정
   - `작업 4 WATCHING 75 정합화 원격 canary`와 `작업 5 WATCHING/HOLDING 프롬프트 물리 분리`의 write scope / rollback 가드 / 비교지표를 확정한다
   - `작업 8 감사용 핵심값 3종 투입`은 `P2`와 독립적으로 같은 날 바로 착수한다
   - 오늘 범위가 명확하면 같은 날 바로 `작업 4/5` 구현에도 착수한다
2. `2026-04-15`
   - `작업 4 WATCHING 75 정합화 원격 canary` 구현 진행 / 원격 ready 확인
   - `작업 5 WATCHING/HOLDING 프롬프트 물리 분리` 구현 진행 / 로그 비교축 확인
   - `작업 8 감사용 핵심값 3종 투입`은 전일 착수분을 이어서 구현/검증한다
3. `2026-04-16`
   - `P1` 원격 canary 1차 결과 평가
   - `작업 3` 구현 착수 준비와 `작업 9` helper scope 초안을 닫는다
4. `2026-04-17`
   - `작업 3 HOLDING hybrid override 조건 명세` 구현 착수 또는 보류 사유 기록
   - `작업 6 HOLDING 포지션 컨텍스트 주입` 착수 또는 보류 사유 기록
   - `작업 9` helper scope 확정
5. `2026-04-18`
   - `작업 7 WATCHING 선통과 조건 문맥 주입` 착수
6. `2026-04-19`
   - `작업 9 정량형 수급 피처 이식 1차` 착수
7. `2026-04-21`
   - `작업 10 HOLDING hybrid 적용` 착수
8. `2026-04-22`
...
- 현재 상태: `In Progress (문서완료/구현대기)`
- 구현 일정:
  - `2026-04-17` 이내 `작업 10 HOLDING hybrid 적용`의 선행조건으로 실제 구현 착수 또는 보류 사유를 문서에 남긴다.
- 자동동기화 상태: In Progress
...
- 늦어도 `2026-04-15 POSTCLOSE`에는 구현 착수 상태를 문서에 남긴다.
...
- `작업 10`은 더 이상 `P3 이후` 설명만 두지 않고 `2026-04-21` 이내 착수한다.
...
- `작업 12`는 구현까지 한 번에 요구하지 않지만, `2026-04-23` 이내 `범위 확정`은 끝낸다.
- parse fail 비율
- HOLDING 조기청산 비율
- WATCHING 진입 통과율
- realized pnl
