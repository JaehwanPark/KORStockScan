...
The exact-date bootstrap refreshes only its handoff coordinates, `KORSTOCKSCAN_THRESHOLD_RUNTIME_AUTO_APPLY_ENABLED=true` and `KORSTOCKSCAN_THRESHOLD_RUNTIME_APPLY_DATE=<target-date>`, before applying operator overrides and locks. These coordinates do not select a new threshold. An explicit operator value still takes precedence and a false or mismatched value fails PREOPEN. PID verification compares the final launcher environment, including the same hash-verified AI-context overlay loaded immediately before `bot_main.py`.
