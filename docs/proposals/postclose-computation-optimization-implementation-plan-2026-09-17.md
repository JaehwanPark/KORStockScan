...
상태: **계획 작성 완료, 아래 소스 최적화는 미착수**. 대상은 ① main AI quality/provider replay ② 삼성 entry·저가주 2leg·AVG_DOWN 연구 계산 ③ source-quality/cumulative EV 반복 원천 처리다. 이번 요청은 계획 작성이며 이 문서는 production 실행·패키지 설치·provider/주문 호출·배포·재기동을 실행시키지 않는다.
