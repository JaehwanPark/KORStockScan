...
- 전체 폐루프 잔여·확대 성능 후속은 [C0–C8/P1–P6 계획](widget-episode-full-closed-loop-and-scale-performance-implementation-plan-2026-09-17.md)을 참조한다. 기존 source/cache 최적화 완료와 새 admission/전향적 실행검증/joint gate/version 회수 미구현을 구분하며, source readiness·21:15 refresh wait·전체 nightly deadline을 성능 acceptance에 포함한다. 이 추가 링크는 운영 timer/resource/실행 절차를 변경하지 않는다.
