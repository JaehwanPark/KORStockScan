...
> 전환 후 재리뷰: 제외 봉의 메타데이터 재혼입·종료 PID를 원천 파일 부재로 오인한 REST 대체 허용·원천 실패 사유 덮어쓰기를 수리한다. 승인된 결손 허용 및 주문 직전 guard는 유지한다. [후속 리뷰](../audit-reports/2026-09-21-widget-ws-completed-bars-implementation-review.md#observed-history-follow-up-review-and-repair)가 배포·검증 증거를 소유한다. 수리·배포 완료: `8e031f494` 푸시,743 PASS,수집기4 새 PID 소비.19:19 AL6 분봉검사 PASS·3종목 실제 WS/분봉REST0 유지. 삼성은 과거봉 검증 PASS이나 현재0B/0D 약13분 경과로 quote guard 대기이며, 잘못된 PID 원인 표시를 바로잡았다.
...
## I/O repair and observation scope follow-up (2026-09-21)

The user-approved next sequence is implemented in the [I/O review](../audit-reports/2026-09-21-widget-io-policy-cache-and-observation-scope-review.md): repair repeated full research-source parsing in the auto-expansion loader, retain the three existing WS research symbols while pausing idle expanded observations, and reuse the parsed shared checkpoint by file generation. Every current policy/publication/registry and quote clock check remains in force. Execution symbols and active advisory episodes are preserved. The priority scope is an observation operating setting, not a change to catalog policy or order authority. Main publisher lock cost and full-universe registration expansion remain outside this release; do not treat reader reuse as proof of additional WS capacity.

Follow-up review extends the same priority scope to the background research fact writer: widget admission refresh and overlapping episode symbols cannot reintroduce excluded widget seed memberships. Episode observations and historical facts remain preserved. Forked readers reset the local checkpoint cache and mutex. This closes two implementation gaps in the operating scope; it does not expand WS registrations or relax quote validity. See the linked I/O review for deployment evidence.

Follow-up deployment completed at20:20 KST: `546693088`,441 release tests passed, five collector definitions and startup verified. Actual research writer receipt confirms the three-symbol widget scope and zero remote calls. Existing episode memberships remain retained; next-session collection/submission and cost-adjusted outcomes stay with the existing natural-acceptance owner.

2026-09-22 Samsung morning startup handoff: the PREOPEN wrapper's verifier release must match the selected main's current runtime contract. A stale verifier binding caused the exact-date authority alert; the scoped binding was repaired without changing the live episode strategy, quote policy or main PID. [Incident review and rollback](../audit-reports/2026-09-22-samsung-morning-preflight-release-binding-recovery.md) owns dated evidence.

2026-09-22 entry-flow consumer repair: widget/episode order routes are preserved independently from integrated WS source selection. Received `_AL` takes priority; malformed/duplicate/stale AL remains blocked and cannot fall back to a native tape. Native-only sources retain exact-route validation. Both preparation and pre-transport checks share this contract, and already terminal attempts remain consumed. [Review and deployment evidence](../audit-reports/2026-09-22-entry-adverse-integrated-ws-route-repair.md).

AL entry-flow repair deployed as `dfe202068` on2026-09-22:706 source/release tests each passed,66 consumer definitions bound, active widget trader restarted and completed morning identity preserved through startup. Future timer starts and live outcomes remain prospective. Separate main sniper thread health failure is recorded in the incident review; this release does not certify whole-fleet health.
