...
작성: 2026-09-23 KST. 상태: **구현 중**. 9/23 구현에는 full-baseline source census와 source-gap 보존, positive-delay carry 계약 및 split seed/outcome lineage 계측이 포함된다. 장후 산출·정책 유효성·실제 PID 소비 확인 전에는 정책 생성이나 경제성 완료로 간주하지 않는다.
...
| `entry_split` / `entry_cancel_wait` | [직접 family 요약](../checklists/2026-09-23-stage2-todo-checklist.md)은 각각 `operating_paired_source_missing` / `execution_producer_census_unsealed`를 첫 blocker로 둔다. 9/22 Entry Split 재생에서는 raw plan 29건 중 20건이 frozen seed missing, 9건이 plan missing/invalid였고, 실현비용 outcome 파일도 없었다. | ES/CW package로 실제 제출·체결·terminal 모집단을 먼저 닫는다. Entry Split은 진단 누적일을 전체 baseline census와 대조해 operating attempt census 누락·seed/replay 결손·완료 비용 영수증 결손·모델 비교 결손을 날짜별 표시한다. 미기록 날짜나 결손 영수증을 0건으로 추정하지 않는다. 두 family의 비용 비교 준비 전에는 null을 0 또는 측정된 no-edge로 바꾸지 않는다. |
...
| 확정 판정→최초 BUY 제출 지연 | 독립 `pre_submit_delay_tuning` report·dated policy·loader·runtime intent는 구현됐으나 현재 clean-baseline paired EV source gap이다. `entry_split_order_policy`나 `entry_cancel_wait` 값을 읽거나 고치지 않는다. 기존 제출 후 10분 tick-band 진단을 이 축의 근거로 오인하지 않는다. |
...
| 실패·승격 | own `source_gap/insufficient_evidence/no_edge/selected`, own hash/rollback | own `source_gap/insufficient_evidence/no_edge/selected`, own hash/rollback; 0초는 paired control만 허용하고 새 정책으로 발행하지 않음. 검증된 같은 축의 양수 지연 정책만 다음 선택이 검증될 때까지 승계 |
...
4. **ES3, paired 선택:** 위 세 결과군을 동일 frozen 기회·목표 수량·가격 상한·hard guard·exit/cost·자본 조건에서 incumbent와 실행 가능한 분할 후보로 비교한다. 비용 후 attempt당 EV와 관측일당 순익, 실제 승리·실패·미진입 각각의 건수와 EV 기여, full/partial/no-fill·missed upside·winner retention·p10/tail을 calibration/시간순 독립 holdout에 출력한다. 실제 성공한 ENTER_NOW는 보존해야 할 양성 사례와 winner-retention 지표로 사용하되 승자만 복제·과가중하지 않는다. 실제 모델은 clean-baseline 안에서 비용·결정 lineage 커버리지 80% 이상이며 건수 10 이상인 가장 이른 두 날짜를 outcome 값과 무관하게 고정하고, 총 20건 이상으로 calibration/holdout을 닫는다. Paired 후보의 기존 30건 바닥값은 calibration 15건·최신 독립 holdout 15건 이상으로 나눠 한 건 holdout 승격을 막는다. Attempt/episode ID는 frozen seed의 해시와 정확히 일치해야 하며 재라벨 표본은 세지 않는다. machine bundle/policy와 compact AI prompt 버전·해시가 다른 표본은 scope를 분리하고, 같은 runtime 유형 bucket에 복수 scope 후보가 생기면 임의 덮어쓰지 않고 source gap으로 남긴다.
...
**짝지어진 실현 경제성 source gate:** 기존 split 누적 수익/승률 진단은 후보 선택용 짝지어진 실현 경제성이 아니다. 전체 clean-baseline 진단 source date마다 (a) operating 적격 attempt census, (b) 동일 frozen seed/plan의 replay 행, (c) main BUY owner journal의 parent·child·fill·terminal, (d) 완료 매도의 비용완료 receipt를 각각 보존한다. completed outcome 원천이 없을 때는 미완료/원천 누락/실제 완료 0건을 구분할 수 있는 producer census receipt가 필요하다. 이 계약이 닫히기 전 `operating_paired_source_missing`/`clean_baseline_paired_realized_population_incomplete`는 source gap이며 후보 선정이나 기본정책 변경을 허용하지 않는다. 런타임 seed 생성 실패는 계획 주문을 바꾸지 않는 관측 상태와 구체 blocker를 제출 원천에 남긴다. 과거의 유실 원문은 새 계측으로 소급 복원하지 않는다.

현재 코드 원천 감사에서는 9/22 compact projection의 29개 sizing plan 중 20개에 frozen replay seed가 없고 9개는 가격/계획 원천 검증이 되지 않았다. 전체 clean-baseline 누적 실매도 1,439행에는 exact `entry_split_actual_economics` receipt가 없었다. 그러므로 과거 성과로 짝지어진 후보를 소급 구성하지 않는다. 새 제출은 `entry_opportunity_replay_seed_status/blocker`를 실제 보유·매도 receipt까지 전달하고, post-sell 산출물은 완료 receipt 수·명시적 원천 gap·비대상·구형 미분류 행을 별도로 census한다.

### PD0–PD4: 판정 후 최초 제출 지연의 독립 튜닝축

**유형별 선택 계약:** 하나의 지연시간을 전 종목에 일괄 적용하지 않는다. 기계판정이 이미 사용하는 사전 관측형 `venue/session`, `price_tick_band`, `liquidity_band`, `volatility_band`를 지연 평가의 후보 구분 근거로 재사용할 수 있다. 지연 family는 이 값과 원본 시각·source hash를 **진입 의도 확정 시점에 동결**하고, 기계 정책의 selector/leaf 또는 임계치를 지연 정책으로 복사하지 않는다. 우선 `venue/session` parent에서 30/60/120/180초 후보를 0초 대조군과 짝지어 평가하고, 충분한 paired 원천이 있는 경우에만 tick 비율 또는 유동성·변동성 중 **한 번에 한 축**으로 분기한다. 0초는 대조군이며 parent 기본 정책이 아니다. 가격대 자체와 시가총액은 당시 유효한 원천·표본·홀드아웃에서 독립적인 체결 차이를 보일 때만 후보로 추가한다. 시총 원천이 없으면 `UNKNOWN`으로 남기고 최신 시총을 과거에 붙이지 않는다. 표본이 부족하거나 시점·route 원천이 없는 leaf는 검증된 양수 지연 parent를 승계한다. 같은 축에 검증된 양수 정책이 전혀 없으면 `unselected_source_gap`으로 남기고, 0초를 선정·승격·carry 정책으로 위장하지 않는다. 기존 주문 경로는 별도 `no_validated_delay_policy` 상태로 유지한다. 정책은 하나의 root+검증된 selector/leaf 묶음으로 발행하고, 각 leaf의 적격/제외 분모·0초 대비 비용 후 paired EV·승리 진입 유지율·시간순 holdout·fallback을 별도로 보고한다. 런타임은 동결된 유형으로 leaf를 고르고 대기 중 venue/session 또는 안전 계약이 바뀌면 그 intent를 취소한다. 유형별 이익을 합산해 기계판정·AI·분할 정책의 개선으로 계상하지 않는다.

**지연 시점 표본 생성:** zero/control 또는 선택된 intent는 commit 시점에 30/60/120/180초 관측 일정을 고정한다. 기존 watch/submit 재검증 주기가 이 일정을 호출해 로컬 WS quote만 저장한다. 그 관측 호출은 조회/주문을 추가하지 않으며, 샘플링 누락·stale depth·route mismatch는 각각 source gap으로 기록한다. commit 직후 0초 quote를 30초 표본으로 재사용하지 않는다.

**전체 clean-baseline 계약:** Entry Split과 최초 제출 지연의 정책 분모는 모두 `2026-06-05T00:00:00+09:00`부터 source date까지의 적격 원천 전체다. 날짜별 신규 partition만으로 후보를 만들지 않는다. 각 보고서에 시작일·종료일·실제 관측 source dates·일별 census/제외·source fingerprint를 기록하고, 과거 누락 구간은 exact lineage로 기존 pipeline/BBO/0D/분봉에서 복원 가능한 행만 보충한다. 원시 quote만 있고 동일 의도·route·terminal·비용에 결속되지 않는 행은 비교 후보가 아니라 `source_gap`으로 남긴다. 수집기 도입 전 구간을 관측하지 않았다고 조용히 제외하거나 현재 날짜 표본을 전체 clean baseline으로 표기하지 않는다. 비용은 당시 고정된 손익 비용률을 1회 적용하고 실행가능 호가 대비 체결가로 슬리피지를 별도 측정한다. 기본 `0.23%` 비용률은 코드의 거래비용 설정이며 broker 실측 슬리피지로 간주하지 않는다. 이미 순손익인 완료 체결에는 비용을 다시 차감하지 않는다.

현재 구현의 첫 유형 키는 `venue|session|price_tick_band`이며 `venue|session` parent로 내려간다. 유동성·변동성·시총은 이 시점에 검증된 값이 없어 `UNKNOWN`으로 기록하고 selector로 사용하지 않는다. 장후 보고서는 관측된 각 parent/leaf의 적격 건수와 후보별 quote coverage를 별도로 낸다. 실행·완료·비용·시간순 holdout의 같은 분모가 닫히기 전에는 leaf를 선택하지 않고 `unselected_source_gap`으로 남긴다. 유효한 양수 정책이 전혀 없는 경우 0초 policy/carry를 발행하지 않으며 runtime은 별도 no-policy 상태로 기존 즉시 제출 동작을 유지한다. 기존 대형 raw pipeline의 호가 존재 진단은 끝 64MiB만 읽으며 하루 전체의 부재나 체결가능성을 주장하지 않는다.

30초 이상 지연 중에는 기존 WATCHING 루프가 기계·AI 판정을 새로 관측할 수 있다. 만료 시 원래 판정과 **같은 정책 세대·ENTER_NOW·PASS/CAUTION**이고 새 판정의 시각 신선도가 기존 제출 가드를 다시 통과하면 그 후속 attempt를 원 의도와 함께 기록한다. 새 AI 호출을 지연 모듈에서 만들지는 않는다. 후속 action·정책 세대·종목 유형·promotion parent가 달라지면 원 의도는 주문 없이 종료한다. 원래 attempt와 만료 시점 attempt를 하나의 학습 표본으로 중복 계산하지 않는다.
