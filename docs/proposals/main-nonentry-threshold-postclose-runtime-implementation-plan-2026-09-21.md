...
현재 목표는 **BLOCK/RECHECK를 ENTER_NOW로 전환할 수 있는 기계판정 임계치 조합을 평가하여 종목 유형·상태에 맞는 기계정책을 만드는 것**이다. 아래 순서를 현재 작업으로 수행한다.
...
사용자의 이전 구현·배포·즉시 적용 승인은 이력으로 보존한다. 최신 지시대로 **전체 운영 손익 검증은 후속이며 기계정책 생성 gate가 아니다. 기계정책의 적용·재기동·장후 연결은 현재 완료 조건이다.** 기존 AI는 기계정책 생성과 별개의 현행 런타임 판단을 계속 소유한다.
