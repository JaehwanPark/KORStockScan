...
9/21 추가 승인: 사용자 요청에 따라 [preflight·제출병목 결함 보완](../audit-reports/2026-09-21-preflight-submit-bottleneck-repair.md), 검증된 immutable 배포 및 메인 graceful 재기동을 수행한다. baseline 복귀는 원본 날짜·해시를 보존하며 실제 입력 freshness guard를 유지한다. 해당 명시 승인 외의 과거 권한 경계와 독립 위젯/에피소드 owner는 유지한다.
