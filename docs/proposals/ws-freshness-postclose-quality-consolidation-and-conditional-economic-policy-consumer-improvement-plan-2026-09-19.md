...
제한 재생성에서 complete pair3건·3일의 비용 후 `selection_opportunity_ev_pct=-2.38954987`과 날짜별 `-0.43517204/-5.79902111/-0.93445645%`가 생성됐다. 상태는 `hold_no_edge`이며 다음 장전 정책은 bonus0·apply false다. 이는 구조적으로 유효한 음수 연구값과 baseline 보존 결론이다. 미선정 arm의 체결수량이 없으므로 원화 일별 순익과 실제 paired EV는 null을 유지하고, 향후 사전 배정 arm의 자연 완료손익으로만 채운다.
