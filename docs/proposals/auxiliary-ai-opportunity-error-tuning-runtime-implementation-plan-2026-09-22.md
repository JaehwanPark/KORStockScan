...
9/26 실제 프롬프트 검증: 9/23 고정 원천의 적격 11건에 대해 기존 v3/opportunity/risk를 각 11회씩 실제 제공자로 호출했고 의미 검증 통과는 9/11·11/11·9/11이었다. v3 실패는 CAUTION에 `NO_BLOCKING_RISK`를 섞거나 blocking risk 없이 VETO를 낸 계약 위반이었다. 이 규칙을 명시한 `contract_v4` 연구 호출 11/11건은 유효했고, 같은 날 고정 10분 비용반영 경로의 짝비교 EV는 기회당 +0.06668%p였다. 이는 실제 체결 손익이 아니며 9/23 사례를 보고 문구를 만들었으므로 독립 holdout이 아니다. v4는 기존 risk 후보를 대신하는 **장후 평가 후보**로만 등록하고 운영 기본 v3는 유지한다. 9/22에 적격 15건의 별도 원천이 있으나 시간 순서상 이미 본 9/23을 미사용 holdout으로 재표기하지 않는다. 다음 적격 자연일에서 후보/holdout·비용 후 선택과 실제 PID 소비를 다시 검증한다. 동시에 장후 실제 호출 전 `TokenCeiling` 생성 실패를 수리하여 보수적인 요청 바이트 한도를 예약하고, 실제 경로 1회 호출과 회귀 테스트로 확인했다.
