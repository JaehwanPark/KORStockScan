...
User authorized implementation, review and deployment/restart after the historical-only alert. The monitor now suppresses historical-only incidents, including machine policy receipt history. An identity incident whose active alert was delivered may send one normal-observation notice after fresh identified events with zero recent gaps. Stale/no-event inputs do not assert health; send failures and cooldown preserve retry intent. A recurrence re-arms the active notice and preserves history, and current examples/timestamps take precedence over old ones. Current identity defects no longer wait for the 10-minute maturity/4-minute persistence gates; the existing five-minute schedule and notification cooldown remain. Other bottleneck persistence criteria and all trading guards are unchanged.

Reviewed snapshot producer, recency/incident transition, saved legacy state migration, notification batching/filtering, failure acknowledgments and existing selected-release cron wrapper. Targeted source tests:230 passed. External test messages were not sent. Deployment receipts follow under `tmp/monitor-notification-20260922/`.

### Notification deployment

- Source commit `c8f9565fa6359f4d8edb7e1c37f8059b2c1a9fc5` pushed; source and clean managed release each passed286 tests (monitor, Sentinel, release router), with no deselections. Compile/diff and checklist parser passed.
- Selected release `monitor-notification-20260922-c8f9565fa`; guarded restart at18:22:18, PID286554 ->294296. Bootstrap/env handoff PASS, main singleton, policy current receipt byte-identical. Inactive machine service and cron router point to the selected reviewed release.
- Initial no-notify CLI check retained all seven historical identity evidence events and queued no notifications. It rejected the already-consumed source as duplicate_or_reversed_source_snapshot; this is not a fresh healthy-observation acceptance. External test messages were not sent. Fresh scheduled verification follows separately.
- Evidence: `tmp/monitor-notification-20260922/` (`release-tests.log`, `release-validation.json`, `deployment.json`, `restart.log`, `cron-check.json`, `acceptance.json`, original state/selection and policy backups). Rollback uses `selection-before.json` and `machine-service-before.conf` plus the standard guarded restart. No historical evidence repair or live EV improvement is claimed.

Fresh scheduled acceptance at18:25:25 (wrapper DONE18:25:26): status=observing, blocker=null, recent identified events54, missing0, historical identity count7 preserved, notification_pending=[], notification_status=idle. The new current_missing_first_at/last_at producer fields confirm the scheduled producer used the new generation. `scheduled-acceptance.json` preserves the full result. Source/code/review/release/PID/scheduled consumer closure is complete; no external test notice was sent and historical evidence was not repaired or discarded.
