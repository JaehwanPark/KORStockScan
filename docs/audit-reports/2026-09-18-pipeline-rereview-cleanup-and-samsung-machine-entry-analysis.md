...
기존 report/strict reader에 shape 검사만 보완했다. 잘못된 policy는 cache miss 후 기존 정상 checkpoint로 실제 계산·parity PASS/fallback을 복구한다. 잘못된 producer rollup은 producer_summary_invalid와 producer_rollup_contract_invalid를 출력한다. 기존 --allow-bootstrap의 명시적 단일 날짜 복구는 손상된 producer rollup만 다시 읽어 정상 count/identity/PASS·재사용을 회복하고 정상 raw checkpoint를 재스캔하지 않는다. 이 복구 flag는 cached invalid terminal 재사용보다 우선하며 실제 CLI 회귀로 확인했다. 잘못된 policy/parity의 strict 인계는 report_contract_invalid·같은 native owner의 OPEN이다. 정상 지원 입력을 모두 inactive로 만드는 수정이 아니다. 또한9/17 실제 native strict는 DONE 미발행 때문에 flag가 없는데 생성된 exact-date 진단을 disabled로 표시했다. 명시적False만 OFF로 유지하고, 생성된 진단은 flag가 없어도 소비하며 파일/선언 둘 다 없으면 not_declared로 구분했다. 없는 실행을 합성하거나 source gap을 정상 완료로 만드는 권한은 없다. 새 producer/service/module은 없다.

최종 report/strict/controller 영향 회귀329PASS, logger/summary/Sentinel/strict/handoff 회귀400PASS다. 앞선 report56PASS는 추가 strict 보완 전 중간 검증이며 최종 수에 합산하지 않는다. compile·diff·링크·parser 및 selected root/공유경로/router·선택 CAS는 최종 validation/deployment receipts를 따른다. 실행 중 immutable 소스와 다른 세션의 canonical dirty 변경은 보존한다.
...
9/21 Samsung PREOPEN builder의 read-only 검사도 candidate source/report/audit/hash/runtime binding 검증을 통과했다. source9/16·policy_mutations0인 기존 baseline/carry 전달이며, 함수 reason candidate_applied는 반환값이고 실제 applied 파일 발행/장전 확정이 아니다. [reader 검증](../../tmp/pipeline-rereview-samsung-analysis-20260918/Samsung-reader-validation.json)에 applied_file_written=false를 고정했다. pinned source 비교에서 morning machine과 confirmation resolver는 현재 검토본과 같지만 regular two-leg machine은 최종 liquidity freshness 재검사 등 upstream 보완이 다르다. [source 비교](../../tmp/pipeline-rereview-samsung-analysis-20260918/Samsung-pinned-source-comparison.json)를 근거로 독립 service owner가 다음 영업일 준비에서 pin/guard/실제 소비를 검증해야 한다. 이번 pipeline 선택이 해당 service 갱신을 대신하지 않는다.
