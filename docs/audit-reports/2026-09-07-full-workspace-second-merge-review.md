...
### 직접 merge/push 및 main graceful restart receipt

- 커밋: formatting `bcc51382`, test isolation `089f6dd7`, PR 규칙/검증 기록 `5e05927a`. 기능 브랜치 push 후 로컬 main을 일반 `git merge --no-ff`로 병합하고 force 없이 origin/main에 push했다. merge commit은 `81fac7dad592457ed6fd23a44eeab2604304421e`다. 기존 PR58은 merge API/승인 리뷰 없이 이 commit의 포함 관계로 자동 `MERGED`가 됐다.
- `./restart.sh` exit0. old main PID356899 자체 graceful 종료는 16:59:13, new PID653712 시작은 16:59:24 KST다. supervisor PID46586을 유지했고 동시 bot singleton을 만들지 않았다. 삼성 오전 owner는 inactive라 prepare/commit 모두 `morning_owner_not_active/not_required`였으며 서비스 시작이나 권한 재발급을 하지 않았다.
- 새 PID env의 commit은 위 merge commit, `KORSTOCKSCAN_RUNTIME_SOURCE_DIRTY=false`, active date는 `2026-09-07`이다. 당일 runtime verify artifact는 PID653712 `status=pass`, findings `[]`이다. `16:59:44` DB/계좌 동기화, `16:59:46` WS 연결·로그인 응답, `16:59:49` 첫 0B/0D, 보유 종목249420/304100의 첫0D를 각각16:59:49/52 확인했다.
- 재기동 직전16:58:53.847485/직후16:59:45.119136 broker KRX/NXT canonical snapshot SHA-256은 모두 `e2488f26a7f9a1c37b2dbaa2b18b47bf0972de2121b10a848197bc330bf67caf`다. 잔고249420/304100 각1주, 미체결0, unresolved intent0, policy 및 manual list hash 불변이다. 로그는 `/tmp/prless_merge_0907_broker_before_restart.log`, `/tmp/prless_merge_0907_broker_after_restart.log`, `/tmp/prless_merge_0907_restart.log`에 있다. 신규 주문·취소를 검증 목적으로 실행하지 않았다.
- 재기동 후 observer는 새 process epoch의 fresh snapshot을 쓰며 17:00:36 기준 `warming_up`, 0B418건, p99 0.136604ms다. 이 warm-up을 검증 완료 또는 과거 지연/결손 복구로 세지 않는다. 수동목록 전환·다음 PREOPEN 자연 소비는 기존 OPEN owner가 유지한다. 위젯 PID9704는 환경 접근 권한 blocker로 그대로 두었고 inactive episode도 기동하지 않았다.
- 최종17:02 확인: merge commit의 원격 [Black run34098154239](https://github.com/JaehwanPark/KORStockScan/actions/runs/34098154239)은 `completed/success`다. observer 새 epoch은 `healthy_observer_canary`, 0B1289건, p99 0.511466ms, capture active, drop0/writer error0이다. 과거16:46 source 결손과 epoch 경계는 그대로 남긴다. 이번 receipt 이후 문서 전용 commit은 실행 소스의 추가 재기동을 요구하지 않는다.
