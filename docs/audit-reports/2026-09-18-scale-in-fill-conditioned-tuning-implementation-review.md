...
실제 첫 재생성에서 기본 태그 전용 필터가 Main `SCANNER` 실제4건을 체결0으로 누락한 결함을 발견했다. 해당 결과를 superseded로 기록하고 기존 Main SCANNER/default custody를 포함하며 widget/episode/manual을 제외하도록 수리했다. 실제 inventory custody 회귀를 추가하고 수정 release에서 재생성한다.
