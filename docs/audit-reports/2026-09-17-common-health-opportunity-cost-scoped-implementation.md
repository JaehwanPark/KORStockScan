...
- Official reference gate: 10:54 KST 조회 upstream HEAD `953e5dbff123f437ab4d11a78a95191a685eb51f`; `kiwoom/core/ws_client.py`, `core/auth.py`, `realtime/packets.py`, `kiwoom/specs.py`, `_data/kiwoom_api_spec.json`의0B/0D/ka10004와 `postman/kiwoom-openapi.postman_collection.json`의ka10004를 대사했다. 현행 tree에 `kiwoom_docs`는 없다. LOGIN/PING/REG/REMOVE·real/demo URL·type별 clock/가격/호가/volume·suffix 계약을 확인했으며 미정의 underlying venue는 계속 null/UNKNOWN이다. packaged request example과 SDK의 data 형태 차이는 기존 wire 유지로 격리한다. 프로토콜/주문 권한 승격 근거가 아니다.
