...
- 구현 commit `c931731d2fb049b0783285b22e03c157f31764c7`. 첫 push는 병행 작업의 원격 main advancement 때문에 거절됐다. 격리 worktree에서 원격 `ec99fd33`를 merge하여 `f7298191cbdbfe1d791f6db371453f258a2eea04`로 main push 완료. 위젯 소스·wrapper·unit의 byte 변경은 없고, 작업본의 unrelated dirty source/document 파일은 hash를 대사해 보존했다.
- immutable release: `/home/ubuntu/KORStockScan-runtime-releases/widget-postclose-source-closure-20260917-f7298191`. shared path·wrapper executable·src/deploy clean·8 module import origin과 위 research contract SHA 일치를 확인했다. release 확장 회귀 **828 passed**, strict verifier **196 passed**. 공유 data symlink에서 mechanical CLI의 기존 상대 default path/no-follow guard가 충돌한 테스트 1건은 **동일 commit의 물리 worktree에서 1 passed**로 재검증했다. 이 CLI는 evaluation wrapper stage가 아니며 이 제한을 숨기려고 source guard를 변경하지 않았다. 실제 shared 경로에서 해당 standalone CLI 기본 호출은 이 변경으로 수리한 범위가 아니다.
- 10:15 source pin/reload·raw collector graceful restart와 inactive runtime observer start 완료. actual raw PID **183163**, observer PID **183275**, cwd/PYTHONPATH/command가 새 release에 일치한다. 실행 정책 seed는 오늘 없음; native observer **19종목 모두 `raw_only_no_seed`**이며 주문/신호/episode 생성 없음. 19 optional calibration projections도 실제 writer에서 생성됐다.
- evaluation은 새 release의 WorkingDirectory/PYTHONPATH/project/Python/ExecStart에 일치하고 inactive(PID 0), 기존 timer **09-17 20:10 active**다. 실제 evaluation PID·오늘 장후 완료는 아직 없다. 자원·local/shared admission budget·timer는 그대로다.
- 배포 전후 main PID **140621**, trader **28368**, Samsung/Doosan/Hanwha legacy collector **672/666/667** 및 global selection hash 보존. 오늘 dated runtime-policy file은 없고 owner/custody/env를 쓰지 않았다. 세 legacy collectors는 기존 pinned code를 계속 소비하므로 이들의 신규 projection 생성은 배포 완료로 주장하지 않는다. 기존 raw fallback은 유지한다.
- machine receipt: `data/runtime/widget_postclose_source_closure_2026-09-17.json`. 기존 effective config 원문/sha와 새 drop-in sha·actual PID/public import environment·module origins·bounded 원천 evidence·합성 성능 수치를 보존했다. 이번 drop-in 3개만 제거하고 reload·관련 read-only collector만 restart하는 rollback 경계도 기록했다.

기동 직후 실제 `SOURCE_ERROR`는 ka10004 BBO/ka10080 chart의 **shared admission wait-budget exhausted**로 명시 식별됐다. local 남은 예산과 shared effective/max limit·waited·stage·route는 receipt에 있고 HTTP/return code는 호출 전 defer라 null이다. `completed_bar_stale`은 별도 source-quality gap이다. source guard/budget를 완화하거나 이를 historical 598행의 원인으로 소급하지 않는다. 첫 전체19 market census와 후속 recurrence/AM·자연 평가·경제성은 동일 current OPEN owner에서 분리 판정한다.

10:19:48 latest snapshot의 actual 새 producer SHA·schema·exact symbol/bare KRX route/session/venue·원래 관측시각과 safety를 대사했다. **market expected19 = source_valid15 + classified_not_applicable0 + explicit_source_gap4, unclassified0**; 여기서 valid는 이 시점의 regular snapshot 범위이며 전체분봉/BBO/clean-baseline/EOD 연구 coverage가 아니다. gap은 `001550 completed_bar_stale`, `007810/361610 ka10080 completed_bars admission defer`, `100790 ka10004 BBO admission defer`다. 재시도에서 처음 `000150` defer는 PASS로 바뀌었지만 기존 raw/error receipt를 보존하고 recurrence 해결로 확대 주장하지 않는다. **advisory expected19 = source_valid0 + classified_not_applicable19(raw_only_no_seed) + explicit_source_gap0, unclassified0**. 19종목 native raw-only producer hash·실제 projection·신호/주문 없음도 bounded128KiB tail로 확인했다. 성장하는 당일 raw 전체 scan은 하지 않았다. 각 gap의 owner artifact·next action·closure test를 machine receipt에 결속하고 AM/20:10 평가·strict final handoff·새 정책 소비·비용 반영 경제성은 OPEN으로 남긴다.
