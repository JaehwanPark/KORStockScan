...
| --- | --- |
| 매일 19:30, 휴일 포함 | [장후 작업지시문](./postclose-tuning-result-review-task-instructions.md) |
...
- OpenAI Responses API, `gpt-5.6-sol`, reasoning `medium`, structured JSON을 사용한다. 작성과 독립 리뷰를 각각 호출하고, findings가 있으면 한 번 보완한다. 시도당 최대 4회, 날짜·문서별 최대 2시도이며 완료 후 중복 호출하지 않는다. 19:30 작업은 하루 한 번 cron이 호출하므로 실패 시 자동 재시도는 다음 날이다. 장중 문서 작업은 다음 분부터 남은 시도를 사용한다.
...
- 독립 API 리뷰와 print-only parser 검증을 통과하고, 검토 중 HEAD·참조 문서 hash·완료 receipt가 바뀌지 않았을 때만 대상 파일을 원자 교체한다. 실패 시 원본을 보존한다. 리뷰에서 수정 필요가 없으면 `unchanged`로 완료한다. 파서는 두 작업지시문을 backlog 입력으로 소비하지 않으며, 해당 문서의 게시 전 검사는 별도 구조/링크 검증이 담당한다.
...
`failed`는 원본 유지, `running`이 15분 이상 남으면 wrapper timeout/중단 여부를 확인한다. 공통 파일 lock으로 중복 writer를 막고 wrapper는 900초 후 종료한다. API 오류·리뷰 결함·동시 수정 원인을 해결한 뒤에만 남은 시도로 wrapper를 재호출한다. 횟수를 자동 초기화하거나 같은 실패를 무한 반복하지 않는다. `--remove`는 전용 cron을 제거하고 설정을 disabled로 저장한다. 설치·preview 성공과 19:30/장후 종료 후 자연 실행 성공은 별도 acceptance다.
