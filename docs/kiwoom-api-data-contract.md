...
| soft-stop dynamic grace | `current_ai_score`, absorption/reaction features, `buy_pressure_10t`, `tick_acceleration_ratio`, `curr_vs_micro_vwap_bp` | AI grace requires `holding_score_runtime_context.usable_for_soft_grace=true`. Micro support requires fresh feature context and trusted pressure. | May defer soft-stop briefly when positive microstructure and usable holding score agree. | Do not defer soft stop from AI score without provenance, heuristic pressure, stale quote, or hard source-quality gap. |
