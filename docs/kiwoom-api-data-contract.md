...
| late-loss AVG_DOWN retry | `current_ai_score`, peak/giveback/loss depth, hold time, `curr_vs_micro_vwap_bp` | AI support requires `holding_score_runtime_context.usable_for_scale_in_support=true`. If micro VWAP is present, `reversal_feature_source_quality` must be usable; the late-loss retry may bypass an adverse fresh micro VWAP threshold, but not stale or missing micro provenance. | Retry only after loss-path criteria and usable AI support pass. | Do not use a numeric AI score with missing `holding_score_*` provenance, and do not use stale/missing micro VWAP provenance to open retry support. |
| loss fallback AI context | `current_ai_score` | AI context requires `holding_score_runtime_context.usable_for_scale_in_support=true`. | May be used only as scale-in/loss fallback support metadata when provenance is usable. | Missing, stale, fallback, disabled, timeout, or lock-contention score is neutral display only. |
| soft-stop micro grace modifier | `current_ai_score`, absorption/reaction features, `buy_pressure_10t`, `tick_acceleration_ratio`, `curr_vs_micro_vwap_bp`, `soft_stop_final_action`, `soft_stop_extension_source` | AI grace requires `holding_score_runtime_context.usable_for_soft_grace=true`. The numeric AI score is a prior/composite support signal, not a standalone hard criterion. Micro support requires fresh feature context and trusted pressure. Dynamic grace and expert absorption are scorer/modifier inputs only; `soft_stop_micro_grace` remains the single real deferral authority. | May defer soft-stop for one bounded confirmation window only when active stop-relative band, usable AI provenance, and either AI prior support or strong fresh absorption support agree. | Do not defer soft stop from AI score without provenance, heuristic pressure, stale quote, hard source-quality gap, standalone dynamic-grace authority, or expert-defense-only extension. |
...
| entry AI remote guard / early-accel recheck / numeric consistency recheck | AI action/score/reason, `buy_pressure_10t`, `tick_acceleration_ratio`, `curr_vs_micro_vwap_bp`, minute-candle freshness | Pressure support requires trusted aggressor provenance. Micro VWAP support/risk and AI reason contradiction checks require `micro_vwap_available=true`, `minute_candle_window_fresh=true`, and usable `minute_candle_context_quality`. | May downgrade suspicious remote BUY or request bounded recheck only when current feature provenance is usable. | Do not treat missing/stale micro VWAP as bearish risk, support, or reason-contradiction evidence. Do not send recheck context without source-quality fields. |
| score65_74 recovery probe | `buy_pressure`, `tick_accel`, `micro_vwap_bp`, AI score/action, threshold family fields | Runtime and postclose audit require trusted pressure fields and minute-candle provenance fields. `micro_vwap_bp` is an alias of the same micro VWAP concept and must not bypass `micro_vwap_available`/`minute_candle_window_fresh`/`minute_candle_context_quality`. | Bounded entry unlock observation only after score band, hard safety, pressure, tick, and fresh micro context pass. | Do not create postclose EV/apply candidate rows from probe events with missing pressure or minute-candle provenance. |
| PYRAMID | `current_ai_score`, profit/peak, `buy_pressure_10t`, `tick_acceleration_ratio`, `curr_vs_micro_vwap_bp`, large sell print | AI support requires `_ai_score_available_for_scale_in=true`. Pressure support and large-sell clear require trusted aggressor pressure. Tick/micro support requires non-stale reversal feature context. | Pyramiding only when composite support score and hard safety gates pass. Runtime prior may relax one soft borderline blocker only; hard safety is not relaxed. | Do not use `buy_pressure_10t` when `tick_aggressor_pressure_usable=false`; do not use stale micro VWAP or stale holding score for support. |
| REVERSAL_ADD / AVG_DOWN probe | `current_ai_score`, AI history, `buy_pressure_10t`, `tick_acceleration_ratio`, `curr_vs_micro_vwap_bp`, large sell print | `reversal_feature_source_quality` must be `usable`; AI must have explicit usable provenance. | May support AVG_DOWN only after PnL/hold/recovery and supply checks pass. | Do not treat pressure/micro metrics with missing provenance as valid supply confirmation. |
