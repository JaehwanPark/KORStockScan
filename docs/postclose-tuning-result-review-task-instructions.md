...
중첩 policy/parity·producer rollup의 schema 손상은 reusable PASS나 정상 상태로 수용하지 않는다. cache miss 후 지원된 checkpoint 계산을 재사용하거나 구체 invalid/owner를 반환하고 strict는 operations OPEN을 보존한다. raw/summary 본문 전수 복구나 provider recovery를 자동 실행하지 않는다.
