...
- machine entry timing 후보가 `supportive_confirmation_candidate` 완료 outcome만 사용해 entry 확인 지연 단일 축만 바꾸고, `adverse_veto_candidate|recheck_required|source_quality_blocked`를 승격 근거로 쓰지 않았는지 확인한다. 선택된 exact-date v2 scope는 signal→due exact-route fresh `ka10004` BBO, 비악화 bid·ask와 정책 고정비용 차감 양의 modeled owner-target edge를 다시 통과해야 한다. 이 값은 broker receipt exact 비용이나 미래 target 체결이 아니며, 이 확인이 신호·수량·가격·target·holding/exit 권한을 만들지 않았는지 점검한다. 별도 `machine_dynamic_micro_confirmation_replay_v1`은 exact signal/economic/source-parent binding, 각 `신호+0/1/3/5초` 직전 1초의 past-only same-epoch BBO·0B/0D, owner별 target 비율/틱, checkpoint별 5분 same-epoch executable target/adverse first-hit과 비용차감 equal-weight·notional-weighted EV, modeled 순이익 uplift·자본시간당 순이익을 재현하고, `REJECT`도 0 exposure로 denominator에 보존했는지 확인한다. 일부 checkpoint 결손을 만료 거부로 보간하거나 신호 뒤 첫 0B 기준의 미래 자료를 앞 checkpoint에 역류시키면 실패다. 이 결과는 source-only이며 `source_only_candidate_ready` 또는 파일 존재를 exact-date policy·다음 기동 적용으로 해석하지 않는다.
