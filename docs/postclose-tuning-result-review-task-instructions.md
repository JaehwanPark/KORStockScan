...
- 설정된 다섯 threshold group은 겹칠 수 있는 고정 taxonomy다. `configured_threshold_group_count|threshold_group_evaluations`를 매일 새 후보 수로 보고하지 않고, record event order의 단일 first explicit blocker 전수는 `primary_blocker_evaluations`, 그중 양(+) 비용 차감 EV·sample floor를 통과한 source-only existing-family evidence만 `threshold_opportunities`로 분리한다. 이전 generation 대비 `candidate_change_status`도 함께 대조한다. 양(+) EV만으로 코드 결함이나 `implement_now`를 만들지 않으며, 기존 family에 결속되지 않는 계측·join 결손이 별도 root-cause order로 입증된 경우에만 구현 대상으로 올린다.
- source coverage는 actual submit의 entry date와 같은 post-sell partition 존재로 판단하지 않는다. terminal `sell_completed record_id`와 `post_sell recommendation_id`의 exact join만 필수 coverage로 삼고 양쪽 stock code가 존재하며 일치해야 한다. submit 후 terminal sell이 없는 row는 `pending_or_right_censored`, terminal receipt 뒤 post-sell 누락만 `source_coverage_gap`으로 판정한다. 같은 `record_id`의 서로 다른 forced primary event, propagated forced event의 stock 충돌, 복수의 상충하는 post-sell outcome, stock-code 누락·불일치 또는 관련 malformed JSON은 last-row-wins·valid-empty로 덮지 않고 `source_identity_conflict|source_coverage_gap`으로 격리해 EV·AI·workorder를 fail-closed한다.
...
- 위 process-local bound 외에 운영 국내주식 계좌/토큰별 조회 TR 5회/초 공통 계약을 별도로 확인한다. scanner-prune·external-census·widget research/advisory·pure-market backfill은 같은 token+origin의 cross-process sliding 1초 gate에서 `source_only` 최대 4 slot만 사용하고, 한 slot은 `runtime_required|execution_critical` 조회에 남아야 한다. 주문 TR 5회/초는 별도 버킷이며 조회 coordinator가 주문 제출·정정·취소를 pacing/retry하지 않아야 한다. 모의투자는 token+origin+`api-id`별 1회/초를 사용한다.
- `ka10004_rate_limited`와 `ka10004_shared_read_budget_deferred`를 빈 응답·자연 BBO 부재·generic provenance invalid로 합치지 않는다. HTTP 429와 `1700|1701|1702`의 shared cooldown receipt, owner/PID/class/api-id/request code/attempt/wait/scope digest, token 비저장, malformed state fail-closed를 점검한다. bounded retry 뒤 exact valid 응답으로 복구된 요청은 `rate_limit_detected=true`, `rate_limit_retry_exhausted=false` warning provenance를 보존하되 실패 표본으로 폐기하지 않는다. 한도 대응으로 retry·일일 예산·호출 밀도를 높인 경우는 보완 실패다. 기존 PID는 수정 전 code snapshot이므로 fresh PID 자연 표본의 admission과 exact response/gap receipt 전에는 process reflection을 완료로 닫지 않는다.
...
- machine entry timing의 fixed mode가 `supportive_confirmation_candidate` 완료 outcome만 사용하고 기존 20건·95%·complete 5/10/20일 기준을 지켰는지 확인한다. 별도 `machine_dynamic_micro_confirmation_replay_v2`는 exact signal/economic/source-parent binding, 각 `신호+0/1/3/5초` 직전 1초 past-only exact-route same-epoch BBO·0B/0D, direct support와 저점 대비 rebound, trade backing/refill, owner별 target 비율/틱, checkpoint별 5분 executable target/adverse first-hit과 비용차감 equal-weight·notional-weighted EV, modeled 순이익 uplift·자본시간당 순이익을 재현해야 한다. 일부 checkpoint 결손을 거부로 보간하거나 future label을 action input에 역류시키면 실패다. 동적 mode의 다음-session bounded canary는 5 observed dates·8 unique/8 completed·replay/paired 85%·right-censored 35% 이하·complete 5 source-day positive/improved EV·순이익·자본효율·0.005%p uplift·p10과 최근 1 KRX 거래일 freshness를 모두 통과한 한 scope만 `machine_entry_timing_policy_applied_v3`로 발행할 수 있다. 이틀 연속 무신호·source/cost/hash 결손·same-stage mutation은 baseline 즉시진입이다. snapshot 파일/상위 계약 결손은 blind 5초 wait 없이 즉시, 정상 snapshot의 exact-route 일시 결손은 bounded terminal에서 기존 owner guard 전체 재검증으로 fallback하며 신규 BUY/DROP 신호로 보간하지 않는다. 이 축이 신호·수량·가격·target·holding/exit·provider/bot/cap·broker/hard-safety 권한을 만들지 않았는지 확인한다.
...
25. 국내주식 조회 TR은 같은 token+origin의 cross-process 5회/초 보존식과 source-only 4/5 reservation을 충족했고, 주문 TR과 분리됐다. `ka10004` HTTP/body limit와 local defer가 owner/PID/request-code별 source-quality gap으로 남았으며, fresh PID의 자연 receipt 없이 구현·테스트만으로 runtime 반영을 선언하지 않았다.
