...
- 두산에너빌리티·한화오션의 실주문 승격은 매일 20:10 장후
  ...
  읽어 관측만 계속한다.
- 장후 Widget 평가 stage는 `update_kospi_YYYY-MM-DD.json`의 완료 상태, 정확한
  날짜와 양수 행수를 계산 슬롯 획득 전에 확인한다. EOD 대기는 기존 예산 안에서
  슬롯을 점유하지 않고 진행하며, 실패·만료·날짜 불일치는 `deferred`로 남긴다.
  worker의 기존 EOD 재검사와 stage 입력 해시 검증은 그대로 유지한다. 재실행은
  해당 날짜 원천이 준비된 경우에만 기존 stage 명령으로 수행한다.
