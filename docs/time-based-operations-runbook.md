...
- postclose 기본 실행은 실매매 판단, source-quality, AI review, PREOPEN apply에 직접 필요한 producer로 제한한다. `ai_watching_score_smoothing_diagnostic`, `codebase_performance_workorder_report`, `time_window_regime_counterfactual`, `producer_gap_discovery`, `stage_hook_workorder_discovery`, `stage_hook_runtime_scaffold`는 기본 OFF이며, 필요한 날짜에 대응하는 `THRESHOLD_CYCLE_RUN_*` env를 명시적으로 켠 경우에만 실행한다. OFF artifact는 당일 freshness 필수 산출물로 취급하지 않는다.
...
| `09:05~15:30` | operator-disabled | `panic_buying_report` cron 미설치 | 기존 산출물은 archive-only | 기본 실행·stale 검사를 하지 않는다. 명시적 운영 지시가 있을 때만 `KORSTOCKSCAN_PANIC_BUYING_REPORT_OPERATOR_OVERRIDE=true`로 제한 실행한다 | 자동 복원 금지. postclose 기본 OFF, 직접 CLI와 wrapper도 명시적 override 없이는 거부 |
...
7. 장후 raw-derived producer parity는 `counts_only_v1`로 생성해 bucket identity/count/coverage만 보존한다. per-second count, sample event, numeric stats, raw offset은 parity 판단에 불필요한 중복이므로 저장하지 않는다. `pipeline_event_verbosity` JSON/Markdown이 raw source, producer summary/manifest, report/summary 코드보다 모두 새롭고 JSON 검증도 통과할 때만 복구 실행에서 재사용하며, 입력·코드가 하나라도 바뀌면 다시 생성한다. Entry ADM, lifecycle bucket daily/rolling, latency recommendation의 wrapper stdout은 `--print-summary`로 제한하되 파일 산출물은 기존 full schema를 유지한다. 이 최적화는 `runtime_effect=false`이고 raw suppression을 활성화하지 않는다.
...
| `kiwoom_auth_8005_restart` | fresh runtime log에서 `8005 Token이 유효하지 않습니다` 계열 인증 실패 감지. 기존 offset 이전 로그, pytest fixture signature, `run_error_detection*` meta log는 제외한다. graceful PID handoff 뒤 발견된 timestamp가 새 PID 시작보다 앞선 행은 이전 PID 종료 구간으로 귀속해 소비하고 현재 장애로 재경보하지 않는다. timestamp가 없거나 새 PID 시작 이후인 행은 계속 actionable이다 | `restart.flag` 기반 graceful restart 후 새 PID, WS 수신, REST 시세/잔고 응답 회복을 확인한다. 이 detector는 현재 PID의 warning도 Telegram alert 대상이다. 하루 3회까지 restart 복구를 허용하고, 이후 fresh 8005는 추가 `restart.flag` 없이 fail artifact/Telegram으로 operator 확인을 요구한다. Kiwoom REST/account/order 호출 지점은 8005 응답에 한해 token cache invalidation, force-refresh, same-request 1회 retry를 수행할 수 있다. 성공한 token handoff는 process-local bounded replacement map에 등록해 기동 시 token을 보유한 장수 caller의 다음 첫 요청부터 갱신 token을 사용한다. WS auth refresh도 동일 handoff를 등록한다 | threshold/spread/order guard 변경, provider route 변경, retry loop 확장 |
...
tail -n 80 logs/run_panic_buying_cron.log  # archive-only; scheduled job is disabled
...
KORSTOCKSCAN_PANIC_BUYING_REPORT_OPERATOR_OVERRIDE=true PYTHONPATH=. .venv/bin/python -m src.engine.panic_buying_report --date $(TZ=Asia/Seoul date +%F) --print-json  # explicit operator instruction only
