...
| `20:10` | cron | `deploy/run_threshold_cycle_postclose.sh` with OpenAI correction, 기본 `THRESHOLD_CYCLE_POSTCLOSE_BOT_ACTION=stop` | postclose threshold-cycle reports, ADM/LDM, source-quality audit, code improvement workorder, runtime approval summary, postclose verification, 다음 영업일 checklist | `[START]/[DONE]/[FAIL]` marker, bot stop/isolation marker, required report freshness, AI review parsed 여부, source-quality gate, workorder handoff, verifier status를 확인한다. postclose wrapper가 시작되면 기존 bot 세션은 wrapper owner로 종료되어야 한다 | 장후 resource isolation 정지는 threshold/order/provider 변경 권한이 아니다. report/proposal/verification 산출물만으로 실주문, provider, bot restart, cap, hard-safety를 변경하지 않는다 |
...
| `KORSTOCKSCAN_ERROR_DETECTOR_BOT_EXPECTED_START_HHMM=07:40`, `KORSTOCKSCAN_ERROR_DETECTOR_BOT_EXPECTED_END_HHMM=20:10` | bot 정상 기동/종료 스케줄 기준. window 밖 dead/stale heartbeat는 `expected_stopped` pass | runbook의 `07:40` 기동, `20:10` postclose 시작 종료 스케줄과 함께 변경 |
...
`build_codex_daily_workorder --slot POSTCLOSE`는 이 절차를 `PostcloseAutomationHealthCheckYYYYMMDD`로 자동 포함한다. 이 항목은 날짜별 개별 구현 backlog가 아니라 `Runbook 운영 확인` 큐다. 20:10 postclose wrapper 이후 자동 감시 범위는 `postclose_done_controller` completion과 22:30/22:40 보관/로그 정리, 23:05 EOD 데이터 갱신까지이며, `codex_workorder_runner`는 사용자 지시 또는 수동 opt-in 실행 결과가 있을 때만 별도 확인한다.
