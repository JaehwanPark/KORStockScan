...
20:10 threshold-cycle wrapper는 기본값 `THRESHOLD_CYCLE_RUN_SAMSUNG_MACHINE_ENTRY_TUNING=true`에서 삼성전자 오전·midday·오후 독립 기계의 당일 state를 `samsung_machine_entry_tuning` JSON/Markdown과 다음 거래일 candidate로 원자 생성한다. 과거 시세/API를 조회하지 않고 자기 이전 report만 clean baseline 이후 누적하며, `observation_source_quality_audit.tuning_input_allowed=true`, cumulative/rolling EV·표본, `HELD`·미해결 guard를 모두 요구한다. candidate 자체는 runtime effect가 없고 오전은 baseline-only, midday/오후는 전체 동일 entry stage에서 하루 한 기계·한 axis tightening만 허용한다.

같은 wrapper의 기본값 `THRESHOLD_CYCLE_RUN_LOW_PRICE_TWO_LEG_TUNING=true`는 삼성중공업 midday/오후와 SK이터닉스 midday의 실제 profile state를 서로 섞지 않고 `low_price_two_leg_tuning`으로 누적한다. 과거 시세를 재조회하지 않으며 cumulative completed leg 20, rolling10/20 completed leg 각 3, 각 window 양의 notional EV, source-quality PASS, `HELD`·미해결 0을 모두 요구한다. 전체 regular-entry stage에서 기존 Samsung candidate까지 포함해 하루 한 machine/profile·한 tightening axis만 다음 PREOPEN candidate가 될 수 있다. Samsung candidate가 먼저 유효 mutation을 소유하거나 같은 날짜 artifact가 invalid이면 lower-price family는 전 profile을 carry-forward한다. 수량 2, 50:50 one-share leg, +2틱, five-bar validity, SOR, 무손절·미청산 보유와 broker guard는 불변이다.

그 다음 기본값 `THRESHOLD_CYCLE_RUN_LOW_PRICE_TWO_LEG_CANDIDATE_RECOMMENDATION=true`는 검토된 9종목 후보군을 최근 46 KRX 거래일의 30일 calibration/16일 untouched holdout으로 다시 평가한다. 모든 종목의 integrated-SOR 분봉 거래일과 source quality가 일치하고 표본·양의 notional EV·held 0·최신 종가 10만원 이하를 통과한 종목×시간대만 source-only 순위로 만든다. cached token 또는 공통 source 계약이 막히면 `source_quality_blocked` 보고서와 추천 미산출 admin 안내를 보내고 다른 장후 producer는 계속한다. JSON/Markdown 원자 기록 뒤 `ADMIN_ONLY` Telegram을 최대 3회 시도하며 target-date state로 recovery 중복을 차단한다. Telegram 설정 누락·전송 실패·authority 위반은 postclose FAIL이고, 정상 추천은 기계 생성, timer 설치, PREOPEN policy, 주문 또는 runtime mutation 권한이 아니다.
...
Lower-price profile PREOPEN wrapper도 같은 순서로 `low_price_two_leg_policy_apply -> profile preflight -> live service`를 실행한다. 삼성중공업 midday와 SK이터닉스 midday는 13:10 preflight/13:14 service, 삼성중공업 afternoon은 13:55 preflight/13:59 service timer가 소유한다. profile별 manual-operator exclusion, 연구 evidence, exact-date applied hash, shared token, main bot active 조건 중 하나라도 빠지면 해당 profile만 시작하지 않는다. 상세 계약과 설치/rollback은 [lower-price two-leg machines](./low-price-two-leg-machines.md)가 소유한다.
