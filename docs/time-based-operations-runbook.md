...
| producer gap discovery | sim/probe/real-flow 결과에서 누락 producer 후보를 source-only로 발굴하고 AI review로 구현요건을 보강해 workorder에 넘김 | 누락 producer 후보를 live 적용, 실주문 전환, threshold/provider/bot/cap 변경 근거로 사용 | `producer_gap_discovery_YYYY-MM-DD.{json,md}`, `code_improvement_workorder`, postclose verifier |
...
3. 각 후보의 `bridge_candidate_state`를 본다.
   - `live_auto_apply_ready`: contract, env key, runtime hook, post-apply attribution 경로가 닫혔고 parsed AI Tier2 review가 명시적 gap을 찾지 않았다. 별도 approval artifact 없이 다음 PREOPEN live auto apply 후보로 소비한다. AI unavailable/parse fail은 fail-closed 차단이고, parsed 모호함만 follow-up이다.
