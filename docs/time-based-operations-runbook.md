...
| `15:10~15:15` | runtime | KRX SCALPING 신규 BUY hard cutoff 이후 주문·체결 정리 | BUY submit guard, pending-order receipt | 15:10 이후 신규 BUY가 넓은 env 또는 일반 time-block OFF로도 제출되지 않는지 확인한다 | threshold·수량·provider 변경이 아니라 no-overnight terminal buffer다 |
| `15:15~15:30` | runtime | KRX-only SCALPING same-session terminal exit | holding pipeline, exact order/receipt ledger | `same_session_terminal_exit_selected -> exit_signal -> sell_order_sent -> exact fill/terminal receipt`를 확인한다 | 기존 fresh quote, sell window, SOR, account/order/quantity/cooldown, pending-entry cancel, exact receipt guard를 우회하지 않는다. NXT 가능 포지션은 이 구간에서 강제 종결하지 않는다 |
...
| `19:45~20:00` | runtime | NXT·sim SCALPING same-session terminal exit 및 종료 reconciliation | holding pipeline, sim post-sell candidate/evaluation, heartbeat terminal fields | NXT/sim의 동일 terminal chain과 20:00 `unresolved_scalping_count=0`을 확인한다. `BUY_ORDERED`, sim pending BUY, `HOLDING`, `SELL_ORDERED`가 남으면 `SCALPING_SAME_SESSION_TERMINAL_FAIL` 및 Telegram 사고 알림이 있어야 한다 | overnight carry·AI HOLD 권한은 없다. 미종결을 COMPLETED로 합성하거나 broker/quote/receipt guard를 우회하지 않고 다음 기동 exact reconciliation 대상으로 보존한다 |
...
- 초기 보호 mode는 `KORSTOCKSCAN_AI_INPUT_PREFLIGHT_MODE=baseline_v1`이다. clean baseline 이후 real 이벤트와 당일 source-quality audit로 `data/report/ai_input_quality_baseline/ai_input_quality_baseline_YYYY-MM-DD.json`을 생성한다. legacy field는 source-quality proxy일 뿐 exact venue provenance가 아니며, 정책은 provider 호출·scale-in support·exit defer 권한을 줄일 수만 있다. SCALPING overnight HOLD 권한은 2026-09-06 폐기되어 어떤 preflight 결과도 다시 열 수 없다.
...
- holding-flow는 주기 계좌 동기화의 `kt00005 + ka10075` broker position/open-order snapshot과 실제 entry broker route를 대사한다. 결손·60초 초과·venue mismatch에서는 AI가 scale-in을 지지하거나 deterministic exit를 유예할 수 없다. same-session terminal exit 역시 이 계좌·주문·receipt guard를 우회하지 않는다.
- 검증 명령:
