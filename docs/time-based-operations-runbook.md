...
| `16:05` | cron | `deploy/run_postclose_done_controller.sh`, predecessor running 또는 status artifact missing이면 기본 `POSTCLOSE_DONE_CONTROLLER_PREDECESSOR_TIMEOUT_SEC=14400`까지 대기, 기본 wrapper rerun 허용, Codex runner enabled | controller artifact, verifier final pass, optional recoverable source refresh, optional same-date postclose wrapper rerun, Codex workorder runner artifact, dedicated worktree branch commit | `data/report/postclose_done_controller/postclose_done_controller_YYYY-MM-DD.{json,md}`, `logs/postclose_done_controller_cron.log`, `data/report/codex_workorder_runner/codex_workorder_runner_YYYY-MM-DD.{json,md}`, `tmp/codex_worktrees/codex-workorder-YYYY-MM-DD`, `codex/workorder-YYYY-MM-DD-*` branch | recoverable warning/fail은 source refresh, retry queue/workorder rebuild, verifier rerun, guarded wrapper rerun으로 `threshold_cycle_postclose_verification.status=pass`와 wrapper/controller `[DONE]`까지 닫는다. wrapper는 controller report status가 `done` 또는 `dry_run_planned`일 때만 Codex runner를 시작한다. 이후 `implement_now` 중 `runtime_effect=false`, `allowed_runtime_apply!=true`, forbidden-use scan 통과 항목만 Codex SDK로 구현/review/fix/검증하고 dedicated worktree branch에만 commit한다. 기존 worktree가 dirty이면 삭제하지 않고 `worktree_dirty`로 차단하며, clean worktree도 current HEAD와 다르면 `worktree_stale_head`로 차단한다. 자동 커밋은 tracked/staged/untracked 전체를 검사하고 docs/tests/automation/monitoring/error-detector safe-scope allowlist path에 한정한다. 선택 order의 `acceptance_tests`는 허용된 pytest/parser/py_compile 명령만 실행하고 미지원 명령은 commit blocker로 기록한다. Codex auth가 없으면 `CODEX_WORKORDER_ALLOW_INTERACTIVE_LOGIN=true`일 때만 device-code wait를 수행하고, 그 외에는 `codex_login_required`로 닫는다. package/auth/timeout은 각각 `codex_package_unavailable`, `codex_login_required`, `codex_login_timeout`으로 기록한다. non-implement 항목은 triage artifact로 재판정한다 | real order authority, PREOPEN live env, provider route, bot process state, cap release, broker/order guard, hard/protect/emergency safety 변경 금지. validation과 forbidden scan은 staged/unstaged/untracked 전체를 포함하도록 HEAD 대비 diff와 untracked path를 검사하며, live engine/runtime path는 명시 금지 문구가 없어도 commit blocker다. SDK/auth/package unavailable, safety veto, real-runtime authority gap, repeated wrapper crash는 fail을 숨기지 않고 `blocked_non_recoverable` 또는 runner blocked 상태로 닫는다 |
...
이 triage 자체는 runtime 변경을 자동 수행하지 않는다. 결과가 `needs_codex_instrumentation` 또는 `promoted`이고 safe-scope 조건을 통과하면 다음 Codex runner pass의 구현 후보가 될 수 있다. `design_backlog_required`는 source-only 설계 backlog로 남기고, `attached_to_existing_family`이면 다음 threshold-cycle/daily EV 산출물에서 재평가되도록 두며 runtime threshold나 주문 guard를 수동 변경하지 않는다.
