...
When the latest exact-date cleanup terminal is `FAIL` solely for an invalid micro-storage receipt and every required generic-lane failure counter is explicitly zero, an explicit finalization recovery may additionally use `--recover-storage-only`. The cleanup wrapper validates that completed generic receipt (today/yesterday, actual aware completion within one hour, no later same-date START/DONE), binds its SHA256/original time, then reruns and strictly validates only micro storage. Purge is forbidden in this mode. Generic cleanup and writer-defer observation counters are not rerun or reset. Any absent field, other failed lane, expired receipt or new active attempt rejects reuse. The new DONE discloses `generic_cleanup_reexecuted=false`; normal cleanup remains unchanged.

The storage maintenance CLI reserves stdout for its one JSON receipt and routes lazy dependency diagnostics to stderr. The wrapper does not strip unknown text, suppress a parser failure, or accept exit 0 without validating schema, authority, counters and the capacity artifact/hash.

After an explicit recovery instruction and the review gate, `bash deploy/run_postclose_finalization.sh SOURCE_DATE --recover-closed-target` accepts only today/yesterday with a recorded exact-date finalization FAIL. It is not a scheduled deadline extension: the original failure remains, the target-date mutex must be free, and every predecessor plus the final strict summary must already pass. Recovery waits zero seconds; failed/missing predecessors prohibit cleanup. The existing cleanup receives the original target date and records its actual execution time. The detector receives that source date explicitly, verifies all seven detectors, and records the real recovery timestamp separately. Cron/artifact checks inspect the source date; process/resource/lock/log/auth checks observe current conditions, not reconstructed historical conditions. All detector mutations are disabled in recovery, and any remaining detector FAIL prevents finalization DONE. Normal scheduled runs retain the 23:20 deadline and existing bounds. Do not use `ALLOW_NONCURRENT_TARGET` to relabel today's detector as yesterday's.

The low-price `--refresh-receipts-only` mode preserves the existing report economics and validates unchanged leg fields before projecting durable manual receipts; it makes no broker calls. Attribution independently binds the manual order to its applied registry owner/date/quantity/price and explicit unavailable timestamp. It leaves fill time/duration null and timing eligibility false. Fully read, valid, closed source partitions with no exact entry market window can issue `closed_exact_market_window_exclusion_v2`: signal minus 30 seconds through plus 6 seconds, preserving pre-signal context and the final 5-second checkpoint's one-second window. Later rows in the enclosing 30-minute exit study do not repair this missing entry window. Source/identity/manifest corruption cannot use this exclusion. The original gaps remain in the report and the existing next-natural-source owner, while verifier/controller distinguish this terminal exclusion from recoverable structural defects.
...
- `lifecycle_bucket_discovery`와 `runtime_apply_bridge`는 retired이며 필수 보고서/승격 대기열이 아니다. 활성 family의 candidate/policy/receipt는 해당 독립 producer/consumer로 확인한다.
...
- 스윙은 현재 operator OFF다. 과거 dry-run/full-live 절차는 보관 참조이며 자동 재활성화나 현행 미완료 작업이 아니다.
...
- 사람이 개입하는 지점은 운영 장애, final full-live/cap/provider/bot/hard-safety 승인, Codex runner 차단 해소, 문서 backlog Project/Calendar 동기화다. safe-scope `runtime_effect=false` workorder는 사용자 구현 지시, 명시적으로 호출된 장후 모니터링의 허용 2-pass, 또는 승인된 opt-in runner 범위에서만 구현한다.
...
- 스캘핑 익절 강약 폐루프 v2의 장후 원천은 `postclose_exit` monitor profile의 같은 원천일 `trade_review → post_sell_feedback → holding_exit_observation`과 세 파일 SHA256 manifest다. 정오 `full` 보고서나 과거 M1 결손 자료는 후보 승격 입력이 아니다. PREOPEN publisher는 후보 없음/표본 부족을 hold 영수증으로 남기고, 적격 결합 정책만 동일 익절 owner의 `--receipt`로 bootstrap에 전달한다. 선택 영수증의 원천/부모 hash 오류는 조용히 기준값으로 대체하지 않는다. 코드 배포, 정확 날짜 정책, 실제 PID 소비, 자연 비용 후 결과를 각각 확인한다.
...
| swing runtime approval | 스윙 dry-run pre-final auto approval 생성, final-stage approval request 생성, dry-run runtime env 후보 연결 | Tier2 실패 상태의 env 반영, dry-run 해제, 사용자 승인 없는 full-live 전환, 승인 없는 real authority 생성, 브로커 주문 허용 | `swing_runtime_approval`, family dated policy receipt, runtime policy bootstrap JSON |
...
`runtime_apply_bridge`와 greenfield bucket authority는 2026-09-06 retired다. 과거 bridge 산출물의 부재를 복구하거나 env로 되살리지 않는다.

현재 적용 점검은 활성 dedicated family의 `source -> candidate/policy -> PREOPEN selection/receipt -> PID -> R6`를 따른다. `runtime_approval_summary`와 `runtime_apply_gap_audit`는 이 연결을 진단할 뿐 runtime executor가 아니다. `entry_cancel_wait_runtime`은 독립 deterministic owner, `wait6579_ev_cohort`는 raw/source-quality provenance이며 은퇴한 standalone bridge 권한이 없다.

| 차단 종류 | 확인·다음 조치 |
| --- | --- |
| source/contract 결손 | exact date/hash/row·scope를 대사하고 안정적 제외 또는 source-only 수리 |
| sample/maturity 부족 | 기존 natural acceptance에서 수집률·성숙시각·최초 소실 stage 확인 |
| measured no-edge | 유효 비용 차감 경제성 및 같은 cohort의 비교근거로 판정 |
| same-stage/authority | 현재 owner·rollback과 명시 권한 확인; 수동 env/lock 우회 금지 |
| OFF/retired | 정상 제외, 복구·승격 후보로 만들지 않음 |
...
| `07:32` | systemd timer | `korstockscan-symbol-owner-policy-auto-apply.timer` → standing-authority exact-date owner policy apply | `data/runtime/symbol_owner_policy/{auto_apply,requests,receipts}`, `owner_custody.env`, exact-date policy | root wrapper가 당시 active였던 widget/episode order service만 정지·복원하고, `ubuntu` runner가 현재 widget·episode 전체 종목 scope와 standing artifact hash를 검증한 뒤 KRX·NXT broker 잔고/미체결과 owner registry가 정확히 맞는 종목만 게시했는지 확인한다 | owner 미확정 잔고, registry deficit, 미등록·모호 미체결이 있는 종목은 해당 종목만 제외하고 기존 manual exclusion을 유지한다. broker 주문 취소·수량 migration 추정·threshold/safety 변경 금지 |
| `07:35` | cron | `deploy/run_threshold_cycle_preopen.sh` | family별 dated publisher, 검증된 dated 보조 AI component의 현재 machine/AI parent CAS 활성화, `data/runtime/policy_bootstrap/runtime_policy_bootstrap_YYYY-MM-DD.{env,json}`, verify JSON, `logs/threshold_cycle_preopen_cron.log` | AI 후보가 없거나 machine/AI parent가 달라지면 기존 pair를 유지한다. 후보가 있으면 `runtime/mechanistic_entry_policy/current.json`의 새 bundle과 실제 PID 소비를 별도 확인한다. family receipt의 날짜·해시·apply 권한과 bootstrap의 incumbent/lock/OFF 합성, self hash, env hash를 확인한다 | 실패했다고 수동 env를 덮어쓰거나 퇴역한 공통 Daily/EV/PREOPEN selector를 복구하지 않는다. provider·주문·cap·hard safety 권한을 추가하지 않는다 |
...
| `09:05~15:20`, `16:00~19:20`; 장후 workorder 직전 source-binding finalize | cron + main postclose wrapper | `deploy/run_intraday_ws_freshness_monitor.sh` 5분 trigger, 기본 `INTRADAY_WS_FRESHNESS_MONITOR_ONLY=true`; growing JSONL 재탐색 부하를 제한하기 위해 성공 완료 기준 기본 cooldown 720초(실효 약 15분)를 사용하며 KRX/NXT가 같은 정책을 공유한다. main postclose는 기존 incremental state를 한 번 finalize하고 workorder보다 먼저 종료한다. 장중에는 target date 이하 최신 canonical symbol master를 strict hash/owner 검증 후 각 record의 effective window로 결속하고 실제 source date/path/hash를 남긴다. 미래 artifact는 선택하지 않고 최신 eligible artifact가 invalid면 과거본으로 묵시 후퇴하지 않는다. 장후 exact-date master가 생성되면 같은 incremental state를 다시 결속하며, 유효한 master가 없으면 wrapper를 중단하지 않고 `official_symbol_master_binding.status=missing|invalid` source-quality blocker를 최종 report에 보존한다 | `data/report/intraday_ws_freshness_monitor/intraday_ws_freshness_monitor_YYYY-MM-DD.{json,md}`, `data/runtime/intraday_ws_freshness_monitor/intraday_ws_freshness_monitor_YYYY-MM-DD.json`, `logs/run_intraday_ws_freshness_monitor_cron.log` | 최초 실행은 memory-bounded full streaming rebuild, 이후 실행과 장후 finalize는 안전 byte offset 이후 append row만 누적한다. 파일 교체·축소, schema/stale threshold 변경, invalid aggregate는 자동 full rebuild한다. `input_processing.mode`, `appended_event_count`, `incremental_state_reason`, source offset, `official_symbol_master_binding.status/source_date/artifact_sha256`를 확인한다. economic attribution은 전체 venue/session status와 cohort×venue×session status를 함께 보며, 다른 cohort의 source gap 때문에 독립적으로 95% BBO join floor를 통과한 cohort EV가 null이 되지 않는지 확인한다. `subscription_stale`, `trade_tick_quiet`, both-WS-stale, scout/submit 영향, `provider=none` incident를 분리 확인한다. NXT 구간에서는 통합 0B/0D route, 직접 체결가 결측, 0D quote proxy 복구, unresolved source gap을 별도 확인한다. 장중과 finalize 모두 monitor-only라 docs workorder를 직접 갱신하지 않는다 | source-quality/report-only이다. finalize는 새 WS 수집이나 runtime mutation이 아니라 이미 수집된 state와 exact-date master의 최종 결속이다. master/BBO 결손은 결손값 보간이나 wrapper 조기종료가 아니라 downstream tuning-input 차단과 workorder 근거다. cohort별 source-only EV는 slot/cooldown/threshold 완화나 실주문 권한이 아니다. NXT 결과를 KRX threshold에 혼입하거나 stale submit/broker guard를 우회하지 않는다. NXT threshold 변경은 별도 NXT runtime family와 근거가 있을 때만 허용한다. `THRESHOLD_CYCLE_RUN_INTRADAY_WS_FRESHNESS_FINALIZE`는 main AI-quality R0→R3가 ON일 때 기본 ON이며, provider·threshold·bot·order 권한을 만들지 않는다 |
...
| `08:05~08:55`, `09:05~15:30`, `16:00~19:50` | cron | `deploy/run_runtime_release.sh holding-exit-sentinel`가 선택된 불변 release의 `deploy/run_holding_exit_sentinel_intraday.sh`를 5분마다 실행한다. 기본 `HOLDING_EXIT_SENTINEL_USE_CACHE=1`; `deploy/install_stage2_ops_cron.sh`가 프리마켓·정규장·통합애프터마켓 trigger를 소유한다 | `data/report/holding_exit_sentinel/holding_exit_sentinel_YYYY-MM-DD.{json,md}`, `data/runtime/sentinel_event_cache/holding_exit_sentinel_events_YYYY-MM-DD.*`, `logs/run_holding_exit_sentinel_cron.log` | 기존 HOLD/EXIT 분류와 별도로 `profit_exit_semantics`의 3시장 익절 첫 crossing→신호 전 투표→SELL→terminal/비용 상태·정책 해시·source gap을 확인한다. `valid_empty`는 자연 표본 없음이며 실거래 적용 증거가 아니다. NXT event는 venue/session과 real/sim authority를 유지해 append 갱신한다 | Sentinel 결과로 자동 매도, threshold mutation, bot restart 금지 |
| `09:05~15:30` | cron | `deploy/run_panic_sell_defense_intraday.sh` 2분 주기 | `panic_sell_defense`, `market_panic_breadth`, `market_weakness_observations`, `tmp/market_weakness_observer_state.json`, cron log | pipeline JSONL은 single-pass memory-bounded streaming으로 읽고 exit/non-real provenance만 보존한다. 반복 exit signal은 broker order identity별 1회로 축약한다. 시장 약세 알림은 기본 2회 활성화·3회 해제이며, source-hash/OOS review가 닫힌 exact-date 정책이 있으면 activation 2~4·release 2~5 범위의 현재값을 observation에 고정해 사용한다. KOSPI·KOSDAQ과 최소 3개 업종 row, 60초 spacing, 명시적 recovery margin 계약은 변하지 않는다. 동일 observation 재사용, 60초 미만 재관찰, source-quality·authority·release-margin 결손, 날짜·policy hash 불일치는 streak를 전진시키지 않는다. 한 세션에서 최초 소비한 policy source/hash·activation/release/spacing은 고정하며 뒤늦게 다른 정책이 나타나도 기존 streak와 섞지 않는다. breadth 조회는 이미 승인된 market-weakness entry guard의 300초 freshness 입력이므로 token-wide shared read coordinator의 `runtime_required` 예약 슬롯을 사용하되 전체 5회/초와 다른 실행 중요 조회의 예약 계약은 바꾸지 않는다. admission 뒤 HTTP 200·return code 0인데 한 시장의 semantic row만 비었을 때에만 1회 추가 조회하고 admission/transport/rate-limit 실패는 반복 호출하지 않고 fail-closed한다. `2026-08-31` 사용자 승인 `WIDGET_EPISODE_MARKET_WEAKNESS_ENTRY_FREEZE_OPEN_BUY_CANCEL_V2`는 current-session `active|release_pending` market latch와 verified listing market이 일치하는 위젯·에피소드 신규·추가 BUY를 veto하고, 당일 exact owner 원주문을 broker reconciliation으로 확인한 경우에만 미체결 잔량을 취소한다 | notifier/attribution 자체로 score/stop threshold, 주문 취소·자동매도, bot restart, 스윙 실주문 전환 금지. 별도 live consumer도 수동·main bot·다른 owner 주문, SELL/target, 체결수량·보유, 수량 resize·가격·provider·broker guard를 변경하지 않으며 source/market scope 또는 현재 잔량 대사가 불명확하면 취소하지 않는다. rollback env는 `KORSTOCKSCAN_WIDGET_EPISODE_MARKET_WEAKNESS_ENTRY_GUARD_ENABLED=0` |
...
| `15:10~15:30` | runtime | KRX SCALPING 신규 BUY hard cutoff 이후 기존 holding 지속 | BUY submit guard, pending-order receipt, holding pipeline | 15:10 이후 신규 BUY가 넓은 env 또는 일반 time-block OFF로도 제출되지 않고 15:15 terminal SELL이 선택되지 않는지 확인한다 | 2026-09-14 이후 보유는 16:00 통합 애프터마켓으로 이어지며, 과거 날짜 replay만 legacy 15:15 경계를 보존한다 |
| `19:40~19:45` | runtime | NXT SCALPING 신규 BUY hard cutoff 이후 주문·체결 정리 | BUY submit guard, pending-order receipt | 19:40 이후 신규 BUY가 제출되지 않는지 확인한다 | terminal SELL 전 5분 reconciliation buffer이며 기존 hard safety를 완화하지 않는다 |
| `19:45~20:00` | runtime | 통합 애프터마켓·sim SCALPING positive-net terminal 평가 및 종료 reconciliation | holding pipeline, executable sell quote/cost fields, sim post-sell candidate/evaluation, heartbeat terminal fields | `terminal_exit_net_ev_pct > 0`이면 기존 SELL chain과 exact receipt를 확인한다. `<= 0`이면 terminal 신호 없이 기존 holding 로직을 계속한다. 20:00 정상 HOLD는 같은 거래일·종료 직전 60초 이내의 유효 판정이고 open BUY/SELL·close-reconciliation blocker가 없어야 한다. 그 외만 `SCALPING_SAME_SESSION_TERMINAL_FAIL`로 본다 | 비양수 terminal HOLD는 hard/protect/emergency 또는 일반 holding SELL을 차단하지 않는다. 미확인 상태를 HOLD/COMPLETED로 합성하거나 broker/quote/receipt guard를 우회하지 않는다 |
...
| `20:10` | cron | `deploy/run_threshold_cycle_postclose.sh` with OpenAI correction, 기본 `THRESHOLD_CYCLE_POSTCLOSE_BOT_ACTION=stop`, `THRESHOLD_CYCLE_RUN_SWING_POSTCLOSE=false` | postclose threshold-cycle reports, source-quality audit, rising missed feedback/scout workorder, 일일 Entry recheck drought controller, Entry AI gate 누적 진단 정기 실행 제외(별도 CLI on-demand 전용), PYRAMID feedback/calibration, entry cancel-wait tuning, 저가 기계 후보 추천·어드민 Telegram, microstructure reaction diagnostic, code improvement workorder, runtime approval summary, postclose verification, 다음 영업일 checklist | `[START]/[DONE]/[FAIL]` marker, bot stop/isolation marker, required report freshness, AI review parsed 여부, source-quality gate, rising missed feedback/scout outcome coverage, Entry recheck controller의 `drought_triggered_bounded_live` 후보 0~1건·`evaluated→armed→submitted→filled→completed→paired` funnel·scope별 escalation 목록, PYRAMID feedback/calibration, entry cancel-wait evidence state, 저가 후보 report가 clean baseline 전체·최신 16거래일 holdout·신규종목/기존종목 미구현시간대 lane·source-only authority·공통 0.23% 비용계약·Telegram delivery 계약을 충족하는지, workorder handoff, verifier status를 확인한다. PREOPEN은 일일 controller만 소비하며 누적 backtest로 fallback하지 않는다. wrapper의 과거 daily/weekly override로 누적 진단을 실행할 수 없고 진단 실패는 필수 체인에 전파되지 않는다. 누적 backtest는 `runtime_candidate_ready=false`, `allowed_runtime_apply=false`인 진단 전용이고 동일 target-date 중복 producer는 lock으로 차단한다. 저가 `ka10080` 수집은 symbol/date/request/content hash checkpoint를 재사용하고 shared-read defer가 계속되면 기존 정상 report를 덮어쓰지 않은 채 exit 75로 중단한다. wrapper는 완료 checkpoint를 이어서 최대 3회만 재개하며 그 뒤에는 FAIL로 닫는다. 저가 후보 Telegram 설정 누락·3회 전송 실패·authority 위반은 조용히 건너뛰지 않고 postclose FAIL로 닫는다. Entry recheck 계약 누락·복수 후보·3거래일 window·scope 불일치는 PREOPEN fail-closed한다. 2026-08-01부터 무효 반복하던 rising-missed bridge discovery, first-touch calibration, AI-score aggregate backtest는 producer·consumer·wrapper에서 제거하고 기존 산출물은 archive evidence로만 남긴다. standalone `quote_consistency_report`도 actionable 소비자가 없어 제거됐으며 실시간 `quote_consistency_normalization` 안전 로직은 유지한다. 2026-09-06부터 sole consumer가 폐기된 dedicated `institutional_flow_context` producer는 override와 무관하게 OFF이며 current EV/runtime summary는 old artifact를 읽지 않는다. microstructure report는 context v2/delivery telemetry v3의 computed/payload_included/confirmed_sent/internal_consumed/cache identity, 공통 quote freshness와 excluded-date 진단을 확인하며 별도 runtime 후보를 요구하지 않는다 | swing OFF 상태를 개별 child flag로 우회하지 않는다. report/proposal/verification/추천 산출물만으로 실주문, 기계 생성·기동, provider, bot restart, cap, hard-safety를 변경하지 않는다 |
...
같은 wrapper의 기본값 `THRESHOLD_CYCLE_RUN_LOW_PRICE_TWO_LEG_TUNING=true`는 exact target-date 실제 profile state를 종목·시간대별로 서로 섞지 않고 `low_price_two_leg_tuning`으로 누적한다. exact-date inventory는 `2026-08-19=20`, `2026-08-21=27`, `2026-08-24=35`, `2026-08-25~26=40`, `2026-08-27=45`, `2026-08-28~30=46`, `2026-08-31~09-06=48`, `2026-09-07=53`, `2026-09-08~09=56`, `2026-09-10 이후=59` profile 세대를 사용하며 `profiles_for_target_date()`와 applied-policy inventory가 권한을 소유한다. 과거 시세를 재조회하지 않으며 `2026-06-05` 이후 생성된 actual profile observation 전부를 단일 `clean_baseline_cumulative` 창에 넣는다. 전일 carry가 후일 자연청산되면 durable state를 원 거래일에 명시적으로 재귀속하며, 미관측 거래일은 coverage로만 공개하고 outcome으로 보간하지 않는다. 현행 producer는 `carry_actual_policy_subset_promotion_retired`로 실제 적용 정책을 carry하며, observed-signal subset은 진단용으로만 사용한다. 예전 completed-leg/양수 EV 조건만으로 새 tightening을 자동 승인하지 않는다. 새 추천은 expanded research의 기존 두 필터 경제성 연구와 정식 승인 owner가 소유하며, 9/8 별도 승인 변경은 원본 evidence와 effective-date 정책으로만 적용한다. schema/hash/source 결손은 현행 fail-closed 계약을 유지한다. 신규 진입 수량은 profile별 10주×2 leg(20주), 50:50 진입 offset, profile별 고정 target/validity, SOR, 무손절·미청산 보유와 broker guard로 고정된다.

9/9 22:50 별도 사용자 승인 후속은 [승인 복구 리뷰](audit-reports/2026-09-09-postclose-authorized-recovery-review.md)를 따른다. source9/9 추천 중 한국전력 오전후반 변경·롯데케미칼 오전/오후와 TYM 오전후반 신규3개를9/10부터 지원하고 SK텔레콤 오전은 동일 수치 재확인이다. 신규 preflight/live timer는 각각09:15/09:19,14:15/14:19,09:55/09:59이며 기존 template·exact-date evidence/정책·독립 owner·broker guard를 사용한다. 내일 예약 설치와 당일 PREOPEN/PID/자연 체결은 별도 receipt다. 영원무역·두산 정오는 기존 half-EV gate 보류이며 기존 보유 목표/수량은 변경하지 않는다.
...
20:10 threshold-cycle wrapper에는 machine attribution·market-weakness-hysteresis·entry-timing·policy-approval 실행 경로와 override env가 없다. 활성화된 21:15 `korstockscan-machine-microstructure-final-refresh.timer`가 expansion→attribution→market-weakness-hysteresis→entry-timing→approval의 단일 scheduled owner다. 명시적 복구는 `deploy/run_machine_microstructure_final_refresh.sh [YYYY-MM-DD]`만 사용한다. wrapper는 최신 완료 machine 거래일을 정확히 한 번 해석하며, 명시 날짜가 있으면 그 날짜와 일치할 때만 실행해 stale 거래일의 report·approval 재생성을 차단한다. 21:15 owner는 target-date 위젯 calibration/symbol-research/collector-expansion과 lower-price active/expanded profile inventory를 읽어 `machine_microstructure_attribution` JSON/Markdown을 한 번 생성한다. 종목 확장은 코드 상수에 고정하지 않고 당일 owner/research report에서 자동 발견한다. 보고서의 `decision_ownership`은 `micro_entry_confirmation`을 `machine_entry_timing_tuning`의 upstream source로만 선언하고, entry timing 정책의 유일한 public owner는 `machine_entry_timing_tuning`으로 고정한다. 60/120/180초 lifecycle turnover 연구와 market-weakness 정책은 별도 owner/section으로 분리하며 entry timing 후보로 혼합하지 않는다. 새 종목의 micro 0B/0D partition·symbol·anchor window가 비어 있으면 scope별 producer/consumer gap으로 기록하며 수익률 0으로 보간하지 않는다. 이 결손은 supplemental micro context만 사용 불가로 만들고 기존 위젯·episode 튜닝과 policy candidate는 계속된다. 일반 micro 산출물은 diagnostic-only다. 다만 사용자 승인된 약세 hysteresis 하위 체인은 같은 attribution 안의 차단/실행 신호 executable-BBO 반사실을 clean-baseline부터 누적하고, 10거래일·50신호·시장별 10신호·latest 3거래일 OOS 및 EV/p10/오분류 guard를 모두 통과한 경우에만 현재 activation/release 값에서 한 축을 ±1 이동한 다음 거래일 exact-date 정책을 발행한다. 표본 또는 review 미달이면 현재 exact policy를 유지하고, exact policy/source hash 결손이면 2/3 baseline으로 fail closed한다. 같은 날 hot mutation과 breadth/spacing·main bot·가격·수량·target·holding/exit 변경은 없다.

같은 producer는 repairable micro gap을 다음 KRX 거래일 exact-date `scalp_micro_reversion_collection_targets`로 되먹임한다. schema v3는 현재 위젯·episode active owner symbol 전부와 각 owner가 선언한 KRX/plain·NXT/`_NX`·SOR/`_AL` exact route를 보존하고, prospective research만 active 선택 뒤 남는 기본 일 4종목 budget에서 한 route씩 회전한다. main bot은 observer가 활성화된 경우에만 이 artifact를 boot에서 읽고 `0B/0D`만 등록한다. 이미 trading runtime이 소유한 exact item은 `protected_runtime_items`로 유지하고 다시 source-only 등록하지 않으며, 같은 code의 다른 exact route만 격리 buffer에 수집해 common trading tick으로 전파하지 않는다. 수동관리 제외종목은 수집·평가에서 제외하지 않는다. target 교체·runtime demotion·reconnect에서도 exact trading item과 observer item을 분리하며, atomic WS snapshot은 machine confirmation용 route별 0B/0D receipt와 bounded trade/depth window만 추가 노출한다. 다음 exact-date set은 이전 source-only set을 교체하므로 일별 구독이 누적되지 않는다. WS 전체 item budget과 collector storage self-disable/projection guard는 그대로 적용하며 budget overflow는 매매 구독을 밀어내지 않고 명시적 registration gap으로 남긴다. path projection 상한은 partition hard-total과 같은 4GiB이고 그보다 크게 구성할 수 없으며, shard당 512MiB·최대 8 shard·5GiB low-disk·1GiB critical-disk guard는 유지한다. 상한 변경은 디스크 확장 확인만으로 현재 PID에 자동 반영되지 않으며 코드 리뷰·owner별 broker 대사·우아한 재기동·새 PID writer 생존과 counter 전진 receipt를 요구한다.

`machine_entry_timing_tuning_report_v3`는 고정-delay와 신호별 동적 0/1/3/5초 mode를 같은 entry-stage 단일-scope 후보로 비교한다. 동적 bounded canary는 5 observed dates·8 unique/8 completed·replay/paired 85%·right-censored 35% 이하·complete 5 source-day positive/improved 비용차감 EV·modeled 순이익·자본효율·0.005%p uplift·p10을 요구한다. 자연 신호 최신성은 report 당일 또는 직전 KRX 거래일까지 허용하지만 이틀 연속 무신호는 차단한다. 조건을 통과하고 same-stage mutation이 없을 때만 다음 거래일 `machine_entry_timing_policy_applied_v3` 한 scope를 원자 생성한다. policy 발행 허용창은 source date 장후부터 effective date 08:00 KST 직전까지다. 그 이후 과거 report를 재생성할 수는 있지만 runtime policy는 쓰지 않고 report에 `blocked_effective_date_preopen_cutoff_elapsed`를 남겨 장중·사후 덮어쓰기를 차단한다. 다음 기동 owner는 exact-route shared WS snapshot을 읽어 기존 신호의 시점만 고른다. snapshot 파일/상위 계약이 없거나 깨졌으면 blind 5초 wait 없이 즉시 기존 owner guard 전체 재검증으로 복귀하고, snapshot 계약은 정상이지만 exact-route 행이 일시 결손이면 다음 checkpoint까지 bounded 재확인한 뒤 terminal에서 같은 fallback을 수행한다. signal·가격·수량·target·holding/exit·provider/bot/cap·broker/hard-safety는 바꾸지 않는다. 조건 미달·정책/hash 결손·scope 미선택은 즉시진입 baseline이며 코드 배포나 report 생성만으로 현재 process에 적용됐다고 보고하지 않는다.
...
이 service의 bounded expansion source wait는 900초이고 전체 `TimeoutStartSec`는 3600초다. Attribution은 multi-gigabyte 0B/0D partition을 streaming으로 읽되 겹치는 owner window의 source object를 공유하며, reviewed unit은 정상 단일 실행 working set을 수용하도록 `MemoryMax=2G`로 제한한다. 이 상한 변경은 provider·매매·정책 권한을 만들지 않으며 host available memory와 단일 PID/lock을 확인한 뒤 설치한다. Immutable outcome label이 끝내 없으면 expansion producer가 source-not-ready 전용 terminal exit `42`를 반환하고, 나머지 fallback 단계까지 성공한 경우 wrapper가 그 코드를 보존하며 unit의 `RestartPreventExitStatus=42`가 같은 대용량 attribution 체인을 반복하지 않게 한다. 다른 expansion 오류와 attribution·policy·builder 실패는 원래 exit code, 기존 실패 우선순위와 `Restart=on-failure`를 유지한다. repository unit 수정은 현재 실행 중이거나 설치된 unit에 자동 반영되지 않는다.

Repository unit 변경만으로 설치된 systemd unit이 자동 교체되지는 않는다. 운영 반영은 `sudo deploy/install_machine_microstructure_final_refresh_systemd.sh`로 기존 timer stamp를 승계하고 기존 `korstockscan-widget-expansion-recommendation.timer`를 중지·제거한 뒤 새 service/timer를 활성화하고, `systemctl cat korstockscan-machine-microstructure-final-refresh.service`가 단일 final-refresh wrapper를 가리키며 `korstockscan-machine-microstructure-final-refresh.timer`가 `enabled + active`인지 확인해야 한다. 두 timer를 동시에 enable하면 동일 산출물을 중복 생성하므로 금지한다. unit 반영은 bot 재기동이나 매매권한 변경이 아니지만 별도 운영 변경이므로 명시적 사용자 지시와 code-review gate 이후에만 실행한다.
...
| `21:55~23:50` | cron finalization gate | `deploy/run_postclose_finalization.sh YYYY-MM-DD`가 exact-date main postclose artifact/log, controller artifact/wrapper(필수 replay follower 포함), tuning monitoring artifact/log, dashboard archive log의 terminal을 bounded wait한 뒤 `deploy/run_logs_rotation_cleanup_cron.sh 30`을 실행 | `logs/postclose_finalization_cron.log`, `logs/log_rotation_cleanup_cron.log`, `tmp/log_rotation_cleanup_writer_defer_state.json` | predecessor 전부가 성공 terminal일 때만 cleanup을 연다. 기본 wait 5100초, 23:20 KST predecessor hard deadline, cleanup/detector 각 600초 상한으로 자정 전 최소 20분 고정 여유를 두고 `archive_generation_compressed/verified`, writer-defer, report/immutable artifact census를 확인한다. 실패/timeout/deadline이면 cleanup을 건너뛰고 bounded final detector만 실행해 원본을 보존한다 | 당일 장애 분석 전 로그 수동 삭제 금지. 숫자 slot은 archive identity가 아니며 원문 SHA-256 기반 generation gzip을 no-clobber 원자 게시·복원검증한다. plain source와 legacy gzip은 덮어쓰거나 삭제하지 않는다. writer/open/recent/압축 중 변경 slot은 `deferred_writer_active`로 분리하며 stable pass가 누적 defer를 초기화한다. 미검증 raw, 당일 source, incomplete checkpoint는 보존하고 generic cleanup에는 runtime/order/provider/threshold/bot 권한이 없다 |
...
| `07:00~21:50/5` + terminal-aware final | cron | `bash deploy/run_error_detection.sh full`; 21:55 trigger는 `run_postclose_finalization.sh`가 predecessor terminal과 cleanup을 닫고 finalization `[DONE] ... detector_handoff=started`를 동기 기록한 직후 같은 detector를 실행 | `data/report/error_detection/error_detection_YYYY-MM-DD.json`, `logs/run_error_detection.log`, `logs/postclose_finalization_cron.log` | 선행 `[DONE]`은 detector의 cron-completion self-audit 순환을 제거한다. detector 실행 실패 시 더 최신 finalization `[FAIL]`이 권위를 가지며, 성공 시 `[DONE] postclose_final_detector`가 뒤따른다. final run은 고정 21:55 진행 중 snapshot이 아니라 실제 장후 terminal 뒤 실행하고 7개 detector의 고유 `run_id`, accounting, `summary_severity`를 검증한다. predecessor failure/timeout은 cleanup 없이 detector를 실행하고 finalization `[FAIL]`을 남긴다 | 탐지 결과로 runtime threshold/spread/주문 자동 변경 금지. Telegram 알림은 report-only 운영 알림이며 자동 복구/재시작 권한이 아니다 |
...
7. 장후 raw-derived producer parity는 `counts_only_v1`로 생성해 bucket identity/count/coverage만 보존한다. per-second count, sample event, numeric stats, raw offset은 parity 판단에 불필요한 중복이므로 저장하지 않는다. raw와 producer의 coverage end가 다르면 더 이른 coverage end의 완료 분 경계까지만 `completed_common_minute` 비교를 추가하고, 미완료 tail은 별도 제외 건수로 남긴다. common-watermark parity가 통과해도 tail pending 동안 full parity와 suppress eligibility는 통과시키지 않는다. `pipeline_event_verbosity` JSON/Markdown이 raw source, producer summary/manifest, report/summary 코드보다 모두 새롭고 JSON 검증도 통과할 때만 복구 실행에서 재사용하며, 입력·코드가 하나라도 바뀌면 다시 생성한다. 폐기된 Entry ADM, lifecycle bucket, latency recommendation producer를 복구 대상으로 취급하지 않는다. 이 최적화는 `runtime_effect=false`이고 raw suppression을 활성화하지 않는다.
...
표준 marker 계약은 `monitor_snapshot`, `system_metric_sampler`, `swing_live_dry_run`, `swing_model_retrain_postclose`, `tuning_monitoring_postclose`, `dashboard_db_archive`, `log_rotation_cleanup`, `postclose_finalization`을 포함한 반복 wrapper 전체에 적용한다. 새 cron/wrapper를 추가할 때는 registry id와 wrapper 출력 id가 일치하는지도 같은 변경 세트에서 확인한다.
...
- 2026-09-04 postclose predecessor 보완 이후 final `observation_source_quality_audit`이 완료된 다음 `entry_split_order_plan`을 실행한다. Entry split 누적 상태는 source-quality 파일 mtime이 아니라 소비 필드의 정규화 hash로 검증하며, 누락 날짜는 최신 완전 상태 뒤부터 일괄 replay하고 구 schema 상태는 v2로 한 번 날짜별 streaming clean rebuild한다. 당일 입력은 중복 scan하지 않는다. 1주 주문은 분할 가능 표본에 포함하지 않고 정책 부모 variant EV와 runtime 자식 variant 진단을 분리한다. Machine microstructure는 당일 exact-route 요청/item census가 manifest와 일치하고 active-owner route의 직접 0B/0D registration receipt가 완결되어야 rolling 후보를 열며, prospective 미수신만으로 active-owner 후보를 막지 않는다. 당일 active receipt 결손은 immutable quarantine이므로 보고서 반복 실행으로 복구하려 하지 않는다. Signal bar/prospective anchor는 timing 튜닝 분모에서 제외한다. Main AI R0는 shock가 없는 `not_applicable|source_unavailable` row의 ask-depletion sidecar 생성을 생략하며 pre-current-design 불변 결손을 terminal exclusion으로 닫는다. 모두 source-only 계약이며 실주문·threshold·provider·bot·수량·가격·safety 권한은 바꾸지 않는다.
...
이 스크립트는 `restart.flag`를 원자적으로 생성하고 `bot_main.py`의 기존 감지 루프가 자체 종료하도록 둔다. 이후 기존 PID 종료와 `src/run_bot.sh` supervisor가 올린 신규 PID를 제한시간 안에서 기다리고, 신규 PID에 대해 당일 `runtime_policy_bootstrap_YYYY-MM-DD.env` handoff 검증 artifact까지 갱신한다. 실행 중인 PID의 `KORSTOCKSCAN_RUNTIME_LAUNCHER_RUN_BOT_SHA256`가 현재 `src/run_bot.sh`와 다르면 기존 child가 완전히 종료된 뒤에만 tmux `bot` supervisor를 교체한다. 이 경로는 장수 supervisor가 이전 launcher 함수나 env 정규화를 재사용하는 것을 막으며, 기존 child가 살아 있는 동안 두 번째 봇을 시작하지 않는다. 소스 변경을 반영하는 재기동은 review gate와 targeted validation을 닫고 로컬 커밋을 만든 뒤 수행해 신규 프로세스의 `KORSTOCKSCAN_RUNTIME_GIT_COMMIT`과 launcher commit/hash가 실제 로드 소스와 일치하고 `KORSTOCKSCAN_RUNTIME_SOURCE_DIRTY=false`가 되게 한다. PID handoff verify가 실패하면 `restart.sh`도 실패로 종료하며 완료로 간주하지 않는다. `pkill -9`, 직접 `nohup python src/engine/kiwoom_sniper_v2.py`, 텔레그램 매니저 별도 중복 기동, provider/threshold/order env hot mutation은 금지한다. 재시작 후에는 새 `bot_main.py` PID, `logs/bot_history.log`, heartbeat, Kiwoom WS/REST 회복, `/proc/<pid>/environ`의 commit/source-dirty와 당일 runtime env 반영 여부, verify artifact의 `status=pass`를 확인한다.

당일 삼성 오전 독립 service가 active이면 `restart.sh`는 restart flag보다 먼저 현재 authority hash·기존 main PID·service PID·custody/order snapshot에 결속된 짧은 TTL의 same-date handoff plan을 원자 생성한다. 기존 main PID가 종료된 뒤 신규 PID의 exact-date runtime verify가 pass하고 handoff 중 신규 BUY order number가 없을 때만 authority의 `decision.main_bot_pid`를 교체하며, 최초 PREOPEN PID부터 이어지는 연속 handoff history를 staging에서 완전 검증한 후 한 번에 publish한다. 이 PID 교체는 기존 `observed_at_kst`, 만료, policy, 수량, custody와 주문을 바꾸지 않고 신규 authority를 만들거나 09:25 publication deadline을 우회하지 않는다. 준비되지 않은 `restart.flag`, 23:50 scheduled restart, 손상·만료 plan, service/PID/hash 불일치와 commit 실패는 재기동 또는 신규 BUY를 fail-closed하며 blocked receipt를 남긴다. stop 이전 실패·중단은 old PID가 살아 있을 때 plan을 자동 abort하고, old PID 종료 뒤 실패는 plan을 보존해 신규 BUY를 차단한다. Samsung gateway는 매 BUY 직전에 same-date authority와 handoff chain을 다시 검증하되 기존 보유의 target SELL·cancel·reconciliation은 차단하지 않는다. 이미 plan 없이 main PID가 교체된 사고의 보완은 old PID 사망, current PID strict runtime verify, 동일 service PID, 두 10주 leg의 pre-restart fill·active target과 reentry activity 0을 모두 입증한 명시적 custody-only recovery에서만 허용한다.

동일 종목 multi-owner는 exact-date `symbol_owner_policy_v2`와 원장의 `owner_custody_policy_activation_v1`이 exact match일 때만 활성화한다. PREOPEN operator request는 `symbol_owner_policy_apply_request_v1` schema로 `active_date`, 실행시각보다 미래가 아닌 deterministic `policy_id/generated_at_kst`, 비-default `broker_account_key`, 종목별 `mode/allowed_owners/expected_broker_quantity/expected_external_manual_remainder/migrated_positions`를 명시한다. 각 migration row는 허용 owner 집합에 포함된 `owner_type`과 exact `owner_id/position_id/client_intent_id/order_date/broker_order_no/quantity/average_price/route/evidence_sha256`를 가져야 한다. 주문번호 identity는 acquisition date와 7자리 주문번호의 쌍이며, 서로 다른 날짜에 재사용된 같은 번호를 중복으로 합치지 않는다. 먼저 아래 dry-run을 실행한다.

```bash
PYTHONPATH=. .venv/bin/python -m src.trading.order.symbol_owner_policy_apply \
  --request data/runtime/symbol_owner_policy/requests/symbol_owner_policy_apply_YYYY-MM-DD.json
```

Dry-run `status=dry_run_ready`, 전 owner process 정지, broker holding/open-order 두 snapshot 동일, KRX·NXT coverage, projected owner quantity conservation과 exact open-order registry match를 확인한다. 기존 custody migration이 있으면 각 row의 acquisition `order_date`와 exact broker BUY order number를 request에 넣고, producer가 official `kt00007` completed BUY의 order quantity·filled quantity·execution price·zero remainder·canonical evidence hash까지 같은 두 snapshot에서 독립 검증한 뒤에만 사용자 승인 범위 안에서 아래 apply를 실행한다. 실제 `--apply`는 systemd order owner와 같은 `ubuntu` effective user 및 exact active date의 `08:35~08:50 KST`에서만 허용하며, confirmation이 맞아도 어느 조건이 다르면 차단한다.

```bash
PYTHONPATH=. .venv/bin/python -m src.trading.order.symbol_owner_policy_apply \
  --request data/runtime/symbol_owner_policy/requests/symbol_owner_policy_apply_YYYY-MM-DD.json \
  --apply --confirm "APPLY SAME SYMBOL OWNER POLICY YYYY-MM-DD"
```

반복 장전 적용은 명시적으로 검토된 `data/config/symbol_owner_policy_standing_authority.json`의 exact symbol/owner/account/hash와 유효기간 안에서만 `07:32` timer가 수행한다. `deploy/run_symbol_owner_policy_auto_apply.sh`는 `07:30` main 종료 뒤 active widget/episode order service를 일시 정지하고, shared token을 당일 발급·재사용해 현재 machine universe 전체를 한 번 대사한다. broker 잔고가 registry owner 수량보다 크거나 작거나, current open order가 registry와 exact match하지 않는 종목은 migration·owner 추정 없이 daily request에서 제외해 legacy manual exclusion을 유지한다. 나머지 안전한 종목만 기존 apply producer의 동일 snapshot·activation·atomic publish 계약을 거치며, 완료 generation 재시도는 request/policy/activation/hash를 전수 재검증한 뒤 broker 재조회 없이 idempotent 종료한다. machine universe 변경, standing artifact 변조·만료, 거래일/시간창 불일치, main 또는 미정지 order process는 전체 자동 apply를 fail-closed한다. `Persistent=false`이므로 host가 예정 trigger를 놓치면 장중 catch-up으로 order service를 정지하지 않고 해당 날짜 공존 entry를 열지 않는다.

Apply는 필요한 기존 custody migration을 먼저 append하고, migration receipt의 exact current tail에서 policy activation event를 원자적으로 결속한 뒤 `owner_custody.env`, apply receipt를 쓰고 policy 파일을 마지막 commit point로 게시한다. Migration append 뒤 activation 전에 중단된 재시도는 exact receipt·owner identity가 같은 기존 row를 읽기 전용으로 인식해 수량을 두 번 더하지 않으며, 충돌 row는 복구하지 않고 차단한다. Main launcher와 모든 widget/episode service·preflight는 `owner_custody.env`의 같은 broker account/registry/policy-file identity를 읽어야 한다. 설치 unit과 repository unit의 `EnvironmentFile` 일치, 새 process env, policy ID/hash, activation event hash, broker snapshot hash, registry hash-chain을 확인하기 전에는 주문 가능 process를 기동하지 않는다. Registry의 `INTENT_RESERVED|INTENT_AMBIGUOUS`는 재기동으로 삭제하거나 새 owner에 재귀속하지 않으며, 동일 symbol/side 신규 제출을 계속 차단한 채 exact broker order receipt로만 결속한다. Aggregate holding은 conservation 확인에만 쓰고 main/widget/episode 수량·평단·청산 owner를 덮어쓰지 않는다. Policy 파일 부재·손상·date mismatch 또는 migration/activation 결손이면 공존 entry와 등록 symbol의 aggregate recovery를 fail closed한다. `COEXIST_EXIT_ONLY` rollback에서는 신규 BUY·추가매수를 막되 각 owner의 기존 보유·미체결 BUY 취소·target/청산 SELL·SELL 취소·영수증 reconciliation process는 유지한다. 정책·activation을 새로 게시하거나 `COEXIST_ENTRY_ENABLED`로 전환하는 것은 별도 PREOPEN apply/user authority이며 커밋·푸시·재기동 자체에 포함되지 않는다.
...
같은 producer의 다섯 threshold group은 고정 taxonomy라서 매일 모두 보이는 것 자체가 신규 기회 5건을 뜻하지 않는다. report는 겹칠 수 있는 `threshold_group_evaluations`, forced event 이전 또는 같은 시각·순서의 단일 first explicit blocker 전수 `primary_blocker_evaluations`, 그중 EV·floor를 통과한 source-only existing-family `threshold_opportunities`를 분리해야 한다. 양(+) EV는 코드 결함이나 `implement_now`가 아니며 `route=existing_family`로만 intake한다. 기존 family에 결속되지 않는 계측·join 결손이 별도 root-cause order로 입증된 경우에만 구현 대상으로 올린다. 누적 pipeline은 변경되지 않은 partition index를 재사용하고 cache hit/miss·scanned/reused bytes·estimated I/O·elapsed를 공개한다. cache source path·device/inode·size·mtime/ctime 또는 taxonomy contract digest가 다르면 해당 partition만 forced-ID discovery 1회와 그 ID의 targeted scan 최대 1회로 다시 읽으며, cache에 전체 record taxonomy를 적재하지 않는다. scan·cache publish 중 source가 변하면 fail-closed한다. post-sell coverage는 entry-date partition이 아니라 terminal `sell_completed record_id`와 post-sell `recommendation_id`의 exact join 및 양쪽 stock-code 존재·일치를 요구한다. 같은 record ID의 복수 forced primary event, propagated forced event stock 충돌, 서로 상충하는 post-sell outcome, stock 누락·불일치 또는 관련 malformed JSON은 last-row-wins·valid-empty로 소비하지 않고 source-identity/coverage gap으로 격리한다. submit 후 terminal sell이 없는 row는 pending/right-censored이고 source gap이 아니다. 현재 코드로 재계산한 actionable semantic digest, AI prompt/schema/provider-config contract digest와 후보별 parsed review census가 모두 같을 때만 기존 AI receipt를 재사용하고, 후보가 없으면 AI 호출을 생략한다.

### Greenfield Real-Env Containment

Greenfield의 LDM bucket authority는 retired/no-op다. 예전 containment env 또는 `restart.flag` 절차는 현행 실행 지시가 아니다. archived policy/lock을 재활성화하지 않고, 자연 PREOPEN/PID/postclose에서 retired OFF와 dedicated owner 보존을 확인한다. 이 검증은 `AdmLdmRetirementNaturalEvidence0907`이 소유하며 봇 재기동 권한을 만들지 않는다.
...
3. family별 dated policy receipt에서 selected/retained/disabled/rejected 상태와 source/content hash를 본다.
4. `data/runtime/policy_bootstrap/runtime_policy_bootstrap_YYYY-MM-DD.json`과 verify JSON에서 incumbent·operator lock·explicit OFF, env owner와 원천 hash를 확인한다. bootstrap이 없거나 verify가 실패하면 `fail`로 분류한다.
...
필수 필드는 `lock_id`, `family`, `stage`, `env_key`, `env_value`, `active_from_date`, `min_observation_until_date`, `allowed_close_reason_keywords`, `decision_authority`, `source_evidence`다. `runtime_policy_bootstrap`은 유효기간 안의 active lock만 읽고 family 신규 후보가 없거나 차단된 경우에도 검증된 incumbent와 lock env를 유지한다. 충돌·만료·malformed lock은 거절 사유를 manifest에 남긴다.
...
ls -l data/runtime/policy_bootstrap/runtime_policy_bootstrap_$(TZ=Asia/Seoul date +%F).json
ls -l data/runtime/policy_bootstrap/runtime_policy_bootstrap_verify_$(TZ=Asia/Seoul date +%F).json
grep -n "SWING_LIVE_ORDER_DRY_RUN_ENABLED" data/runtime/policy_bootstrap/runtime_policy_bootstrap_$(TZ=Asia/Seoul date +%F).env || true
tmux ls
```
...
2. swing family의 dated policy receipt에서 `approved`, `blocked`, `selected`, `decisions`를 확인한다.
3. `data/runtime/policy_bootstrap/runtime_policy_bootstrap_YYYY-MM-DD.json`의 family receipt와 `env_overrides`에서 승인된 incumbent/lock만 유지됐는지 확인한다.
...
명시적으로 구현/장후 모니터링을 지시받았을 때만 2-pass를 실행한다. intake 전수보존·stable native ID·권한분류·재생성·fixed-point의 상세 계약은 [장후 모니터링 §7](./postclose-tuning-result-review-task-instructions.md#7-implement-now-및-위젯에피소드-추천-2-pass-구현)이 소유한다. 두 번이라는 횟수로 중단하지 않고 eligible new/changed 및 미분류·actionable open이 0일 때 닫는다. 각 코드 수정 후 review gate를 통과하기 전에는 재생성하지 않는다. 내부 단계는 아래 네 단계다.
...
```bash
bash -n deploy/run_threshold_cycle_preopen.sh deploy/run_threshold_cycle_postclose.sh
...
- 초기 보호 mode는 `KORSTOCKSCAN_AI_INPUT_PREFLIGHT_MODE=baseline_v1`이다. clean baseline 이후 real 이벤트와 당일 source-quality audit로 `data/report/ai_input_quality_baseline/ai_input_quality_baseline_YYYY-MM-DD.json`을 생성한다. legacy field는 source-quality proxy일 뿐 exact venue provenance가 아니며, 정책은 provider 호출·scale-in support 권한을 줄일 수만 있다. legacy overnight AI 판단은 2026-09-06 폐기 상태를 유지한다. 2026-09-14 이후 19:45 terminal 평가에서 유효한 비용 차감 수익률이 `<= 0`이면 terminal SELL 신호 없이 기존 holding 로직을 계속하는 것은 AI overnight 승격이 아니라 별도 deterministic runtime 계약이다.
...
- holding-flow는 주기 계좌 동기화의 `kt00005 + ka10075` broker position/open-order snapshot과 실제 entry broker route를 대사한다. 결손·60초 초과·venue mismatch에서는 AI가 scale-in을 지지할 수 없고 terminal 판정을 유효한 SELL/HOLD로 합성할 수 없다. same-session terminal SELL 역시 이 계좌·주문·receipt guard를 우회하지 않는다.
- 검증 명령:
...
- 다중분봉 문맥의 runtime payload 소유자는 entry-side `entry_candle_context_v1`과 holding/scale-in/exit-side `holding_decision_context_v1`이다. 두 schema는 같은 venue/session-normalized candle source와 `input_bundle_version=scalping_multi_timeframe_context_v1` 파생 feature를 공유한다. `scalping_multi_timeframe_context_v1`을 세 번째 병렬 model payload로 만들거나 entry context를 holding prompt에 재사용하지 않는다. `decision_kind=overnight`는 환경값과 무관하게 비활성이다.
...
- `decision_quality_v1` primary cohort는 `replay_exact=true`, `request_capture_status=captured`, `preflight_mode=exact_v2`, 동일 canonical context schema, 동일 `input_bundle_version`, 동일 eligible trace ID/payload hash, mature horizon, fresh conflict-free source를 요구한다. Primary baseline horizon은 entry/entry-price/post-probe `10m`, scale-in `20m`, holding/exit `30m`로 고정해 서로 다른 성숙 구간을 같은 stage bucket에 섞지 않는다. Historical overnight `60m` trace는 archive/audit evidence이며 current primary cohort에서 제외한다. Entry 후보는 `entry_candle_context_v1`, holding/exit 후보는 `holding_decision_context_v1` 내부에서 각각 control과 paired replay하며 서로 합치지 않는다. Control과 candidate는 같은 provider/model/temperature/reasoning budget을 사용한다. AI 호출·parse 성공률은 운영 지표이며, 승격 판단은 stage·venue·session별 `source_quality_adjusted_ev_pct`, missed-upside, adverse-first BUY, loss tail, exit giveback을 사용한다. Net profit과 scale-in add/no-add incremental EV는 notional·fill counterfactual join이 완료되기 전에는 `not_available`로 유지한다.
...
- Main 전략 임계치는 기존 `ai_action_outcome_calibration --machine-only --write --activate-now`가 원시 입력 재판정·유형/상태 selector·비용 후 동일 기회 비교를 함께 평가한다. 적격 후보만 기존 mechanistic publisher가 immutable generation으로 만들고 `data/runtime/mechanistic_entry_policy/current.json`을 parent CAS 후 원자 교체한다. 이 generation은 즉시 새 attempt부터 적용되며 다음 적격 generation까지 날짜·PREOPEN·재시작에 관계없이 승계한다. 불합격/결손 후보는 유효 incumbent를 유지하고 활성 current 자체의 손상은 진입 차단이다. current 선택과 장후 dated handoff는 별도 증거이며 파일 생성만으로 PID 소비나 경제성 수용을 표시하지 않는다. 원천 미지원 전략 좌표와 후보 AI/실행 경로 미지원은 보고서에 남기고 PASS·비용0·회복 이익으로 채우지 않는다. source/order/custody/보유 추가매수 및 별도 operator guard는 이 초기진입 정책의 튜닝 좌표가 아니다.
- 표준 장후 `src.engine.scalping.ai_decision_quality --date YYYY-MM-DD --mode postclose --write`는 v2 source-label 계약의 control·원 라벨 artifact를 생산한다. 후보 선택 권한은 없다. 최종 source-quality 뒤 main wrapper는 `entry_setup_paired_replay_batch --compact-only --execute-compact-candidate --finalize-compact`를 한 번 호출한다. `compact_auxiliary_paired_replay`가 fingerprint가 바뀐 eligible generation만 평가하고 canonical paired artifact를 기존 mechanistic publisher, consumer, checklist와 직접 결속한다. source/model gap은 provider 0과 유효 incumbent carry로 종결하며 EV 성공으로 표시하지 않는다. `ai_action_outcome_calibration --postclose-phase`와 복제 `compact_summary_handoff` 계약은 퇴역했다. runtime summary와 verifier는 paired·dated policy·PREOPEN/PID receipt를 직접 대조한다. 주문·guard·수량/budget/custody·operator override·provider/bot 권한은 보존한다.

- `scalping.micro_reversion.ai_quality_cycle`의 실행·Provider replay·R2/R3 인계·전용 legacy/current-axis runtime는 완전 제거한다. 과거 파생 연구 산출물은 제거하되 원본 AI·시장·주문·체결 및 공유 비용·공식 master·현행 compact 정책/rollback 증거는 보존한다. 공용 source primitive의 존재는 폐기 실행기의 복원 권한이 아니다.
...
- Compact auxiliary의 활성 실행 owner는 main postclose wrapper 한 곳이다. 고정 21:05 전용 schedule과 DONE-controller follower는 제거됐다. `deploy/run_ai_entry_setup_paired_replay_postclose.sh`는 수동 호환 wrapper로만 남고 main과 같은 단일 compact evaluator/finalizer를 호출한다. finalize는 provider를 호출하거나 EV를 다시 계산하지 않으며 paired→dated policy→consumer→checklist hash가 모두 맞을 때만 성공한다. 장중 고정 원천의 유효 후보가 있으면 `PYTHONPATH=. .venv/bin/python -m src.engine.scalping.mechanistic_entry_runtime_policy --activate-auxiliary-now --source-date YYYY-MM-DD`가 잠금·현재 machine/AI 부모·stage 재생을 확인해 AI component만 적용한다. 후보가 없으면 기존 정책을 유지한다. 기존 full-day Entry/Holding 연구 코드는 명시적 수동 분석용 잔여 코드이며 설치 schedule 또는 현재 compact 완료 조건이 아니다. 다음 07:35 PREOPEN은 검증된 dated bundle만 읽고, actual PID 소비와 비용 후 자연 성과는 별도로 확인한다.
- V2.14의 누적 승격 표본은 candidate exposure 10건/3종목이다. 준비 요청 30건/10종목 floor와 혼합하지 않는다. 성과 승격 판단은 비용 차감 후 exposure EV `>=+0.10%`, 양의 paired EV delta, net missed-upside **value**, 완전한 execution-cost provenance, bounded probe risk budget을 함께 사용한다. missed-upside 건수는 진단값으로만 남겨 작은 기회 여러 건의 포기 때문에 더 큰 순기회 회복이 기계적으로 차단되지 않게 한다. KRX gate가 통과하면 batch가 다음 KRX 거래일 candidate를 자동 생성하고, 07:35 PREOPEN이 원본 batch/detailed hash와 authority 계약을 재검증해 date-scoped activation을 기록한다. 이때 bot launcher와 같은 `threshold runtime env -> persistent operator override -> dated operator override` 순서로 파일을 비실행 파싱하고, 당일 runtime apply date와 configured V2.13/probe 계약을 검증한다. 같은 PID의 실시간 선택도 당일 runtime handoff, configured V2.13 owner, KRX 정규장 provenance, activation/candidate hash, WAIT-probe handoff, probe quantity 1, probe-first `DAILY|target_date`, post-probe resolver를 다시 확인한다. 하나라도 실패하거나 전일 PID가 당일 env 없이 남아 있으면 V2.13으로 자동 복귀한다. NXT는 자체 승격 gate 전까지 V2.13 control이다.
