...
| `21:00` | cron | `deploy/run_logs_rotation_cleanup_cron.sh 30` | `logs/log_rotation_cleanup_cron.log` | 기존 log/tmp/cache 지표와 함께 `micro_reversion_storage_status`, `micro_reversion_storage_actions`, `micro_reversion_storage_compressed`, `micro_reversion_storage_purged`, `micro_reversion_storage_source_bytes`, `micro_reversion_storage_purge_enabled/status/candidates` 확인 | 당일 장애 분석 전 로그 수동 삭제 금지. 최신 target date의 micro 0B/0D/path/reference는 절대 변경하지 않는다. 자동 경로는 closed-date micro JSONL만 1일 후 SHA-256 gzip roundtrip 검증으로 압축한다. 14일 초과 partition도 기본 `MICRO_REVERSION_STORAGE_PURGE_ENABLED=false`에서 census만 남기고 삭제하지 않는다. 삭제는 별도 운영 승인 후 env=true와 CLI `--purge-expired`가 함께 있는 explicit opt-in에서만 가능하다. 원본 `data/pipeline_events`, clean-baseline quarantine/raw audit evidence는 cleanup 대상이 아니다 |
