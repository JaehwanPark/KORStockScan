...
| `22:30` | disabled | `eod_analyzer.py` / `KORSTOCKSCAN_EOD_TOP5_ENABLED=false` | `logs/eod_analyzer.log` if manually run | AI 호출 절감을 위해 내일의 주도주 TOP5 자동 실행은 OFF로 본다 | threshold daily EV를 대체하지 않는다. 재개하려면 cron과 env enable, detector registry를 같은 변경 세트로 복구한다 |
...
4. entry ADM은 matched bucket의 dominant action이 `WAIT_REQUOTE/SKIP_STALE`이면 `BUY -> WAIT`, `NO_BUY_AI/SKIP_SOURCE_QUALITY/SKIP_PRE_SUBMIT_SAFETY`이면 `BUY -> DROP`, `BUY_NOW/BUY_DEFENSIVE` bucket의 `source_quality_adjusted_ev_pct < 0`이면 `BUY -> WAIT`로 보정한다. joined sample 부족 시에도 hypothesis fallback이 stale high, source-quality blocker, low liquidity, weak momentum + chase risk 조합을 WAIT/DROP으로 보정할 수 있다.
5. holding/exit ADM은 `prefer_exit`이면 `HOLD/TRIM -> EXIT`, `prefer_avg_down_wait` 또는 `prefer_pyramid_wait`이면 `EXIT/TRIM -> HOLD`로 보정할 수 있다. matrix가 비어 있으면 loss/AI 회복 구간은 `holding_exit_matrix_avg_down_bias`, profit/AI 강세 구간은 `holding_exit_matrix_pyramid_bias`로 scale-in evaluator에 전달한다.
...
런타임에서는 `KORSTOCKSCAN_SCALP_ENTRY_ADM_ADVISORY_ENABLED=true`일 때 entry prompt에 `[Entry ADM Advisory Context]`가 붙고, `KORSTOCKSCAN_SCALP_ENTRY_ADM_RUNTIME_BIAS_ENABLED=true`일 때 AI 결과를 직접 보정한다. 기존 응답 schema는 유지하되 merge 단계에서 action만 보정하므로, downstream은 일반 AI 결과처럼 처리한다.
...
4. joined sample이 아직 부족해도 `KORSTOCKSCAN_SCALP_ENTRY_ADM_HYPOTHESIS_FALLBACK_ENABLED=true`이면 stale high, source-quality blocker, low liquidity, weak momentum + chase risk 조합을 가설 기반 `WAIT` 또는 `DROP`으로 적용한다.
...
`KORSTOCKSCAN_HOLDING_EXIT_MATRIX_ADVISORY_ENABLED=true`이면 holding/exit prompt에 `[ADM Advisory Context]`가 붙는다. `KORSTOCKSCAN_HOLDING_EXIT_MATRIX_RUNTIME_BIAS_ENABLED=true`이면 flow AI 결과를 직접 보정한다.

Holding/Exit ADM 보정 규칙은 아래와 같다.

1. matrix bias가 `prefer_exit`이면 AI `HOLD` 또는 `TRIM`을 `EXIT`로 바꿀 수 있다.
2. matrix bias가 `prefer_avg_down_wait` 또는 `prefer_pyramid_wait`이면 AI `EXIT` 또는 `TRIM`을 `HOLD`로 바꿀 수 있다.
...
4. `KORSTOCKSCAN_HOLDING_EXIT_MATRIX_SCALE_IN_BIAS_ENABLED=true`이면 해당 bias가 `AVG_DOWN` 또는 `PYRAMID` action 후보를 만들 수 있다.
