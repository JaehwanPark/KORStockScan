...
4. 보관/압축은 `compress_db_backfilled_files`가 소유한다. EC2는 22:00 종료될 수 있으므로 정규 cron은 평일 21:10에 `run_dashboard_db_archive_cron.sh 0`로 같은 날짜 verified/backfilled raw/snapshot 압축을 시도한다. 검증되지 않은 raw는 skip되며, 운영자가 수동 정리할 때는 dry-run으로 verified/backfilled 대상과 `skipped_unverified`를 먼저 확인한다. 미검증 파일 강제 삭제는 금지한다.
...
`build_codex_daily_workorder --slot POSTCLOSE`는 이 절차를 `PostcloseAutomationHealthCheckYYYYMMDD`로 자동 포함한다. 이 항목은 날짜별 개별 구현 backlog가 아니라 `Runbook 운영 확인` 큐다. 15:45 postclose wrapper 이후 자동 감시 범위는 `postclose_done_controller` completion과 21:10/21:20 보관/로그 정리까지이며, `codex_workorder_runner`는 사용자 지시 또는 수동 opt-in 실행 결과가 있을 때만 별도 확인한다.
