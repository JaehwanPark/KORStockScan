...
동일 종목 multi-owner가 exact-date `symbol_owner_policy_v1`로 활성화된 날의 재기동은 policy ID/hash, broker account key, `owner_custody_migration_receipt_v1.registry_tail_hash`가 기존 process와 신규 process에서 동일하고, append-only `order_owner_registry_event_v1` hash chain이 완전한지 확인한다. Registry의 `INTENT_RESERVED|INTENT_AMBIGUOUS`는 재기동으로 삭제하거나 새 owner에 재귀속하지 않으며, 동일 symbol/side 신규 제출을 계속 차단한 채 exact broker order receipt로만 결속한다. `kt00005` aggregate holding은 conservation 확인에만 쓰고 main/widget/episode 수량·평단·청산 owner를 덮어쓰지 않는다. Policy 파일 부재·손상·date mismatch 또는 registry generation 결손이면 공존 entry와 등록 symbol의 aggregate recovery를 fail closed한다. `COEXIST_EXIT_ONLY` rollback에서는 신규 BUY·추가매수를 막되 각 owner의 기존 보유·미체결 BUY 취소·target/청산 SELL·SELL 취소·영수증 reconciliation process는 유지한다. 정책·migration receipt를 새로 게시하거나 `COEXIST_ENTRY_ENABLED`로 전환하는 것은 별도 PREOPEN apply/user authority이며 커밋·푸시·재기동 자체에 포함되지 않는다.
