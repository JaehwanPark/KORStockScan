...
4. 명시적 운영 override로 `KORSTOCKSCAN_HOLDING_EXIT_MATRIX_SCALE_IN_BIAS_ENABLED=true`가 켜진 경우에만 해당 bias가 `AVG_DOWN` 또는 `PYRAMID` action 후보를 만들 수 있다. lifecycle AI context v1에서는 scale-in은 관찰/provenance만 남기고 실제 scale-in action을 만들지 않는다.
