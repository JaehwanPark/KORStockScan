...
| `21:00` | cron | `deploy/run_logs_rotation_cleanup_cron.sh 30` | `logs/log_rotation_cleanup_cron.log` | `active_rotation_status/deferred`, `archive_compressed/verified_existing_source_preserved/failures/sources_preserved/source_unlink_deferred(_bytes)`, `active_retention_deferred(_bytes)`, `archive_retention_deferred(_bytes)`, `data_source_unlink_deferred(_bytes)`, `raw_row_exclusion_*_delete_deferred(_bytes)`, `find_enumeration_failures`, `micro_reversion_storage_status/partition_failures/failed_candidates/failed_candidate_bytes/recovery_required/actions/compressed/purged/purge_partial/source_bytes` 확인 | 당일 장애 분석 전 로그 수동 삭제 금지. generic unknown-writer active/numeric archive/raw-row에 대한 rename·truncate·shift·prune·source unlink·retention delete는 기본 deferred다. oversized active는 `disabled_pending_writer_owner`와 `[FAIL]`을 남기고, quiet/stable archive는 no-clobber gzip copy만 게시하며 plain을 authoritative로 보존한다. active/archive retention은 `disabled_pending_writer_owner`, raw duplicate/backup·sentinel/snapshot plain unlink은 `disabled_pending_storage_owner`의 candidate count/bytes만 남기고 이 deferred backlog 자체는 DONE-by-design이다. micro closed-date만 writer의 trade-date shared lock(open 전~fsync/close 후)과 maintenance exclusive non-blocking lock(session group preflight~manifest~unlink)이 맞물린 경우 verified gzip compaction을 적용한다. ownership/preflight의 overlap·invalid·symlink·open/changed group은 0 mutation으로 차단한다. publish 후 실패는 plain source를 authoritative로 보존하고 실제 partial action·unresolved candidate bytes·recovery count를 보고하며 다른 session/date/lane을 계속한다. 최신 KST/as-of date는 보호하고 14일 초과 purge는 기본 `MICRO_REVERSION_STORAGE_PURGE_ENABLED=false`에서 census만 남긴다. 명시 opt-in purge가 일부만 삭제된 뒤 실패하면 `purge_partial>0`, 남은 candidate bytes, `recovery_required>0`과 최종 `[FAIL]`로 남긴다. 압축·micro lock/producer·find producer 실패는 peer 진행 후 한 번의 `[FAIL]`, 0 `[DONE]`으로 집계한다. 원본 `data/pipeline_events`, clean-baseline quarantine/raw audit evidence는 cleanup 대상이 아니다 |
...
- Provider replay 기본 상한은 `THRESHOLD_CYCLE_MAIN_AI_QUALITY_PARENT_CAP=8`, `THRESHOLD_CYCLE_MAIN_AI_QUALITY_DAILY_ATTEMPT_CAP=96`, `THRESHOLD_CYCLE_MAIN_AI_QUALITY_DAILY_USD_CAP=1.0`이다. 근거는 `2026-08-10~2026-08-14` 메인 봇의 평가 완료 호출 `[676,369,781,1113,999]`, 중앙값 781회이며 A/B/C 논리 요청 최대 24회는 중앙값의 약 3.1%다. OpenAI/Bedrock 단가는 operator-reviewed accounting 기준으로 input/output 모두 USD 0이며, USD cap보다 96회 attempt cap이 실제 circuit breaker다. request bound는 A/B/C 한 parent를 원자 단위로 선택하며 한두 arm만 실행하지 않는다. 각 correction/schema attempt는 KST 일일 append-only ledger에 먼저 예약한다. OpenAI SDK retry와 budgeted Bedrock key rotation은 0이며, Bedrock 내부 key 재시도도 별도 예약 없이는 실행되지 않는다. timeout/transport/usage 결손은 환불하지 않고 attempt를 유지한다. cap exhaustion, partial result, provider provenance 실패는 execution complete가 아니며 wrapper에 warning을 남긴다. `THRESHOLD_CYCLE_MAIN_AI_QUALITY_EXECUTE_PROVIDER_REPLAY=false`로 Provider 호출만 중지해도 source bundle/materialization/lifecycle/R2-R3 관찰은 유지된다.
...
- R0→R3 producer 자체는 source-only에서 끝난다. `runtime_effect=false`, `allowed_runtime_apply=false`, `actual_order_submitted=false`, `broker_order_forbidden=true`이며 R3 산출물만으로 prompt/runtime/order를 바꾸지 않는다. 별도 등록된 `main_ai_quality_entry_prompt_contract_v1` family가 유일한 `entry/KRX/KRX_REGULAR` 후보의 exact current/recommended prompt SHA, standing-intent SHA, bounded-contract SHA, trusted registry-entry SHA와 latest verified symbol-master payload SHA를 다시 검증한 경우에만 operator decision receipt를 자동 생성한다. 다음 KRX PREOPEN에 generic approval ledger가 exact-date handoff를 만든 뒤 family consumer가 당일 activation과 apply receipt를 원자 파일로 기록한다. live selector는 activation뿐 아니라 apply receipt의 candidate/handoff/registry/activation hash와 직전 master hash가 일치하고 호출 종목이 effective-date KOSPI/KOSDAQ 보통주 record에 있을 때만 `hot_v1 -> decision_quality_v2_6`을 선택하며 결손·tamper·same-stage conflict·master scope 이탈은 configured prompt로 복귀한다. 이 family는 provider/model, order price/quantity, threshold/cap, bot, broker/account/order/cooldown, hard/protect/emergency safety를 바꾸거나 주문을 제출하지 않는다.
- 최초 exact 후보는 `2026-08-18`부터 clean eligible 거래일 20일이 모두 쌓여야 하므로 결손이 없을 때 가장 이른 생성 시점은 `2026-09-14` postclose이고 첫 적용은 `2026-09-15` PREOPEN이다. one-shot standing intent는 exact 두 prompt SHA와 registry/consumer/evidence contract에 고정되고 `2026-10-16T09:30+09:00`에 만료된다. 이 만료는 최초 후보 승인만 제한한다. 최초 apply receipt만으로 auto-chain enrollment를 열지 않으며, 당일 postclose에서 actual selected prompt의 complete lifecycle 20건 이상, positive cost-adjusted EV, 그리고 최초 후보 대비 p10·severe-tail·HELD/unresolved 비열화가 모두 확인되어 `POST_APPLY_ATTRIBUTED`가 된 뒤에만 같은 exact V2.6 policy를 다음 거래일로 standing intent 갱신 없이 carry-forward한다. 어느 날이든 continuation gate가 실패하면 다음 거래일 activation은 생성되지 않아 configured `hot_v1`으로 자동 복귀한다. 새 prompt hash로 확대하려면 별도 bounded registry 계약이 필요하다.
