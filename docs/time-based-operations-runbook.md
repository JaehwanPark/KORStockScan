...
| swing runtime approval | 스윙 dry-run pre-final auto approval 생성, final-stage approval request 생성, dry-run runtime env 후보 연결 | Tier2 실패 상태의 env 반영, dry-run 해제, full-live 전환, legacy real canary 생성, 브로커 주문 허용 | `swing_runtime_approval`, `threshold_apply_YYYY-MM-DD.json`, runtime env JSON |
...
| `15:45` | cron | `deploy/run_threshold_cycle_postclose.sh` with OpenAI correction, 기본 `THRESHOLD_CYCLE_POSTCLOSE_BOT_ACTION=stop` | postclose threshold-cycle reports, ADM/LDM, source-quality audit, code improvement workorder, runtime approval summary, postclose verification, 다음 영업일 checklist | `[DONE]` marker, bot stop/isolation marker, required report freshness, AI review parsed 여부, source-quality gate, workorder handoff, verifier status를 확인한다 | 장후 resource isolation 정지는 threshold/order/provider 변경 권한이 아니다. report/proposal/verification 산출물만으로 실주문, provider, bot restart, cap, hard-safety를 변경하지 않는다 |
| `15:50` | cron | `deploy/run_postclose_done_controller.sh` | controller artifact, verifier final pass, optional source refresh/rerun | `postclose_done_controller_YYYY-MM-DD.{json,md}`, controller log, verifier pass/fail, recoverable warning closure를 확인한다 | 기본값에서는 Codex runner를 실행하지 않고 runner 완료를 `[DONE]` 조건으로 요구하지 않는다. opt-in runner도 `runtime_effect=false` safe scope만 처리한다 |
...
| `20:45` | cron | `deploy/run_logs_rotation_cleanup_cron.sh 30` before the 21:00 EC2 shutdown window | `logs/log_rotation_cleanup_cron.log` | `active_rotated`, `active_deleted`, `archive_deleted`, `archive_compressed`, `size_before/size_after`, `tmp_deleted`, `cache_deleted`, `sentinel_compressed`, `snapshot_compressed`, `raw_row_exclusion_deleted`, `raw_row_exclusion_backup_deleted` 추세 확인. 기본적으로 active cron/wrapper 로그가 20MB 이상이면 `.log.1`로 회전하고 숫자 백업 5개를 유지한다. 이때 `.log.1`은 plain으로 두고 기본 `LOG_ROTATION_COMPRESS_MIN_INDEX=2` 이상 숫자 백업은 gzip으로 압축한다. stale active 로그는 14일, 회전/압축/날짜형 archive 로그는 30일, `system_metric_samples.jsonl`은 3일 retention으로 정리한다. 추가로 repo 내부 임시 worktree/cache를 정리하고, 최신 target date를 제외한 sentinel event cache와 threshold-cycle pipeline snapshot은 gzip 압축하며, `raw_row_exclusion`은 같은 날짜 최신 run 1개만 유지하고 gzip backup은 기본 7일 retention 뒤 삭제한다 | 당일 장애 분석 전 로그 수동 삭제 금지. 최신 target date plain sentinel cache/snapshot, 원본 `data/pipeline_events`, clean-baseline quarantine/raw audit evidence는 cleanup 대상이 아니다. 회전/압축/날짜형 archive 로그도 retention 안에서는 cron completion/postclose verifier의 marker source로 인정한다 |
...
6. apply plan의 `swing_runtime_approval` 섹션에서 `requested`, `approved`, `blocked`, `selected`, `dry_run_forced`를 확인한다. `dry_run_auto_apply_ready`는 parsed AI Tier2 contract가 있어야 통과하며, `approval_required`는 final-stage 사용자 승인 artifact 없으면 정상 차단이다. Historical real-canary request/artifact가 있어도 env override가 생성되지 않아야 한다.
