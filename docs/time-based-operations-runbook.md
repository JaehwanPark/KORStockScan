...
- 표준 장후 `src.engine.scalping.ai_decision_quality --date YYYY-MM-DD --mode postclose --write`는 v2 source-label 계약의 control·원 라벨 두 artifact만 생산/대기한다. 원 라벨은 byte-preserving revision으로 보호하며 generic 가격 baseline/taxonomy는 라벨의 diagnostic section→현행 machine case table에 통합한다. 진단 proxy는 비용 후 경제성·실제 순익·독립 승격 근거가 아니다. 동일 frozen 원천/config/implementation/라벨 해시는 raw·가격/provider 재계산 없이 재사용하며 horizon 미도달과 고정 과거 window source gap을 분리한다. 승인된 기존 원천 migration은 `--reuse-materialized-labels`로 원 as-of·exclusion을 그대로 보존하며 자연 재검증으로 표기하지 않는다. Legacy standalone baseline/paired preparation/retired canary lifecycle은 native 필수 write/wait에서 제거한다. R0–R3 독립 연구 실행기 및 전용 PREOPEN/live adapter는 폐기했으며 과거 환경 플래그는 소비하지 않는다. 현행 compact 최종 policy/consumer/summary/strict는 `RUN_AI_DECISION_ACTION_OUTCOME_CALIBRATION`의 독립 finalization 경로를 유지한다. 검증된 운영 모델·원 비용·동일 자본·sealed chronological holdout·현재 scope proof만 현대적 dated publisher/PREOPEN/runtime가 소비하며 후보0이면 incumbent carry와 구체 blocker를 전달한다. 주문·guard·수량/budget/custody·operator override·provider/bot 권한은 보존한다. [ADQ 구현 review](audit-reports/2026-09-18-ai-quality-source-label-consolidation-review.md)를 따른다.
...
- V2.14 setup-risk의 full-day candidate 실행은 21:05 KST 별도 runner `deploy/run_ai_entry_setup_paired_replay_postclose.sh`가 소유한다. 이 runner는 main postclose controller의 target-date `succeeded`를 최대 12시간 기다린다. 대기 중 `failed/error/blocked/missing`은 tail-repair로 복구 가능한 중간 상태이며 즉시 종료하지 않는다. 12시간 안에 `succeeded`가 되면 60분 outcome 성숙을 확인한 뒤 KRX 정규장과 NXT 애프터마켓을 분리하고, 끝까지 복구되지 않으면 `blocked_predecessor_timeout`으로 종료한다. 고정 runner의 bounded wait보다 predecessor tail-repair가 늦게 끝나는 경우를 위해 `deploy/run_postclose_done_controller.sh`는 target-date 21:05 이후 controller 성공 직후 같은 runner를 fail-fast follower로 다시 호출한다. Entry terminal detailed 뒤에는 #82 calibration v5 → optimizer(`--preserve-entry-batch-selection --require-action-outcome-calibration`) → provider0 batch metadata-only binding → Holding base exact-hash manifest → Holding live-compatible paired replay/policy candidate → Main AI consumer closure를 갱신한다. 당일 실행 후보는 고정하며 다음 세션 권고와 구분한다. #81 legacy runtime OFF를 복구하지 않는다. Holding base manifest는 기존 optimizer/source-only 2×2 입력을 동결하고, 별도 `holding_prompt_live_policy --phase postclose`가 라이브 `holding_score` 스키마와 동일한 후보만 target-date 전체 최대 10건으로 실행·checkpoint 재사용한다. 후보는 clean baseline 이후 동일 stage×venue×session의 explicit cost-adjusted 결과 10건/3종목, EV `>=+0.10%`, Control 대비 비열화 없음, severe-tail 비증가를 모두 만족할 때만 다음 거래일용으로 발행된다. `TRIM`의 exact 잔여수량/비용 반사실이 없으면 해당 행은 승격 근거에서 제외한다. 07:35 PREOPEN은 cohort별 candidate/hash/date를 다시 검증해 activation을 쓰며, 결손·실패·비양수 경제성은 `holding_score_v2`로 복귀한다. 이 경로는 provider/model, 주문가격·수량·cap, deterministic exit/hard-safety를 바꾸지 않고 실제 PID 소비와 post-apply 경제성은 별도다. Consumer는 Entry/Holding base와 enriched common-parent 2×2 cell을 `connected_and_hash_bound|intentionally_blocked_with_owner_and_acceptance_test|retired_as_duplicate_of_existing_r0_r3` 중 하나로 종결하며 기존 R0-R3 exact cell을 중복 queue에 넣지 않는다. 21:05 refresh는 `--require-entry-terminal`을 사용하여 Entry가 exact-hash 연결됐거나 exact-date 자연 무표본으로 종결된 경우만 성공한다. 이전 optimizer hash, missing batch, 경로·prompt·input mismatch는 분류가 되어 있어도 재시도 상태다. 이미 `completed_offline_only` batch가 있고 KRX candidate reference가 있으면 source date, status, candidate 본문 재계산 self-hash, batch reference artifact hash와 candidate 파일뿐 아니라 consumer self-hash, source-only authority, exact 3-path census, child terminal status, `unclassified_request_path_count=0`, `entry_followup_terminal_ready=true`, `runtime_prompt_update_allowed=false`를 함께 검증한 뒤 중복 Provider 실행 없이 건너뛴다. follower가 0으로 끝났는데 terminal batch/consumer가 없으면 controller wrapper는 DONE을 출력하지 않고 fail-closed한다. 각 Entry cohort는 outcome을 읽지 않는 symbol round-robin 방식으로 신규 candidate를 최대 30건 실행한다. 전체 eligible 수가 잔여 cap 이하이면 `complete_eligible_census`를 허용하되 `deferred=0`, `selected=eligible`, `distinct<=cap`, outcome-blind 계약을 모두 검증한다. 결과가 있는 요청만 checkpoint report에 넣어 일부 실행을 `missing result` 결함으로 오판하지 않는다. Transient provider/schema 실패는 이미 검증된 결과 checkpoint를 재사용해 기본 2회까지만 재시도한다. OpenAI key/provider preflight 실패나 `provider=none`, schema/selection contract 결손은 fail-closed이다.
