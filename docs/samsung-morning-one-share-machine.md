...
`ka10080` 분봉 읽기는 다른 독립 에피소드 기계와 공용 요청 제어를 사용한다. 같은 KST 분 안의 정상 완료봉 snapshot은 프로세스 안에서 재사용하고, 분 경계의 프로세스 간 호출은 관측 요청량에 여유를 둔 로컬 0.4초 간격으로 직렬화한다. 명시적인 `1700`/HTTP 429 읽기만 최대 2회 bounded backoff 재시도하며, 실패·계약오류 snapshot은 캐시하지 않는다. 주문·취소 API는 중복 주문 방지를 위해 이 재시도 경로에서 제외한다.
