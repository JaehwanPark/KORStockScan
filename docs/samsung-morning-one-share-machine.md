...
- 하루 신규 매수는 `005930` 1주 1회로 제한한다. NXT PREMARKET이 우선이고 NXT 미체결 주문의 취소·미체결이 계좌조회로 확인된 뒤에만 09:00 SOR 통합 주문을 허용한다.
...
| 순서 | 시장 | 기준가 | 매수가 | 주문 종료 |
|---:|---|---|---|---|
| 1 | NXT PREMARKET | 08:00 첫 1분봉 시가 | 기준가 대비 -3.0%, 유효 호가 내림 | 08:10 |
| 2 | SOR 정규장 | KRX 09:00 첫 1분봉 시가를 가격 기준점으로 사용 | 기준가 대비 -0.75%, 유효 호가 내림 | 09:30 |

체결 후 실제 체결가에서 +2호가 지정가 매도를 낸다. 목표 주문에는 시간청산과 손절이 없다. 주문이 열려 있는 동안 계속 체결을 확인하고, 브로커에서 미체결 종료된 것이 확인되면 `HELD`로 종료해 1주를 그대로 보유한다. 목표 주문 취소, 최우선 지정가 강제매도, 다음 날 자동 목표 재주문, 보유 중 신규매수는 하지 않는다.
...
추가 월의 2026-05-26 NXT 08:00 봉은 297,000원 시가, 240,000원 저가, 거래량 227주이고 다음 봉은 08:04에 시작해 297,000원 이상으로 복귀했다. 지정가 체결을 288,000원으로 가정할 때 분봉상 최대 불리폭은 -16.67%다. 이는 +2호가 도달 여부와 별개로 NXT 초기 유동성·분봉 순서 및 무손절 보유 위험이 매우 크다는 증거다. 사용자의 명시적 무손절·미청산 보유 원칙을 적용하되 1주 상한은 유지한다.
...
- `window_policy`: at most one NXT-premarket-first/SOR-regular-fallback entry episode per trading date; unfilled target becomes held inventory
...
키움 공식 참조는 `Kiwoom-Securities/Kiwoom-REST-API` commit `69642586f7d84ba9fd8a6faf1f1537c7fda6568b`이며, `주문.md`, `계좌.md`, `차트.md`, API spec, Postman을 2026-08-11 11:43:54 KST에 다시 대조했다. 공식 `kt10000/kt10001/kt10003/kt00007` 계약의 `dmst_stex_tp`가 `SOR`를 지원함을 확인했다. 사용 API는 `ka10080`, `kt10000`, `kt10001`, `kt10003`, `kt00007`뿐이다.
...
전용 기계는 `COMPLETE`, `NO_TRADE`, `HELD`, `BLOCKED`에서 종료한다. `HELD`는 목표가 매도가 체결되지 않아 전용 1주를 그대로 보유하는 정상 종결 상태다. 실패 재시작도 당일 권한 artifact와 원자적 write-intent 상태를 다시 검증하므로 모호한 broker write를 반복하지 않는다. 전용 기계 문제가 생기면 다음 unit만 중지하며 widget unit에는 손대지 않는다.
