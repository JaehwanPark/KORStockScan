...
스캘핑 sim-only `scalp_sim_panic_lifecycle`은 이 context를 `lifecycle_decision_matrix_runtime`의 `risk_regime_context`로 소비한다. `panic_level>=2` 또는 liquidity broken은 방어 actuator로 entry/scale-in block과 holding 축소/청산만 허용한다. 반면 `panic_level=1` market breadth risk-off에서는 개별 종목 feature가 `symbol_regime=BOTTOMING`으로 판정될 때 `risk_off_bottoming_probe_entry`를 sim-only로 열 수 있다. 이 경우 모든 event는 `actual_order_submitted=false`, `broker_order_forbidden=true`, `decision_authority=sim_observation_only`를 유지하고, lifecycle matrix에는 `scalp_sim_panic_bottoming_entry_allowed` entry row로만 들어간다. 이 경로는 real broker submit, Telegram BUY 알림, threshold/provider/bot 변경 권한이 없다.
