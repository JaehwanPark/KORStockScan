...
7. `lifecycle_decision_matrix`가 entry ADM, sim post-sell evaluation, wait/missed-entry cohort 등 기존 source adapter를 읽고, 있으면 institutional flow feature를 `runtime_features`에 join해 lifecycle row matrix와 weighted ADM policy를 생성한다. 이 artifact는 label future leakage를 금지하고, selected family가 될 때만 다음 PREOPEN env policy로 소비된다. `entry_bucket_attribution`은 같은 row/label에서 `score_band`, `source_stage`, `stale_bucket`, `liquidity_bucket`, `strength_bucket`, `overbought_bucket`, `time_bucket`, 복합 entry spot별 EV/MFE/MAE를 자동 집계하고 `runtime_approval_candidates`와 `code_improvement_workorders`를 source-only로 노출한다. 해당 candidate는 `approval_required=true`, `allowed_runtime_apply=false`이며 rolling/cumulative 확인 전 단독 runtime apply 권한이 없다.
...
## 2.4 LDM Entry Bucket Handoff Contract
