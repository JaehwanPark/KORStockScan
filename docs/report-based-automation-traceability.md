...
5. `scalp_entry_action_decision_matrix`가 entry snapshot, sim buy/sell, sim post-sell evaluation을 `candidate_id`/`sim_record_id`로 join해 action별 ADM source를 생성한다. 기본 산출물은 report/workorder source지만, 운영 override에서는 해당 source와 hypothesis fallback이 AI `BUY`를 `WAIT`/`DROP`으로 보정할 수 있다. broker submit/order guard 권한은 없고, 기존 pre-submit safety guard가 항상 우선한다.
...
AI prompt 반영은 `KORSTOCKSCAN_SCALP_ENTRY_ADM_ADVISORY_ENABLED=true`일 때 entry prompt에만 `[Entry ADM Advisory Context]`를 append한다. 기존 응답 schema는 바꾸지 않고 cache token에 `entry_adm:<matrix_version>:<bucket_token>`을 포함한다. 운영 override의 직접 영향도는 `KORSTOCKSCAN_SCALP_ENTRY_ADM_RUNTIME_BIAS_ENABLED=true`가 담당한다. matched bucket의 dominant action이 `WAIT_REQUOTE/SKIP_STALE`이면 `BUY -> WAIT`, `NO_BUY_AI/SKIP_SOURCE_QUALITY/SKIP_PRE_SUBMIT_SAFETY`이면 `BUY -> DROP`, `BUY_NOW/BUY_DEFENSIVE` bucket의 `source_quality_adjusted_ev_pct < 0`이면 `BUY -> WAIT`로 보정한다. joined sample이 부족해도 `KORSTOCKSCAN_SCALP_ENTRY_ADM_HYPOTHESIS_FALLBACK_ENABLED=true`이면 stale high, source-quality blocker, low liquidity, weak momentum + chase risk 조합을 WAIT/DROP 가설로 적용한다. stale quote, liquidity, overbought, latency, price freshness pre-submit safety guard는 여전히 ADM보다 우선한다.
...
`holding_exit_decision_matrix`는 운영 override에서 보유/청산 AI flow에도 직접 연결된다. `KORSTOCKSCAN_HOLDING_EXIT_MATRIX_RUNTIME_BIAS_ENABLED=true`이면 matrix bias가 `prefer_exit`일 때 `HOLD/TRIM -> EXIT`, `prefer_avg_down_wait` 또는 `prefer_pyramid_wait`일 때 `EXIT/TRIM -> HOLD`로 보정할 수 있다. matrix entry가 비어 있더라도 hypothesis fallback은 손실 구간의 `AVG_DOWN` 대기, 수익 구간의 `PYRAMID` 대기를 `holding_exit_matrix_scale_in_bias`로 남기고, `KORSTOCKSCAN_HOLDING_EXIT_MATRIX_SCALE_IN_BIAS_ENABLED=true`이면 scale-in evaluator가 `holding_exit_matrix_avg_down_bias` 또는 `holding_exit_matrix_pyramid_bias` action을 생성할 수 있다.

hard stop, emergency stop, market closed, invalid feature 같은 hard veto는 ADM보다 우선한다. scale-in 주문은 기존 `ENABLE_SCALE_IN`, 계좌 cap, cooldown, price resolver, spread, qty cap, swing canary approval guard를 통과해야 하며 ADM이 broker submit guard를 우회하지 않는다.
