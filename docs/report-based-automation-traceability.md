...
| `data/report/runtime_apply_bridge/runtime_apply_bridge_YYYY-MM-DD.{json,md}` | `runtime_apply_bridge` | `threshold_cycle_preopen_apply`, operator PREOPEN/POSTCLOSE review | `R4_preopen_apply_candidate -> R5_bounded_calibrated_apply` LDM bucket bridge | `RuntimeApplyBridge0521` | LDM entry/scale bucket을 named family `entry_wait6579_score66_69_recovery_gate_v1`, `scale_in_bucket_runtime_policy_v1`로 변환한다. report 자체는 `runtime_effect=false`이며 `bridge_candidate_state=ready_for_approval`와 별도 approval artifact(`ldm_entry_runtime_bridge_YYYY-MM-DD.json`, `ldm_scale_in_runtime_bridge_YYYY-MM-DD.json`)가 모두 있어야 다음 PREOPEN env에 쓰인다. bootstrap/source-quality/rolling conflict 상태, artifact missing, unknown family는 `runtime_apply_blocked_bridge_not_ready` 또는 contract gap으로 닫고, broker guard/qty cap/price resolver/cooldown/near-close guard 우회 금지 |
