...
6. `lifecycle_decision_matrix`가 entry ADM, sim post-sell evaluation, wait/missed-entry cohort 등 기존 source adapter를 읽어 lifecycle row matrix와 weighted ADM policy를 생성한다. 이 artifact는 label future leakage를 금지하고, selected family가 될 때만 다음 PREOPEN env policy로 소비된다.
...
7. `pipeline_event_verbosity_report`가 raw volume과 V1/V2 producer summary parity를 생성한다. 이 artifact는 workorder source-quality/ops 입력이며 threshold/order/provider/bot restart 권한이 없다.
7. `codebase_performance_workorder_report`가 코드베이스 성능점검 문서를 workorder source artifact로 변환한다. 이 artifact는 user-instructed performance backlog이며 전략 로직/데이터품질/튜닝축 변경 권한이 없다.
...
`lifecycle_decision_matrix`는 고정 score bucket 정책이 아니라 개별 후보의 `runtime_features`와 사후 `labels`를 분리한 lifecycle row matrix다. runtime 입력 가능 feature는 AI score/action, microstructure, latency/source quality, price resolver, OFI/QI, position PnL/peak/held_sec, 현재 적용된 fixed threshold 값, scale-in add type/가격/수량 provenance로 제한하고, `mfe_10m_pct`, `mae_10m_pct`, `close_10m_pct`, realized profit, fill outcome, avoided loss, missed upside는 label 전용이다. runtime resolver는 label 필드를 읽지 않는다.
