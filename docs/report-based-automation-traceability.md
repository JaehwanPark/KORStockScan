...
| `data/report/plan_rebase_daily_renewal/plan_rebase_daily_renewal_YYYY-MM-DD.{json,md}` | `plan_rebase_daily_renewal` | README/런북(runbook)/Plan Rebase/prompt/AGENTS daily renewal queue | `R6_post_apply_attribution` bounded document mutation queue | `PlanRebaseDailyRenewal0513` | postclose source bundle 기반 bounded 문서 갱신(document mutation), `document_mutation_allowed=true`, `runtime_mutation_allowed=false`, `1차 수정(first-pass bounded update) -> 2차 감리(second-pass audit review) -> 최종 수정(finalize)` |
...
12. `plan_rebase_daily_renewal`은 `runtime_approval_summary` 이후 README/런북(runbook)/Plan Rebase/prompt/AGENTS 갱신을 후순위 문서 작업으로 queue한다. 기본은 `bounded_document_mutation_queue`이며 `document_mutation_allowed=true`, `runtime_mutation_allowed=false`, `document_mutation_self_apply=false`다. 실제 갱신은 후행 문서 작업자가 `1차 수정(first-pass bounded update) -> 2차 감리(second-pass audit review) -> 최종 수정(finalize after second-pass review)`로 닫고, 이력성 내용은 archive 또는 execution-delta로 보존하며 영어 약칭은 첫 사용 시 한글/영어 병기를 원칙으로 한다.
...
- `plan_rebase_daily_renewal`은 README/런북(runbook)/Plan Rebase/prompt/AGENTS에 한해 `document_mutation_allowed=true`인 후순위 문서 갱신(document mutation) queue다. 다만 `runtime_mutation_allowed=false`는 유지하며 checklist 신규 작업항목, runtime env, threshold, 주문, provider, bot restart를 수정하지 않는다. 문서 갱신 자체도 `1차 수정(first-pass bounded update) -> 2차 감리(second-pass audit review) -> 최종 수정(finalize)` 없이는 완료로 보지 않는다.
