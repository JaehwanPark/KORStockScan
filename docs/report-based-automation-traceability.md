...
2. `swing_daily_simulation_report`를 먼저 생성하고, 그 직후 `swing_strategy_discovery_sim -> swing_strategy_discovery_label_builder --refresh-matured -> swing_strategy_discovery_ev_report`를 source-only로 실행한다. `swing_lifecycle_audit`/`swing_runtime_approval`은 해당 JSON/Markdown이 존재하고 JSON 검증이 끝난 뒤에만 실행한다.
