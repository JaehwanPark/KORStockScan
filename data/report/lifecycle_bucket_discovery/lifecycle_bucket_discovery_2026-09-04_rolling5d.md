# Lifecycle Bucket Discovery - 2026-09-04_rolling5d
...
- surfaced_candidate_count: `98`
- canonical/legacy buckets: `168` / `480`
- dual_proposals: deterministic=`500` ai=`1` hybrid_selected=`1`
- absorbed/source_quality_blocker: `343` / `0`
- lifecycle_flow_parent_granularity: `target_pass` level=`L2_default` parents=`36` target=`30-60`
- lifecycle_flow_absorbed_children: child=`91` sample=`7807` conflict_parents=`0`
...
- active_sim_priority: eligible=`2` active=`2` source_blocked=`8` parent_count_decoupled=`0`
...
- quiet_gap_count: `406` / sim_live_connected: `0`
...
### AI Review Shards
- `live_contract_review` status=`skipped_empty` candidates=`0` omitted=`0` context_chars=`34506`
- `lifecycle_flow_review` status=`parsed` candidates=`1` omitted=`90` context_chars=`39580`
- `sim_policy_review` status=`skipped_empty` candidates=`0` omitted=`0` context_chars=`34476`
- `gap_workorder_review` status=`skipped_empty` candidates=`0` omitted=`0` context_chars=`34482`
- `taxonomy_discovery_review` status=`skipped_empty` candidates=`0` omitted=`0` context_chars=`34478`
...
- `lifecycle_flow:combo_lifecycle_flow:entry_entry_combo_entry_spot_score_score_lt60_source_scalp_entry_action_decision_snapshot_stale_` stage=`lifecycle_flow` state=`source_only_keep_collecting` action=`tighten_or_exclude` relation=`existing_bucket_refinement` canonical=`lifecycle_flow:combo_lifecycle_flow:entry=entry_observed|submit=submit_observed|holding=holding_observed|scale_in=scale_in_observed|exit=exit_observed` joined=`4` ev=`-0.8275` ai_final=`-` taxonomy=`absorb_as_dimension`
...
- `lifecycle_flow:combo_lifecycle_flow:entry_entry_combo_entry_spot_score_score_63_65_source_scalp_entry_action_decision_snapshot_stale` stage=`lifecycle_flow` state=`source_only_keep_collecting` action=`tighten_or_exclude` relation=`existing_bucket_refinement` canonical=`lifecycle_flow:combo_lifecycle_flow:entry=score_watch_recovery|submit=submit_observed|holding=holding_observed|scale_in=scale_in_observed|exit=exit_observed` joined=`1` ev=`-0.75` ai_final=`-` taxonomy=`absorb_as_dimension`
