# Lifecycle Bucket Discovery - 2026-09-04
...
- surfaced_candidate_count: `39`
- canonical/legacy buckets: `130` / `265`
- dual_proposals: deterministic=`281` ai=`2` hybrid_selected=`1`
- absorbed/source_quality_blocker: `162` / `0`
- lifecycle_flow_parent_granularity: `too_broad` level=`L2_default` parents=`15` target=`30-60`
- lifecycle_flow_absorbed_children: child=`30` sample=`642` conflict_parents=`0`
...
- active_sim_priority: eligible=`0` active=`0` source_blocked=`12` parent_count_decoupled=`0`
...
- sim_policy_approved_total_count: `2` (direct=`1`, entry_only=`1`, lifecycle_flow=`0`)
- lifecycle_flow_sim_probe_candidate_count: `0`
- source_dimension_gap_count: `26` / actionable_unknown_gap_count: `0`
- quiet_gap_count: `233` / sim_live_connected: `0`
...
### AI Review Shards
- `live_contract_review` status=`skipped_empty` candidates=`0` omitted=`0` context_chars=`34871`
- `lifecycle_flow_review` status=`parsed` candidates=`1` omitted=`29` context_chars=`39911`
- `sim_policy_review` status=`parsed` candidates=`1` omitted=`1` context_chars=`38332`
- `gap_workorder_review` status=`skipped_empty` candidates=`0` omitted=`0` context_chars=`34847`
- `taxonomy_discovery_review` status=`skipped_empty` candidates=`0` omitted=`0` context_chars=`34843`

### Source Dimension Gap Enrichment
- gap_count: `26` / actionable_unknown_gap_count: `0`
...
- `lifecycle_flow:combo_lifecycle_flow:entry_entry_combo_entry_spot_score_score_lt60_source_scalp_entry_action_decision_snapshot_stale_` stage=`lifecycle_flow` state=`source_only_keep_collecting` action=`tighten_or_exclude` relation=`existing_bucket_refinement` canonical=`lifecycle_flow:combo_lifecycle_flow:entry=entry_observed|submit=submit_observed|holding=holding_observed|scale_in=scale_in_observed|exit=exit_observed` joined=`1` ev=`-0.84` ai_final=`-` taxonomy=`absorb_as_dimension`
