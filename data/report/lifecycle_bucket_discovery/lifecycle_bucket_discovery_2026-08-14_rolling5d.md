# Lifecycle Bucket Discovery - 2026-08-14_rolling5d
...
- surfaced_candidate_count: `88`
- canonical/legacy buckets: `155` / `483`
- dual_proposals: deterministic=`500` ai=`36` hybrid_selected=`36`
- absorbed/source_quality_blocker: `356` / `0`
- lifecycle_flow_parent_granularity: `target_pass` level=`L2_default` parents=`40` target=`30-60`
- lifecycle_flow_absorbed_children: child=`88` sample=`7168` conflict_parents=`1`
...
- active_sim_priority: eligible=`1` active=`1` source_blocked=`9` parent_count_decoupled=`0`
...
- source_dimension_gap_count: `63` / actionable_unknown_gap_count: `0`
- quiet_gap_count: `394` / sim_live_connected: `1`
...
### AI Review Shards
- `live_contract_review` status=`skipped_empty` candidates=`0` omitted=`0` context_chars=`37649`
- `lifecycle_flow_review` status=`parsed` candidates=`1` omitted=`87` context_chars=`44216`
- `sim_policy_review` status=`parsed` candidates=`1` omitted=`1` context_chars=`44179`
- `gap_workorder_review` status=`skipped_empty` candidates=`0` omitted=`0` context_chars=`37625`
- `taxonomy_discovery_review` status=`skipped_empty` candidates=`0` omitted=`0` context_chars=`37621`

### Source Dimension Gap Enrichment
- gap_count: `63` / actionable_unknown_gap_count: `0`
...
- `lifecycle_flow:combo_lifecycle_flow:entry_entry_combo_entry_spot_score_score_63_65_source_scalp_sim_entry_ai_price_skip_order_stale_` stage=`lifecycle_flow` state=`source_only_keep_collecting` action=`tighten_or_exclude` relation=`existing_bucket_refinement` canonical=`lifecycle_flow:combo_lifecycle_flow:entry=score_watch_recovery|submit=submit_observed|holding=holding_observed|scale_in=scale_in_observed|exit=exit_observed` joined=`1` ev=`-2.1951` ai_final=`-` taxonomy=`absorb_as_dimension`
- `lifecycle_flow:combo_lifecycle_flow:entry_entry_combo_entry_spot_score_score_63_65_source_scalp_sim_entry_ai_price_skip_order_stale_` stage=`lifecycle_flow` state=`source_only_keep_collecting` action=`hold_no_edge` relation=`existing_bucket_refinement` canonical=`lifecycle_flow:combo_lifecycle_flow:entry=score_watch_recovery|submit=submit_observed|holding=holding_observed|scale_in=scale_in_observed|exit=exit_observed` joined=`1` ev=`-0.0415` ai_final=`-` taxonomy=`absorb_as_dimension`
- `lifecycle_flow:combo_lifecycle_flow:entry_entry_combo_entry_spot_score_score_70p_source_blocked_ai_score_stale_fresh_liquidity_liqui` stage=`lifecycle_flow` state=`lifecycle_flow_sim_probe_candidate` action=`hold_no_edge` relation=`existing_bucket_refinement` canonical=`lifecycle_flow:combo_lifecycle_flow:entry=score_mid_recovery|submit=submit_observed|holding=holding_observed|scale_in=scale_in_observed|exit=exit_observed` joined=`1` ev=`0.1639` ai_final=`-` taxonomy=`absorb_as_dimension`
...
- `lifecycle_flow:combo_lifecycle_flow:entry_entry_combo_entry_spot_score_score_70p_source_scalp_sim_entry_ai_price_skip_order_stale_st` stage=`lifecycle_flow` state=`source_only_keep_collecting` action=`tighten_or_exclude` relation=`existing_bucket_refinement` canonical=`lifecycle_flow:combo_lifecycle_flow:entry=score_mid_recovery|submit=submit_observed|holding=holding_observed|scale_in=scale_in_observed|exit=exit_observed` joined=`1` ev=`-1.3447` ai_final=`-` taxonomy=`absorb_as_dimension`
