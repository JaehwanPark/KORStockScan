# Main AI Prompt Optimizer - 2026-09-03
...
## Stage Optimizers
- `entry` aggregate base/full-factorial parents=`78/5`; selection is cohort-only
  - `KRX/KRX_REGULAR` champion=`decision_quality_v2_13_recovery_confirmation_probe` challenger=`decision_quality_v2_14_setup_risk_adjudicator` action=`continue_current_challenger_new_mature_parents_only` base/full-factorial parents=`58/1`
  - `NXT/NXT_AFTERMARKET` champion=`decision_quality_v2_13_recovery_confirmation_probe` challenger=`decision_quality_v2_14_setup_risk_adjudicator` action=`continue_current_challenger_new_mature_parents_only` base/full-factorial parents=`20/4`
- `holding` aggregate base/full-factorial parents=`97/7`; selection is cohort-only
