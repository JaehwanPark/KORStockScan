...
- 근거: LDM `sim_auto_approved=123` (`-17`), `live_auto_apply_ready=0` (`+0`), swing sim-auto `8` (`+8`).
...
- Parent bucket: daily parent_granularity_status `45`/`target_pass`, mtd `36`/`target_pass`, absorbed_sample `145239`, conflict_children `16`.
- Bridge/verifier: greenfield_policy_emit_state `not_emitted_no_complete_lifecycle_flow`, promotion_contract_passed `False`, verifier_status `fail`, verifier_missing `[]`.
...
- Swing matrix: rows `63838` (`+735`), probe `59752` (`-147`), pending future quotes `2729` (`+432`).
- Swing bucket: sim-auto `8` (`+8`), code-patch `0` (`+0`).
...
- selected orders `154`, selected decisions `{"attach_existing_family": 104, "implement_now": 50}`, routes `{"auto_patch_required": 48, "existing_family": 104, "implement_now": 2}`.
...
- runtime_apply_bridge: `/home/ubuntu/KORStockScan/data/report/runtime_apply_bridge/runtime_apply_bridge_2026-05-29.json` exists=true json_valid=true
...
- threshold_cycle_postclose_verification: `/home/ubuntu/KORStockScan/data/report/threshold_cycle_postclose_verification/threshold_cycle_postclose_verification_2026-05-29.json` exists=true json_valid=true
