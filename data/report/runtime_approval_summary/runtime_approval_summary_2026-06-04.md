...
- first_bot_start_at: `2026-06-04T07:40:02`
- first_bot_start_after_env_at: `2026-06-04T07:40:02`
...
| `score65_74_recovery_probe` | family id는 score65_74로 유지하지만 현 runtime floor 기준 AI 점수 60~74 WAIT 구간 중 수급/가속 조건이 좋은 후보를 1주/소액 canary로 회수하는 축 | 관찰/리포트 only: runtime 변경 없음 | `hold_sample` | `entry_unlock_probe` | runtime env/operator lock plus post-apply attribution | 축은 유지/관찰하지만 표본 부족으로 runtime 변경은 하지 않는다 | 0.1 | `-` | 표본 부족 |
...
| `swing_gatekeeper_accept_reject` | 스윙 gatekeeper가 accept/reject한 후보의 후행 성과를 비교하는 진입 판단 축 | 스윙 dry-run/probe 관찰: 실주문 변경 없음 | `hold_sample` | `legacy_hard_gate_contract_gap` | workorder로 accept/reject policy guard를 만들거나 reject cooldown family로 우회 조정 | 축은 유지/관찰하지만 표본 부족으로 runtime 변경은 하지 않는다 | 0.3131 | `-` | 표본 부족, 계측 gap, runtime guard 없음 |
| `swing_gatekeeper_reject_cooldown` | gatekeeper reject 이후 같은 후보를 다시 볼 cooldown 시간을 조정하는 축 | 스윙 dry-run/probe 관찰: 실주문 변경 없음 | `hold_sample` | `approval_route_available` | ML_GATEKEEPER_REJECT_COOLDOWN approval/env route | 축은 유지/관찰하지만 표본 부족으로 runtime 변경은 하지 않는다 | 0.3131 | `-` | 표본 부족, 계측 gap |
...
| `swing_trailing_stop_time_stop` | 스윙 trailing/time stop 청산 조건의 적정성을 보는 exit 축 | 스윙 dry-run/probe 관찰: 실주문 변경 없음 | `hold_sample` | `runtime_contract_gap_exit_axis` | exit runtime guard/workorder before approval | 축은 유지/관찰하지만 표본 부족으로 runtime 변경은 하지 않는다 | 0.3131 | `-` | 표본 부족, 계측 gap, runtime guard 없음 |
