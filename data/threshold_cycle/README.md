...
- postclose 제출물은 `threshold_cycle_ev_YYYY-MM-DD.{json,md}` daily EV 리포트로 통일한다. 스윙은 예외적으로 `swing_runtime_approval`의 요청을 daily EV와 preopen apply manifest에 노출한다. 일반 swing runtime 요청은 수동 approval artifact 없이는 env를 쓰지 않지만, phase0 `swing_one_share_real_canary_phase0`와 `swing_scale_in_real_canary_phase0`는 source report auto approval로 env를 쓸 수 있다.
...
| `data/report/swing_runtime_approval/swing_runtime_approval_YYYY-MM-DD.{json,md}` | 스윙 hard floor + EV trade-off score 기반 요청과 blocked reason. 일반 runtime 후보는 `approval_required`이며 요청 자체는 runtime 변경이 아니다. Phase0 one-share/scale-in real canary 요청은 source-quality/cap/allowlist 통과 시 `auto_approved_real_canary`로 다음 PREOPEN에 소비될 수 있다 |
...
| `data/threshold_cycle/approvals/swing_one_share_real_canary_YYYY-MM-DD.json` | 스윙 초기 BUY/SELL 1주 real canary 전용 optional narrowing artifact. Phase0 auto approval은 source report allowlist/cap을 기본으로 쓰며, 이 artifact가 있으면 종목 allowlist와 cap을 더 좁힐 수 있다 |
| `data/threshold_cycle/approvals/swing_scale_in_real_canary_YYYY-MM-DD.json` | 승인된 real swing holding의 AVG_DOWN/PYRAMID 1주 추가매수 real canary 전용 optional narrowing artifact. Phase0 auto approval은 source report arm/cap을 기본으로 쓰며, 이 artifact가 있으면 arm과 cap을 더 좁힐 수 있다 |
...
`swing_one_share_real_canary`는 위 dry-run approval과 별도의 broker execution 품질 수집축이다. 시작 조건은 `swing_runtime_approval` hard floor와 EV trade-off 통과, source report phase0 auto approval, 장전 preopen apply, DB load/instrumentation/fallback contamination/severe downside/same-stage conflict 없음이다. Optional real-canary artifact `data/threshold_cycle/approvals/swing_one_share_real_canary_YYYY-MM-DD.json`는 code allowlist/cap narrowing에만 사용한다. phase0 guard는 `KORSTOCKSCAN_SWING_ONE_SHARE_REAL_CANARY_ENABLED=true`, 승인 code allowlist, `qty=1`, `max_new_entries_per_day=1`, `max_open_positions=3`, `max_total_notional_krw=300000`, same-symbol active real canary 1개, AVG_DOWN/PYRAMID/scale-in 실주문 금지다. `threshold_cycle_preopen_apply`는 source report auto approval이 없으면 env를 만들지 않고, auto approval이 있어도 `KORSTOCKSCAN_SWING_LIVE_ORDER_DRY_RUN_ENABLED=true`를 유지한다. 이 canary의 realized PnL은 combined EV에 들어갈 수 있지만, broker receipt/order number binding/fill ratio/slippage/cancel/sell receipt는 real-only로만 판정한다. 통과해도 스윙 전체 실주문 전환은 별도 2차 계획/승인 없이는 금지한다.
