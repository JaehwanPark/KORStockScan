...
적용 범위는 스캘핑 전용이 아니다. 이 README는 스캘핑과 스윙이 공통 threshold-cycle, daily EV, preopen runtime env, code-improvement workorder 체인에 들어오는 부분의 산출물 정의다. 스캘핑 `scalp_ai_buy_all` live simulator는 `scalp_sim_*` pipeline event와 quote-based completed rows를 만들며, report는 `real`/`sim`/`combined`를 분리 표시한다. `combined`는 diagnostic-only이며 실매매 동급 calibration, broker execution 품질, 실주문 전환 근거로 사용하지 않는다. 스윙 전용 흐름은 `swing_strategy_discovery_sim -> labels / EV -> swing_lifecycle_decision_matrix -> swing_lifecycle_bucket_discovery -> swing_sim_auto_approval / swing_sim_policy_catalog -> PREOPEN sim policy env -> runtime sim/probe observation -> postclose attribution -> final live conversion approval`이다. 일반 스윙 dry-run env 반영은 hard floor/source-quality와 parsed AI Tier2 review가 닫힌 `dry_run_auto_apply_ready`일 때 pre-final auto approval로 다음 장전 env에 반영한다. Tier2 missing/unavailable/parse-rejected는 fail-closed다. 반영 후에도 `SWING_LIVE_ORDER_DRY_RUN_ENABLED=True`와 브로커 주문 차단은 유지된다. `swing_one_share_real_canary_phase0`와 `swing_scale_in_real_canary_phase0`는 제거된 legacy stage이며, historical approval artifacts are ignored by PREOPEN apply. 사용자 승인은 final full live conversion, bounded cap 초과 cap release, provider/bot 변경, hard/protect/emergency safety 완화에만 남긴다.
...
| POSTCLOSE 16:10 | `deploy/run_threshold_cycle_postclose.sh` | raw pipeline event를 family partition으로 backfill하고 장후 report, AI correction, ADM/lifecycle matrix, lifecycle AI context attribution/context, scalping/swing automation, latency classifier 추천, rising missed first-touch calibration, 전체 one-share event 기반 PYRAMID opportunity feedback 및 quality calibration, daily EV report를 생성 | `date=YYYY-MM-DD/family=*/part-*.jsonl`, `data/report/threshold_cycle_YYYY-MM-DD.json`, 파생 `statistical_action_weight`, `scalp_entry_action_decision_matrix`, `lifecycle_decision_matrix`, `lifecycle_ai_context_attribution`, `lifecycle_ai_context`, `holding_exit_decision_matrix`, `threshold_cycle_cumulative` JSON/MD, `threshold_cycle_ai_review_YYYY-MM-DD_postclose.{json,md}`, `latency_classifier_recommendation_YYYY-MM-DD.{json,md}`, `rising_missed_intraday_feedback_YYYY-MM-DD.{json,md}`, `rising_missed_first_touch_calibration_YYYY-MM-DD.{json,md}`, `scalping_pyramid_intraday_feedback_YYYY-MM-DD.{json,md}` including `one_share_pyramid_opportunity_rows`, `scalping_pyramid_quality_calibration_YYYY-MM-DD.{json,md}`, `scalping_pattern_lab_automation_YYYY-MM-DD.{json,md}`, `swing_improvement_automation_YYYY-MM-DD.{json,md}`, `swing_runtime_approval_YYYY-MM-DD.{json,md}`, `swing_pattern_lab_automation_YYYY-MM-DD.{json,md}`, `swing_strategy_discovery_sim_YYYY-MM-DD.{json,md}` including bottom rebound source when its source-only contract passes, `threshold_cycle_ev_YYYY-MM-DD.{json,md}`, `runtime_approval_summary_YYYY-MM-DD.{json,md}`, `runtime_apply_gap_audit_YYYY-MM-DD.{json,md}`, `data/report/threshold_cycle_postclose_status/threshold_cycle_postclose_YYYY-MM-DD.status.json` |
