...
| `data/report/swing_strategy_discovery_sim/swing_strategy_discovery_sim_YYYY-MM-DD.{json,md}` | 스윙 safe pool 후보를 sim-only candidate와 8개 가상 arm으로 확장하는 source-only artifact |
...
| `data/report/plan_rebase_daily_renewal/plan_rebase_daily_renewal_YYYY-MM-DD.{json,md}` | README/런북(runbook)/Plan Rebase/prompt/AGENTS daily renewal queue. 기본 `bounded_document_mutation_queue`, `document_mutation_allowed=true`, `runtime_mutation_allowed=false`. 실제 갱신은 `1차 수정(first-pass bounded update) -> 2차 감리(second-pass audit review) -> 최종 수정(finalize)` 순서로만 닫는다 |
...
스윙 discovery sim은 이 sim-first 원칙의 스윙 탐색 구현체다. `swing_strategy_discovery_sim`이 safe pool 후보와 8개 arm을 만들고, `swing_strategy_discovery_label_builder`가 성숙 quote 기준으로 label을 채우며, `swing_strategy_discovery_ev_report`가 source-only EV를 집계한다. postclose wrapper는 `swing_daily_simulation` 직후 이 세 단계를 실행한다. `threshold_cycle_ev`와 `runtime_approval_summary`는 candidate/arm/labeled/pending/top surviving/avoid bucket 요약만 소비하고, `code_improvement_workorder`는 `runtime_effect=false` source-quality/report order만 만들 수 있다. 이 체인은 기존 스윙 runtime이나 `recommendation_history`를 대체하지 않는다.
