"""Admin-only Telegram notices for Doosan widget entry and exit events.
...
from src.engine.monitoring.doosan_widget_contract import DOOSAN_CODE, DOOSAN_NAME
...
DEFAULT_STATE_FILE = PROJECT_ROOT / "tmp" / "doosan_widget_telegram_notify_state.json"
...
def _env_enabled() -> bool:
    return str(
        os.getenv("KORSTOCKSCAN_DOOSAN_WIDGET_TELEGRAM_ENABLED", "true")
...
def build_exit_message(payload: dict[str, Any], event: dict[str, Any]) -> str:
    ...
    reason_label = {
        "doosan_target_1pct_reached": "+1% 기준가 도달",
        "doosan_completed_close_below_entry_support": "확정 1분봉 지지 이탈",
...
class DoosanWidgetTelegramNotifier:
    ...
    def _valid_event(
        ...
        *,
        ...
        return bool(
            ...
            and payload.get("symbol") == DOOSAN_CODE
