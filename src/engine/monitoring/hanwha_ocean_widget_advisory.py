...
def _neutral_relative_context(
    *, current_price: int, bars: list[MinuteBar], observed_at: datetime
    ...
    Relative context is not decision-bearing in V1. Equal comparison values
    therefore provide no positive or negative authority; the Hanwha Ocean
...
def apply_hanwha_ocean_entry_policy(
    ...
    *,
    ...
    tier = "HIGH" if high_confidence_structure else "STANDARD"
    policy = {
    ...
    result.setdefault("provenance", {}).update(
        {
            ...
            "strategy_profile": contract.STRATEGY_PROFILE,
            "relative_context": "neutralized_schema_compatible_no_authority",
            "external_context": "not_used_for_hanwha_ocean_v1_promotion",
...
class HanwhaOceanWidgetCollector:
    ...
    def collect_once(self, observed_at: datetime | None = None) -> dict[str, Any]:
        ...
        advisory = evaluate_advisory(
            ...
            external_points={},
            flow={
                "status": "NOT_USED_HANWHA_OCEAN_V1",
