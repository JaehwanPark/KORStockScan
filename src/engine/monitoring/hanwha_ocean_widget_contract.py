"""Contract and KRX session boundary for the Hanwha Ocean advisory widget."""
...
HANWHA_OCEAN_CODE = "042660"
HANWHA_OCEAN_NAME = "한화오션"
SNAPSHOT_SCHEMA_VERSION = 1
DEFAULT_SNAPSHOT_PATH = Path("data/runtime/hanwha_ocean_widget_advisory_snapshot.json")
...
EXIT_EVENT_REASONS = frozenset(
    {
        "hanwha_ocean_target_1pct_reached",
        "hanwha_ocean_completed_close_below_entry_support",
    }
)
...
KIWOOM_OFFICIAL_REFERENCE = {
    ...
    "retrieved_at_kst": "2026-08-06T09:31:34+09:00",
    "inspected_paths": [
        "kiwoom_docs/종목정보.md",
        "kiwoom_docs/차트.md",
        "kiwoom/specs.py",
...
def session_context(observed_at: datetime) -> SessionContext:
    ...
    if not is_krx_trading_day(now.date()):
        return SessionContext(
            "CLOSED", "KRX", "KRX", HANWHA_OCEAN_CODE, None, None, 0, False
        )
...
def advisory_event_contract_is_valid(
    ...
    *,
    ...
    expected_prefix = f"{HANWHA_OCEAN_CODE}:{now.date().isoformat()}:{expected_type}:"
    if not (
