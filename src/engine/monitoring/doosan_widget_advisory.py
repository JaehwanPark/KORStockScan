"""Read-only Doosan Enerbility KRX intraday advisory collector.

The strategy is deliberately isolated from the trading runtime.  It reuses the
portable deterministic widget structure detector, then applies a Doosan-only
drawdown/rebound profile derived from clean-baseline replay.  It consumes only
the existing Kiwoom token cache and never issues tokens, reads accounts, or
submits orders.
"""
...
def apply_doosan_entry_policy(
    ...
    *,
    ...
    """Apply the clean-baseline Doosan first-pullback policy before promotion."""
    ...
    policy = {
        ...
        "minimum_session_drawdown_pct": contract.MIN_SESSION_DRAWDOWN_PCT,
    ...
    result["doosan_policy"] = policy
    derived["doosan_policy"] = policy
    ...
    if raw_state not in ACTIONABLE_ENTRY_STATES:
        return result
    if (
        session_return_pct is None
        or session_return_pct > contract.MIN_SESSION_DRAWDOWN_PCT
    ):
        ...
        result["unmet_conditions"] = _append_unique(
            result.get("unmet_conditions"), "doosan_session_drawdown_pending"
        )
        return result
    if volume_mode != "standard_rebound":
        ...
        result["unmet_conditions"] = _append_unique(
            result.get("unmet_conditions"),
            "doosan_standard_rebound_volume_required",
        )
        return result
...
class DoosanDailyEpisodeTracker:
    ...
    def restore(self, value: object, *, observed_at: datetime) -> bool:
        ...
        if isinstance(entry_event, dict):
            try:
                ...
                pass
        event_prefix = f"{contract.DOOSAN_CODE}:{today}:"
        if entry_issued:
            if not (
    ...
    def _capture_entry(
        ...
        *,
        ...
        event_id = (
            f"{contract.DOOSAN_CODE}:{self.scope_date}:ENTRY:{now.strftime('%H%M%S')}"
        )
        ...
        return {
            ...
            "unmet_conditions": (
                ...
                if self.active
                    ...
                    (
                        "doosan_entry_episode_not_active"
                        if not self.entry_issued
                        else "doosan_daily_entry_episode_completed"
                    )
                ]
            ),
            "observed_at": _as_kst(observed_at).isoformat(),
        ...
        *,
        ...
        if capture_attempted and not captured:
            ...
            current_advisory["unmet_conditions"] = _append_unique(
                ...
                "doosan_entry_episode_contract_invalid",
            )
        if self.active:
        ...
        if self.active and source_quality.get("status") == "PASS":
            ...
            if self.target_price and current_price >= self.target_price:
                ...
                reason = "doosan_completed_close_below_entry_support"
        ...
        if reason and self.exit_event is None:
            ...
            self.exit_event = {
                "event_id": (
                    f"{contract.DOOSAN_CODE}:{self.scope_date}:EXIT:"
        ...
        if self.entry_issued and not captured:
            ...
            current_advisory["unmet_conditions"] = _append_unique(
                current_advisory.get("unmet_conditions"),
                (
                    "doosan_entry_episode_active"
                    if self.active
                    else "doosan_daily_entry_episode_consumed"
                ),
            )
        return current_advisory, exit_advisory


class DoosanWidgetCollector:
    def __init__(
        self,
        *,
        ...
        self.episode_tracker = DoosanDailyEpisodeTracker()
        self.recorder = ObservationRecorder(
            observation_dir, file_prefix="doosan_widget_advisory"
        )
    ...
    def _restore_state(self, observed_at: datetime, context) -> None:
        ...
        if (
            ...
            or payload.get("symbol") != contract.DOOSAN_CODE
            or payload.get("token_mode") != "shared_cache_only"
            or payload.get("quote_request_code") != contract.DOOSAN_CODE
        ...
        self.episode_tracker.restore(
            payload.get("doosan_episode"), observed_at=observed_at
        )
    ...
    def _notify(self, payload: dict[str, Any], observed_at: datetime) -> None:
        ...
        try:
            ...
            print(
                "[WARN] Doosan widget Telegram notification isolated: "
                f"{type(exc).__name__}"
            )
    ...
    def collect_once(self, observed_at: datetime | None = None) -> dict[str, Any]:
        ...
        if not context.active:
            ...
            payload = {
                ...
                "status": "closed",
                "symbol": contract.DOOSAN_CODE,
                "name": contract.DOOSAN_NAME,
                "observed_at_kst": now.isoformat(),
                "market_venue": "KRX",
                "market_cohort": "KRX",
                "market_session": "closed",
                "quote_request_code": contract.DOOSAN_CODE,
                ...
                "doosan_episode": self.episode_tracker.snapshot(),
                "entry_event": None,
                "exit_event": None,
            }
        ...
        quote = client.post(
            "/api/dostk/stkinfo", "ka10001", {"stk_cd": contract.DOOSAN_CODE}
        )
        ...
        bbo_payload = client.post(
            "/api/dostk/mrkcond", "ka10004", {"stk_cd": contract.DOOSAN_CODE}
        )
        ...
        advisory = apply_doosan_entry_policy(
            advisory,
            current_price=current_price,
            bars=bars,
            context=context,
        ...
        payload = {
            ...
            "quote_request_code": contract.DOOSAN_CODE,
            "source": "doosan_widget_collector_kiwoom_krx",
            "token_mode": "shared_cache_only",
            "strategy_profile": contract.STRATEGY_PROFILE,
            "observation": {
