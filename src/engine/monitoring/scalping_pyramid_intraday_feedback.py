...
def _pyramid_blocked_record(row: dict[str, Any]) -> dict[str, Any] | None:
    ...
    return {
        ...
        "position_tag": fields.get("position_tag") or fields.get("entry_position_tag"),
        "scale_in_arm": "PYRAMID",
        "scale_in_blocker_reason": fields.get("scale_in_blocker_reason") or fields.get("blocked_reason"),
...
def _update_snapshot(item: dict[str, Any], row: dict[str, Any]) -> None:
    ...
    if profit_rate is not None:
        item["min_profit_seen"] = (
            profit_rate if item.get("min_profit_seen") is None else min(item["min_profit_seen"], profit_rate)
        )
        item["max_profit_seen"] = (
            profit_rate if item.get("max_profit_seen") is None else max(item["max_profit_seen"], profit_rate)
        )
    for key in (
        "current_ai_score",
        "buy_pressure_10t",
    ...
    if item.get("one_share_event"):
        ...
        if profit_rate is not None and profit_rate >= min_profit_pct and opportunity_profit is None:
            ...
            item["pyramid_opportunity_peak_profit"] = _safe_float(fields.get("peak_profit"))
...
def _update_submit(item: dict[str, Any], row: dict[str, Any]) -> None:
    fields = _fields(row)
    text = json.dumps({"stage": row.get("stage"), "fields": fields}, ensure_ascii=False).lower()
...
def _feedback_label(item: dict[str, Any]) -> str:
    ...
    if item.get("one_share_event") and not item.get("pyramid_opportunity_seen") and not item.get("pyramid_submit_seen"):
        return "pyramid_correctly_blocked"
    if "micro_vwap_overheated" in blocker and final_profit <= max(float(profit or 0.0), 0.5):
        return "pyramid_overheat_or_reversal_risk"
    if final_profit <= 0 or (profit is not None and final_profit <= float(profit)):
        return "pyramid_overheat_or_reversal_risk"
    if max_seen is not None and profit is not None and float(max_seen) >= float(profit) + 0.8:
...
def _aggregate_by_blocker(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for blocker, items in sorted(grouped.items()):
        ...
        recovered = sum(1 for item in items if item.get("pyramid_feedback_label") == "pyramid_would_have_helped")
        reversal = sum(
            1 for item in items if item.get("pyramid_feedback_label") == "pyramid_overheat_or_reversal_risk"
        )
        submitted_profit = sum(
            1
            for item in items
            if _boolish(item.get("actual_order_submitted")) and _safe_float(item.get("final_profit_rate"), 0.0) > 0
        )
        metrics.append(
            {
                ...
                "blocked_then_recovered_rate": recovered / sample_count if sample_count else 0.0,
...
def _one_share_summary(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    closed = [item for item in rows if item.get("pyramid_feedback_label") != "pyramid_open_unresolved"]
    opportunity_rows = [item for item in rows if bool(item.get("pyramid_opportunity_seen"))]
    costs = [_safe_float(item.get("pyramid_opportunity_cost_pct"), 0.0) for item in opportunity_rows]
    missed = [item for item in rows if item.get("pyramid_feedback_label") == "pyramid_would_have_helped"]
    return {
        ...
        "one_share_pyramid_missed_upside_rate": len(missed) / len(closed) if closed else 0.0,
        "one_share_pyramid_avg_opportunity_cost_pct": sum(costs) / len(costs) if costs else 0.0,
        "one_share_pyramid_label_counts": [
            {"pyramid_feedback_label": key, "count": value} for key, value in label_counts.most_common()
        ],
    }
...
def build_report(
    ...
    *,
    ...
    for row in iter_jsonl(pipeline_path):
        ...
        if blocked:
            ...
            if key in one_share_records:
                one_share_records[key].update({k: v for k, v in blocked.items() if v not in (None, "")})
                one_share_records[key]["pyramid_opportunity_seen"] = True
                one_share_records[key]["pyramid_opportunity_ts"] = row.get("emitted_at")
                one_share_records[key]["pyramid_opportunity_profit_rate"] = blocked.get("profit_rate")
        if _is_pyramid_submit_event(row):
            ...
            if key in one_share_records:
                one_share_records[key].update({k: v for k, v in submitted.items() if v not in (None, "")})
        ...
        if key in candidates and row.get("pipeline") == "HOLDING_PIPELINE":
            ...
            elif stage in {"stat_action_decision_snapshot", "bad_entry_refined_candidate"} or "profit_rate" in fields:
    ...
    for item in candidates.values():
        ...
        item["decision_authority"] = "source_only_pyramid_intraday_feedback_no_runtime_mutation"
        item["forbidden_uses"] = FORBIDDEN_USES
        rows.append(item)
    rows.sort(key=lambda item: (str(item.get("first_observed_ts") or ""), str(item.get("record_id") or "")))
    ...
    for item in one_share_records.values():
        ...
        item["runtime_effect"] = False
        item["allowed_runtime_apply"] = False
        item["decision_authority"] = "source_only_one_share_pyramid_opportunity_backtest_no_runtime_mutation"
        item["forbidden_uses"] = FORBIDDEN_USES
        item["pyramid_opportunity_cost_pct"] = round(_one_share_opportunity_cost(item), 4)
        one_share_rows.append(item)
    one_share_rows.sort(key=lambda item: (str(item.get("first_one_share_ts") or ""), str(item.get("record_id") or "")))
    ...
    return {
        ...
        "summary": {
            ...
            "pyramid_would_have_helped_count": label_counts.get("pyramid_would_have_helped", 0),
            "pyramid_correctly_blocked_count": label_counts.get("pyramid_correctly_blocked", 0),
            "pyramid_overheat_or_reversal_risk_count": label_counts.get("pyramid_overheat_or_reversal_risk", 0),
            "pyramid_open_unresolved_count": label_counts.get("pyramid_open_unresolved", 0),
...
def write_outputs(report: dict[str, Any], *, output_json: Path, output_md: Path) -> None:
    ...
    for item in report.get("blocker_metrics") or []:
        lines.append(
            ...
            "blocked_then_recovered_rate={blocked_then_recovered_rate:.2f}".format(**item)
        )
    lines.extend(["", "## Rows", ""])
    ...
    for item in report.get("one_share_pyramid_opportunity_rows") or []:
        lines.append(
            ...
            "final={final_profit_rate}".format(
                **{
                    **item,
                    "pyramid_opportunity_seen": bool(item.get("pyramid_opportunity_seen")),
                    "pyramid_opportunity_profit_rate": item.get("pyramid_opportunity_profit_rate"),
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Build scalping PYRAMID intraday feedback report.")
