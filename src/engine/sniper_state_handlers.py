...
SCALPING_ENTRY_BLOCKER_ROLE_REGISTRY = {
    ...
    "broker_account_order_quantity_cooldown": {
        "role": "hard_safety",
        "gate_action": "block",
        "hard_block_allowed": True,
    },
...
def _lifecycle_bucket_discovery_policy_source_date_for_guard() -> str:
    return str(os.environ.get("KORSTOCKSCAN_LIFECYCLE_BUCKET_DISCOVERY_POLICY_SOURCE_DATE") or "").strip()
...
def _load_scalp_sim_auto_policy_cache() -> dict:
    direct_enabled = _rule_bool("SCALP_SIM_AUTO_POLICY_ENABLED", False)
    direct_policy_file = _rule_str("SCALP_SIM_AUTO_POLICY_FILE", "").strip()
    direct_policy_version = _rule_str("SCALP_SIM_AUTO_POLICY_VERSION", "").strip()
    lifecycle_enabled = _rule_bool("LIFECYCLE_BUCKET_DISCOVERY_ENABLED", False)
    lifecycle_policy_file = _rule_str("LIFECYCLE_BUCKET_DISCOVERY_POLICY_FILE", "").strip()
    lifecycle_policy_version = _rule_str("LIFECYCLE_BUCKET_DISCOVERY_POLICY_VERSION", "").strip()
    use_lifecycle_handoff = not direct_enabled and lifecycle_enabled and bool(lifecycle_policy_file)
    ...
    policy_version = direct_policy_version if direct_enabled else lifecycle_policy_version
    policy_source = "scalp_sim_auto_policy" if direct_enabled else "lifecycle_bucket_discovery_catalog_handoff"
    ...
    if not isinstance(payload, dict) or schema_version not in {
        ...
        catalog_buckets = payload.get("buckets") if isinstance(payload.get("buckets"), list) else []
    ...
    if schema_version == "lifecycle_bucket_catalog_v1":
        for row in catalog_buckets:
            if not isinstance(row, dict):
                continue
            if row.get("sim_lifecycle_handoff_allowed") is not True:
                continue
            approved_rows.append(row)
            source_bucket_id = str(row.get("source_bucket_id") or "").strip()
            bucket_id = str(row.get("bucket_id") or row.get("lifecycle_flow_bucket_id") or "").strip()
            if source_bucket_id:
    ...
    if (
        status == "loaded"
        and schema_version != "lifecycle_bucket_catalog_v1"
        and not (rows_by_source_bucket_id or rows_by_bucket_id or active_seeds_by_prefix or hypotheses)
    ):
        status = "policy_invalid"
...
def _scalp_sim_bucket_policy_fields(source: dict | None) -> dict:
    ...
    base = {
        "scalp_sim_auto_policy_enabled": status == "loaded",
        "scalp_sim_auto_policy_direct_enabled": cache.get("direct_scalp_sim_auto_policy_enabled") or False,
        "scalp_sim_policy_source": cache.get("policy_source") or "scalp_sim_auto_policy",
        "lifecycle_bucket_catalog_handoff_enabled": cache.get("lifecycle_bucket_catalog_handoff_enabled") or False,
        ...
        "scalp_sim_auto_policy_schema_version": cache.get("policy_schema_version") or "-",
...
def _log_swing_probe_discard(stock, code, origin_stage: str, reason: str, **fields) -> bool:
    ...
    base_fields = {
        "probe_id": source_probe_id or discard_source_record_id,
        "probe_arm": origin_stage or "discarded_before_probe_open",
        "evidence_quality_weight": _safe_float(fields.pop("evidence_quality_weight", None), 0.25),
        "probe_origin_stage": origin_stage,
...
def _sim_lifecycle_source_contract_fields(
    *,
    risk_context_owner: str,
    risk_direction: str,
    action_namespace: str,
) -> dict:
    return {
        "source_family": "panic_lifecycle_actuator",
        "decision_family": "panic_lifecycle_actuator",
        ...
        "real_gate_allowed": False,
        "pre_submit_gate_allowed": False,
        "exclude_from_live_approval": True,
        "risk_context_owner": risk_context_owner,
...
def _scalp_sim_panic_context_fields(panic_context: dict | None) -> dict:
    ...
    fields = {
        ...
        "risk_regime_context_status": context.get("panic_context_status", "MISSING"),
        "risk_regime_level": context.get("panic_level", 0),
        "risk_regime_reason": context.get("panic_level_reason", "-"),
        "risk_regime_epoch_id": context.get("panic_epoch_id", "-"),
        "risk_regime_source_files": context.get("panic_source_files") or {},
    }
    fields.update(
        _sim_lifecycle_source_contract_fields(
            risk_context_owner="panic_sell",
            risk_direction="risk_off",
            action_namespace="panic_sell_lifecycle",
        )
    )
    return fields


def _scalp_sim_panic_stock_context_fields(stock: dict | None) -> dict:
    ...
    fields.update(
        {
            ...
            "risk_regime_context_owner": "lifecycle_decision_matrix_runtime",
            "panic_epoch_id": payload.get("panic_epoch_id"),
            "last_panic_action_level": payload.get("last_panic_action_level"),
            "last_panic_action_type": payload.get("last_panic_action_type"),
            "panic_lifecycle_action_id": payload.get("panic_lifecycle_action_id"),
            "risk_regime_epoch_id": payload.get("panic_epoch_id"),
            "risk_regime_level": payload.get("last_panic_action_level"),
        }
    )
    return fields
...
def _scalp_sim_euphoria_context_fields(euphoria_context: dict | None) -> dict:
    ...
    fields.update(
        {
            "risk_regime_context_owner": "lifecycle_decision_matrix_runtime",
            "euphoria_context_status": context.get("euphoria_context_status", "MISSING"),
            ...
            "euphoria_source_quality": context.get("euphoria_source_quality", "UNKNOWN"),
            ...
            "market_wide_panic_buy_confirmed": bool(context.get("market_wide_panic_buy_confirmed")),
            "market_breadth_risk_on_advisory": bool(context.get("market_breadth_risk_on_advisory")),
            "source_quality_blockers": context.get("source_quality_blockers") or [],
            "decision_confidence": context.get("decision_confidence", 0.0),
            "risk_regime_context_status": context.get("euphoria_context_status", "MISSING"),
...
def _resolve_scalp_sim_euphoria_context(now_ts: float | None = None) -> dict:
    if not _scalp_sim_panic_enabled():
        return {"euphoria_context_status": "DISABLED", "euphoria_risk_level": 0, "euphoria_level_reason": "disabled"}
    resolved_now = None
    if now_ts:
        try:
            resolved_now = datetime.fromtimestamp(float(now_ts))
...
def _log_scalp_sim_euphoria_context_noop(stock: dict, code: str, stage: str, euphoria_context: dict) -> None:
    ...
    if stock.get("last_scalp_sim_euphoria_context_warning_key") == warning_key:
        return
    _mutate_stock_state(stock, set_fields={"last_scalp_sim_euphoria_context_warning_key": warning_key})
...
def _scalp_sim_euphoria_action_id(stock: dict, euphoria_context: dict, level: int, action_type: str) -> str:
...
def _scalp_sim_euphoria_action_already_applied(
    ...
    if previous_level == int(level) and previous_action == action:
        return True
    if action in {"TAKE_PARTIAL_PROFIT_RUNNER", "CLEANUP_PARTIAL_PROFIT"} and previous_level == int(level):
...
def _mark_scalp_sim_euphoria_action(
    ...
    _mutate_stock_state(
        ...
        set_fields={
            ...
            "last_euphoria_partial_tp_at": now_ts
            if action in {"TAKE_PARTIAL_PROFIT_RUNNER", "CLEANUP_PARTIAL_PROFIT"}
...
def _scalp_sim_euphoria_entry_conditions(stock: dict, runtime: dict, ws_data: dict | None, curr_price: int) -> dict:
    runtime = runtime if isinstance(runtime, dict) else {}
    explicit_retest = runtime.get("euphoria_retest_confirmed", stock.get("euphoria_retest_confirmed"))
    ...
    spread_bps = _spread_bps_from_ws(ws_data or {}, curr_price) or 0.0
    spread_ok = spread_bps <= _rule_float("SCALP_SIM_EUPHORIA_MAX_STARTER_SPREAD_BPS", 80.0)
...
def _should_block_scalp_sim_entry_for_euphoria(
    *,
    ...
    euphoria_context = _resolve_scalp_sim_euphoria_context(_safe_float((runtime or {}).get("now_ts"), 0.0))
    ...
    if level <= 0:
        return False
    curr_price = _safe_int((ws_data or {}).get("curr") or stock.get("target_buy_price"), 0)
    conditions = _scalp_sim_euphoria_entry_conditions(stock, runtime or {}, ws_data or {}, curr_price)
    if level >= 3:
    ...
    _log_entry_pipeline(
        stock,
        code,
        "scalp_sim_euphoria_retest_starter_allowed" if level == 2 else "scalp_sim_euphoria_level1_starter_observed",
        **_scalp_sim_event_fields(
            threshold_family="panic_lifecycle_actuator",
            sim_parent_record_id=stock.get("id"),
            source_stage="entry",
            runtime_effect="sim_arm_allowed" if level == 2 else "sim_entry_allowed_level1_observation",
            euphoria_action_type=action,
            euphoria_action_id=action_id,
            ai_score=f"{current_ai_score:.1f}",
            original_action=str((runtime or {}).get("original_action") or "BUY_SIGNAL"),
...
def _complete_scalp_simulated_sell(
    *,
        ...
        source_fields.update(
            {
                ...
                "euphoria_lifecycle_action_id": stock.get("euphoria_lifecycle_action_id"),
    ...
    _log_holding_pipeline(
        ...
        **_scalp_sim_event_fields(
            threshold_family=source_fields.get("threshold_family", "statistical_action_weight"),
            entry_adm_candidate_id=stock.get("entry_adm_candidate_id"),
            sim_record_id=stock.get("sim_record_id"),
            sim_parent_record_id=stock.get("sim_parent_record_id"),
            sell_reason_type=sell_reason_type,
...
def complete_scalp_sim_euphoria_partial_profit(
    *,
    ...
    if _scalp_sim_euphoria_action_already_applied(stock, euphoria_context, level, action):
        return False
    ratio = max(0.0, min(1.0, _safe_float(partial_ratio, 0.0)))
    sell_qty = int(math.floor(prev_buy_qty * ratio))
    if ratio > 0 and sell_qty <= 0:
    ...
    if fill_price <= 0:
        ...
        return False
    action_id = _mark_scalp_sim_euphoria_action(stock, euphoria_context, level, action, now_ts)
    buy_price = _safe_int(stock.get("buy_price"), 0)
...
def _apply_scalp_sim_euphoria_holding_actuator(
    *,
    ...
    if str(euphoria_context.get("euphoria_context_status") or "") != "OK":
        if level > 0:
            _log_scalp_sim_euphoria_context_noop(stock, code, "holding", euphoria_context)
        return False
    if level <= 0:
        return False
    action = "NO_CHANGE"
    ...
    if level == 2 and profit_rate >= _rule_float("SCALP_SIM_EUPHORIA_L2_PARTIAL_PROFIT_MIN_PCT", 0.70):
...
def _log_swing_entry_policy_evaluated(stock, code, *, strategy, source_stage, **policy_inputs):
    ...
    _log_entry_pipeline(stock, code, "swing_entry_policy_evaluated", **policy.as_log_fields())
    return policy
...
def _log_scale_in_counterfactual_started(
    *,
    ...
    decision_profit_rate = round((curr_price - prev_price) / prev_price * 100.0, 2) if prev_price > 0 else 0.0
    proposed_notional = proposed_add_price * qty if proposed_add_price > 0 and qty > 0 else 0
    _log_holding_pipeline(
        stock,
        code,
...
def _build_preset_tp_soft_stop_decision(
    ...
    *,
    ...
    max_worsen_pct = max(0.0, _rule_float("SCALP_PRESET_TP_SOFT_STOP_MAX_WORSEN_PCT", 0.30))
    recovery_buffer_pct = max(0.0, _rule_float("SCALP_PRESET_TP_SOFT_STOP_RECOVERY_BUFFER_PCT", 0.05))
    started_at = _safe_float(stock.get("preset_tp_soft_stop_started_at"), 0.0)
    anchor_profit = _safe_float(stock.get("preset_tp_soft_stop_anchor_profit"), profit_rate)
    if started_at <= 0:
        anchor_profit = float(profit_rate or 0.0)
    elapsed_sec = max(0, int(now_ts - started_at)) if started_at > 0 else 0
    additional_worsen = max(0.0, float(anchor_profit or 0.0) - float(profit_rate or 0.0))
    ...
    if started_at > 0 and profit_rate > trigger_pct:
        return {
            **base,
            "action": "recovered",
            "reason": "preset_tp_soft_stop_recovered"
...
def _early_accel_recheck_feature_values(ws_data: dict | None) -> dict:
    source = ws_data if isinstance(ws_data, dict) else {}
    tick_accel = _safe_float(
        source.get("tick_acceleration_ratio", source.get("tick_accel", source.get("tick_acceleration_ratio_raw"))),
        0.0,
    )
...
def _early_accel_recheck_pre_ai_context_flags(stock: dict | None) -> dict:
    ...
    for key, value in context.items():
        ...
        if gate_action == "source_quality_block" or risk_state.startswith("source_quality"):
...
def _resolve_early_accel_recheck(
    ...
    *,
    ...
    base = {
        "scanner_promotion_reason": str((stock or {}).get("scanner_promotion_reason") or "scanner_runtime_watch"),
    ...
    if str(strategy or "").upper() != "SCALPING" or str(pos_tag or "").upper() != "SCANNER":
        return {"allowed": False, "skip_reason": "scope_not_real_scalping_scanner", **base}
    if str(base.get("scanner_promotion_reason") or "") not in EARLY_ACCEL_RECHECK_PROMOTION_REASONS:
        return {"allowed": False, "skip_reason": "scanner_promotion_reason_not_early_accel", **base}
    ...
    if promotion_price <= 0 or promoted_at <= 0:
        return {"allowed": False, "skip_reason": "missing_scanner_promotion_anchor", **base}
    if current_price <= 0:
    ...
    if (
        ...
        return {"allowed": False, "skip_reason": "liquidity_blocked_not_allowed", **base}
    if (
        {"blocked_strength_momentum", "blocked_vpw"} & blocked_stages
        and not _rule_bool("EARLY_ACCEL_RECHECK_ALLOW_STRENGTH_BLOCKED", True)
    ):
        return {"allowed": False, "skip_reason": "strength_blocked_not_allowed", **base}
    if _safe_float(features.get("micro_vwap_bp"), 0.0) < _rule_float("EARLY_ACCEL_RECHECK_MIN_MICRO_VWAP_BP", 0.0):
    ...
    *,
    ...
    return {
        ...
        "sim_price_status": (
            ...
            and (
                str(latency_gate.get("reason") or latency_gate.get("latency_danger_reasons") or "").find("stale") >= 0
...
def _score65_74_recovery_probe_micro_guard(probe: dict | None) -> dict:
    ...
    min_micro_vwap_bp = max(
        ...
        _rule_float("AI_SCORE65_74_RECOVERY_PROBE_EFFECTIVE_MIN_MICRO_VWAP_FLOOR_BP", 10.0),
    )
    failed = []
...
def _score65_74_recovery_probe_repeat_guard(
    ...
    *,
    ...
    try:
        ...
        pass
    cancel_until = _safe_float(stock.get("score65_74_recovery_probe_cancel_cooldown_until"), 0.0)
    ...
    curr_price = _safe_int((ws_data or {}).get("curr") or stock.get("curr_price") or stock.get("target_buy_price"), 0)
...
def _score65_74_recovery_probe_decision(
    ...
    *,
    ...
    if not _rule_bool("AI_SCORE65_74_RECOVERY_PROBE_ENABLED", False):
        return {"allowed": False, "evaluated": False, "score65_74_recovery_probe_skip_reason": "disabled"}
    if bool((ai_decision or {}).get("ai_fallback_score_50", False)):
        return {"allowed": False, "evaluated": False, "score65_74_recovery_probe_skip_reason": "fallback_score_50"}
    if str((ai_decision or {}).get("action", "WAIT") or "WAIT").upper() == "BUY":
        return {"allowed": False, "evaluated": False, "score65_74_recovery_probe_skip_reason": "already_buy"}
    ...
    if strategy == 'SCALPING':
        ...
        if entry_arm:
            ...
            if pos_tag == 'VCP_NEXT':
                ...
                if scanner_price > 0:
                    ...
                    if gap_pct >= 1.5:
                        _log_entry_pipeline(
                            ...
                            code,
                            "blocked_gap_from_scan",
                            **_build_observation_contract_fields("baseline_prior_feature"),
                            gate_action="risk_context_only",
                ...
                runtime_refresh_allowed = bool(runtime_refresh_allowed or early_accel_recheck.get("allowed"))
                ...
                if ai_engine and is_vip_target and (
                    ...
                    try:
                        ...
                        if ws_data.get('orderbook') and recent_ticks:
                            ...
                            if early_accel_recheck.get("allowed"):
                                state_fields["early_accel_recheck_count"] = _safe_int(
                            ...
                            if not smoothing_buy_guard_blocked and score65_74_probe_decision.get("allowed"):
                                action = "BUY"
                                reason = (
                                ...
                                _mutate_stock_state(
                                    ...
                                    set_fields={
                                        ...
                                        'score65_74_recovery_probe_last_buy_pressure': float(
                                            feature_probe.get("buy_pressure", 0.0) or 0.0
                                        ),
                            ...
                            elif not smoothing_buy_guard_blocked and score65_74_probe_decision.get("evaluated"):
                                _log_entry_pipeline(
                                    stock,
                                    code,
                                    ...
                                    **_score65_74_recovery_probe_block_contract_fields(),
                                    ...
                                    score65_74_recovery_probe_skip_reason=(
                                        score65_74_probe_decision.get("score65_74_recovery_probe_skip_reason") or "-"
                                    ),
                                    ai_score=f"{float(ai_score or 0.0):.1f}",
                                    ...
                                    same_symbol_recovery_probe_cooldown_until=(
                                        score65_74_probe_decision.get("same_symbol_recovery_probe_cooldown_until", "-")
                                    ),
                                    ...
                                    score65_74_recovery_probe_anchor_price=(
                                        score65_74_probe_decision.get("score65_74_recovery_probe_anchor_price", "-")
                                    ),
                                    score65_74_recovery_probe_current_price=(
                                        score65_74_probe_decision.get("score65_74_recovery_probe_current_price", "-")
                                    ),
                                    latency_state=str((ws_data or {}).get("latency_state", "") or "").strip().upper() or "-",
                                    **_build_tick_source_quality_log_fields(feature_probe),
                                )
        ...
        gap_prior_triggered = fluctuation >= max_gap
        if gap_prior_triggered:
            _log_entry_pipeline(
                stock,
                code,
                "blocked_swing_gap",
        ...
        score_vpw_pass = (score >= buy_threshold or is_shooting) and vpw_condition
        gatekeeper = {
            'allow_entry': None,
        ...
        gatekeeper_allow = None
        action_label = gatekeeper.get('action_label')
        action_key = gatekeeper.get('action_key')
        ...
        if score_vpw_pass:
            if not ai_engine:
                ...
                gatekeeper = {
                    'allow_entry': None,
                    'action_label': 'GATEKEEPER_MISSING',
                    'action_key': 'gatekeeper_missing',
                    'report': '',
                    'eval_ms': 0,
                    'cache_hit': False,
                    'cache_mode': 'missing',
            ...
            action_label = gatekeeper.get('action_label', 'UNKNOWN')
            action_key = normalize_gatekeeper_action_key(gatekeeper.get('action_key') or action_label)
            gatekeeper_allow = gatekeeper.get('allow_entry')
            if not gatekeeper_allow:
                ...
                _log_entry_pipeline(
                    ...
                    cooldown_sec=0, cooldown_policy="baseline_prior_feature",
                ...
                maybe_start_swing_intraday_probe(
                    ...
                    extra_fields={
                        ...
                        "cooldown_sec": 0,
                        "cooldown_policy": "baseline_prior_feature",
                    },
                )
        else:
            _log_entry_pipeline(
        ...
        if gap_prior_triggered:
            source_stage = "blocked_swing_gap"
        elif not score_vpw_pass:
            source_stage = "blocked_swing_score_vpw"
        elif not gatekeeper_allow:
        ...
        policy = _log_swing_entry_policy_evaluated(
            ...
            current_vpw=float(current_vpw),
            vpw_condition=bool(vpw_condition),
            v_pw_limit=float(v_pw_limit),
            gap_pct=float(fluctuation),
        ...
        if policy.submit_allowed_by_policy:
            runtime["swing_entry_policy_source_stage"] = source_stage
            runtime["swing_entry_policy_action"] = policy.policy_action
...
def _submit_watching_triggered_entry(stock, code, ws_data, admin_id, runtime):
    ...
    if not latency_gate.get('allowed') or entry_mode == 'reject':
        ...
        _log_entry_pipeline(
            ...
            effective_reason=latency_gate.get('effective_reason', latency_gate.get('reason')),
            threshold_family=latency_gate.get('threshold_family', 'latency_classifier_runtime_profile'),
            runtime_effect=(
    ...
    _log_entry_pipeline(
        ...
        latency=latency_gate.get('latency_state'), latency_state=latency_gate.get('latency_state'),
        orders=len(planned_orders), reason=latency_gate.get('reason'),
...
def _cancel_pending_entry_orders(stock, code, *, force=False):
    ...
    with ENTRY_LOCK:
        ...
        for order in open_orders:
            ...
            already_resolved = any(keyword in err_msg for keyword in ['취소가능수량', '잔고', '주문없음', '체결'])
    ...
    if stock.get('exit_mode') == 'SCALP_PRESET_TP':
        ...
        if legacy_broker_recovered:
            ...
            preset_soft_stop_fields = _preset_tp_soft_stop_common_fields(
                ...
                anchor_profit=_safe_float(preset_soft_stop_decision.get("anchor_profit"), profit_rate),
                current_profit=_safe_float(preset_soft_stop_decision.get("current_profit"), profit_rate),
                ...
                formal_live_selected=bool(preset_soft_stop_decision.get("formal_live_selected")),
            )
            if preset_soft_stop_action == "recovered":
                _log_holding_pipeline(
                    ...
                    **preset_soft_stop_fields,
                    preset_tp_soft_stop_reason=preset_soft_stop_decision.get("reason", "-"),
                )
                _clear_preset_tp_soft_stop_state(stock)
            elif preset_soft_stop_action == "defer":
                stage = str(preset_soft_stop_decision.get("reason") or "preset_tp_soft_stop_deferred")
                _log_holding_pipeline(
                    stock,
                    code,
                    stage,
...
def _evaluate_scalp_sim_euphoria_scale_in(
    *,
    ...
    if str(euphoria_context.get("euphoria_context_status") or "") != "OK":
        if level > 0:
            _log_scalp_sim_euphoria_context_noop(stock, code, "scale_in", euphoria_context)
            return None
        return {}
    if level <= 0:
        return {}
    if level >= 3 or profit_rate <= 0:
        ...
        action_id = _scalp_sim_euphoria_action_id(stock, euphoria_context, level, action_type)
        _log_holding_pipeline(
            stock,
            code,
        ...
        return None
    if level == 2:
        profit_locked = profit_rate >= _rule_float("SCALP_SIM_EUPHORIA_PROFIT_LOCKED_MIN_PCT", 0.30)
        action_type = "ALLOW_PROFIT_LOCKED_SCALE_IN" if profit_locked else "DISABLE_AGGRESSIVE_SCALE_IN"
        if not profit_locked or _scalp_sim_euphoria_action_already_applied(stock, euphoria_context, level, action_type):
            _log_holding_pipeline(
                stock,
                code,
                "scalp_sim_euphoria_scale_in_blocked",
            ...
            return None
        action_id = _mark_scalp_sim_euphoria_action(stock, euphoria_context, level, action_type, time.time())
        return {
            "should_add": True,
            "add_type": "PYRAMID",
...
def _evaluate_scalp_sim_scale_in_window_expansion(
    *,
    ...
    log_funnel("eligible", "arm_window_and_attempt_quota_pass")
    return {
        "should_add": True,
        "add_type": add_type,
...
def execute_scale_in_order(*, stock, code, ws_data, action, admin_id):
    ...
    if _is_scalp_simulated_position(stock, strategy):
        ...
        scale_in_decision_id = f"{stock.get('sim_record_id')}+{add_type}+{decision_time_ms}+{ord_no}"
        prev_qty = _safe_int(stock.get("buy_qty"), 0)
        prev_price = _safe_int(stock.get("buy_price"), curr_price)
        execution_observation_enabled = _rule_bool(
            "SCALP_SIM_SCALE_IN_EXECUTION_OBSERVATION_ENABLED", False
        ) and action.get("source") == "scalp_sim_scale_in_window_expansion"
        execution_arms = {
            str(item).strip().upper()
            for item in str(
