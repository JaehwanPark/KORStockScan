    ...
    *, target_stock, record, code: str, broker_remaining_qty: int
    ...
    if str(state.get("partial_order_kind") or "") == "nxt_rising_missed_tp1":
        filled_qty = max(
            ...
            or (
                _to_int(state.get("carried_qty"))
                + _to_int(state.get("cumulative_qty"))
            ),
        )
        fill_amount = max(
            0,
...
def sync_balance_with_db():
    ...
    try:
        ...
        log_error(
            "[SELL_RECEIPT_RECOVERY_PRUNE_FAILED] "
            f"startup cleanup deferred: {exc}"
        )
    ...
    try:
        with DB.get_session() as session:
            ...
            db_holdings = [
                ...
                or (
                    str(getattr(record, "status", "") or "").upper()
                    == "SELL_ORDERED"
            ...
            for active_record in db_holdings:
            ...
            for record in db_holdings:
                ...
                if active_count_by_code.get(code, 0) > 1:
                    ...
                    for target in ACTIVE_TARGETS:
                        if (
                            str(target.get("code", "")).strip()[:6] == code
                            and (
            ...
            for record in active_records:
                ...
                if code not in real_codes:
                    ...
                    with _with_state_lock():
                        ...
                        active_sell_receipt = (
                            _active_sell_receipt_reconciliation(receipt_target)
                        )
                    ...
                    if active_sell_receipt is not None:
                        position_qty = _to_int(
                            active_sell_receipt.get("position_qty")
                        )
