...
from src.engine.bedrock_nova_provider import MICRO_INPUT_USD_PER_1M, MICRO_OUTPUT_USD_PER_1M, micro_profile_from_env
...
def runtime_manager() -> BedrockNovaShadowManager:
    ...
    profile = micro_profile_from_env()
    signature = (
        env_bool("KORSTOCKSCAN_BEDROCK_NOVA_MICRO_SHADOW_ENABLED", False),
        profile,
        env_int("KORSTOCKSCAN_BEDROCK_NOVA_MICRO_WORKERS", 1),
        env_int("KORSTOCKSCAN_BEDROCK_NOVA_MICRO_QUEUE_MAX", 200),
        env_float("KORSTOCKSCAN_BEDROCK_NOVA_MICRO_SAMPLE_RATE", 1.0),
    )
    if _RUNTIME_MANAGER is None or _RUNTIME_SIGNATURE != signature:
        _RUNTIME_MANAGER = BedrockNovaShadowManager(
            ...
            profile=profile,
            openai_input_usd_per_1m=MICRO_INPUT_USD_PER_1M,
            openai_output_usd_per_1m=MICRO_OUTPUT_USD_PER_1M,
            workers=signature[2],
