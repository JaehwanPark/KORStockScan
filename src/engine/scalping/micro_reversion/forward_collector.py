...
FORWARD_COLLECTOR_SCHEMA = "scalp_micro_reversion_forward_collector_v1"
...
class ForwardObservationCollector:
    ...
    def close(self, *, timeout_sec: float = 10.0) -> None:
        ...
        if thread is not None:
            ...
            if thread.is_alive():
                raise TimeoutError("forward collector did not drain in time")
        ...
        if close_errors:
            raise RuntimeError(
                f"{len(close_errors)} path writer(s) failed to close"
            ) from close_errors[0]

    def refresh_manual_exclusions(self, symbols: tuple[str, ...] | None = None) -> None:
        """Refresh outside producer callbacks; the adapter performs no file I/O."""

        self._adapter.refresh_manual_exclusions(symbols)
    ...
    def observe_kiwoom_0b(
        ...
        *,
        ...
        try:
            ...
            item = str(
                (snapshot.get("last_realtime_type_item") or {}).get("0B")
                or snapshot.get("last_ws_item")
                or ""
            ).strip()
            declared_venue = (
                str(
            ...
            exchange_timestamp = _exchange_timestamp_from_0b(
                trade.get("exchange_time_raw"), received_at_ms=received_at_ms
            )
            ...
            source_sequence = self._next_source_sequence(series_key)
            ...
            result = self._adapter.observe(
                ...
                bid_depth=None,
                ask_depth=None,
                quote_age_ms=_nonnegative_float_or_none(trade.get("quote_age_ms")),
    ...
    def runtime_snapshot(self) -> ForwardCollectorSnapshot:
        ...
        with self._metrics_lock:
            ...
            return ForwardCollectorSnapshot(
                ...
                manual_control_event_leak_count=0,
            )

    def _next_source_sequence(self, series_key: tuple[str, str, str]) -> int:
        with self._state_lock:
            previous = self._source_sequences.get(series_key, self._sequence_epoch)
            sequence = previous + 1
...
def _exchange_timestamp_from_0b(
    value: object, *, received_at_ms: int
) -> datetime | None:
...
def _to_price_observation(
    ...
    return PriceObservation(
        ...
        venue=envelope.venue,
        session_bucket=envelope.session_bucket,
        source_event_id=f"kiwoom_0b_local_receive_sequence:{envelope.source_sequence}",
