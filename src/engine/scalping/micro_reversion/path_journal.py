...
class NonBlockingPathJournalWriter:
    ...
    def __init__(
        ...
        *,
        ...
        self._queue: queue.Queue[MarketPathPoint] = queue.Queue(maxsize=max_queue_size)
    ...
    def submit(self, point: MarketPathPoint) -> bool:
        with self._lock:
        ...
        while not stopping:
            try:
                ...
                if self._stop_requested.is_set():
                    stopping = True
            if isinstance(item, MarketPathPoint):
                batch.append(item)
            ...
            if batch and (
                ...
                try:
                    ...
                    if (
                        ...
                        with self._lock:
                            self._storage_self_disabled = True
                        raise OSError("partition size limit reached")
                    write_metrics = append_market_path_points(self._path, batch)
...
def append_market_path_points(
    path: Path, points: Iterable[MarketPathPoint]
) -> PathAppendMetrics:
...
def _encoded_size(points: Iterable[MarketPathPoint]) -> int:
    return sum(
        len(
            (
                json.dumps(point.as_dict(), ensure_ascii=False, sort_keys=True) + "\n"
