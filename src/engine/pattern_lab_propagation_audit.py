...
from src.engine.pattern_lab_currentness_audit import report_paths as currentness_report_paths
from src.engine.runtime_approval_summary import summary_paths as runtime_summary_paths
from src.engine.scalping_pattern_lab_automation import automation_report_paths as scalping_automation_report_paths
...
def _automation_fresh(report: dict[str, Any], target_date: str, summary_key: str) -> tuple[str, str]:
    if not report:
...
def _runtime_effect_violations(*reports: dict[str, Any]) -> list[str]:
    ...
    def walk(value: Any, path: str) -> None:
        if isinstance(value, dict):
            if value.get("runtime_effect") is True or value.get("runtime_change") is True:
...
def build_pattern_lab_propagation_audit(target_date: str) -> dict[str, Any]:
    ...
    currentness_ok = bool(currentness) and currentness.get("report_type") == "pattern_lab_currentness_audit"
    checks.append(
        _check(
            ...
            recommended_order=None if currentness_ok else _order("currentness_audit_available", "Connect pattern_lab_currentness_audit before workorder build", [currentness_path]),
        )
    )

    workorder_source = _source_dict(workorder)
    workorder_source_ok = bool(workorder) and _has_source(workorder_source, "pattern_lab_currentness_audit")
    checks.append(
        _check(
            ...
            recommended_order=None if workorder_source_ok else _order("workorder_consumes_currentness_audit", "Add currentness audit source to code improvement workorder", [workorder_path, currentness_path]),
        )
    )

    currentness_order_count = len(currentness.get("code_improvement_orders") or []) if isinstance(currentness.get("code_improvement_orders"), list) else 0
    workorder_summary = workorder.get("summary") if isinstance(workorder.get("summary"), dict) else {}
    consumes_currentness_orders = int(workorder_summary.get("pattern_lab_currentness_source_order_count") or 0) >= currentness_order_count
    checks.append(
        _check(
            "workorder_currentness_order_count",
            status="pass" if consumes_currentness_orders else "fail",
            severity="info" if consumes_currentness_orders else "source_quality_blocker",
            finding="currentness audit selected orders must be counted by code_improvement_workorder.",
            source_paths=[workorder_path, currentness_path],
            recommended_order=None if consumes_currentness_orders else _order("workorder_currentness_order_count", "Consume currentness selected orders in code improvement workorder", [workorder_path, currentness_path]),
        )
    )
    ...
    checks.append(
        _check(
            ...
            recommended_order=None if ev_currentness_ok else _order("threshold_cycle_ev_currentness_source_link", "Expose currentness audit in threshold_cycle_ev", [ev_path, currentness_path]),
        )
    )
    checks.append(
        _check(
            ...
            severity="info" if ev_propagation_ok else "post_propagation_ev_refresh_pending",
    ...
    if not runtime_summary:
        ...
        runtime_status = "pass" if _has_source(runtime_source, "pattern_lab_propagation_audit") else "fail"
        runtime_severity = "info" if runtime_status == "pass" else "source_quality_blocker"
    checks.append(
        _check(
    ...
    deepseek_quality_path = PROJECT_ROOT / "analysis" / "deepseek_swing_pattern_lab" / "outputs" / "data_quality_report.json"
    ...
    checks.append(
        _check(
            ...
            recommended_order=None if sim_probe_ok else _order("deepseek_sim_probe_provenance_propagated", "Propagate DeepSeek sim/probe provenance", [swing_path]),
        )
    )

    runtime_violations = _runtime_effect_violations(currentness, scalping, swing, workorder)
    checks.append(
        _check(
            ...
            + (f"violations={runtime_violations[:10]}" if runtime_violations else "No violations."),
            source_paths=[currentness_path, scalping_path, swing_path, workorder_path],
            recommended_order=None if not runtime_violations else _order("runtime_effect_false_contract", "Reject runtime_effect=true pattern lab propagation", [currentness_path, scalping_path, swing_path, workorder_path]),
        )
    )
    ...
    report = {
        ...
        "summary": {
            ...
            "workorder_currentness_source_order_count": int(workorder_summary.get("pattern_lab_currentness_source_order_count") or 0),
        },
        "checks": checks,
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"pattern_lab_propagation_audit status={report['status']} json={json_path} md={md_path}")
    return 0
