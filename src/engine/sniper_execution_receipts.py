...
def _broker_execution_context(
    ...
    if raw_time.isdigit() and len(raw_time) == 6:
        try:
            ...
            candidate = datetime.combine(received_at.date(), parsed_time)
...
def _resolve_sell_execution_receipt(
    ...
    *,
    ...
    carried_economics_complete = bool(
        state.get("carried_economics_complete", True)
    )
    ...
    carried_unit_fill_consistent = bool(
        state.get("carried_unit_fill_consistent", True)
    )
    ...
    if isinstance(prior_order, dict):
        prior_expected_qty = max(
            0, _safe_int(prior_order.get("expected_qty"), 0)
        )
        ...
        prior_remaining = max(
            0, _safe_int(prior_order.get("remaining_qty"), 0)
        )
        ...
        updated_prior.update(
            {
                ...
                "cumulative_qty": max(
                    0, _safe_int(delayed.get("cumulative_qty"), 0)
                ),
        ...
        state.update(
            {
                ...
                "carried_economics_complete": all(
                    ...
                    and item.get("economics_complete") is True
                    for item in prior_orders.values()
                ),
                ...
                "carried_unit_fill_consistent": all(
                    isinstance(item, dict)
                    and item.get("unit_fill_consistent") is True
                    for item in prior_orders.values()
                ),
        ...
        return {
            ...
            "incremental_net_pnl_krw": round(
                aggregate_net_pnl - previous_net_pnl, 4
            ),
    ...
    return {
        ...
        "reason": "sell_receipt_full_position_reconciled" if final else "sell_receipt_partial_fill",
        "final": final,
        "order_no": normalized_order_no,
        "execution_no": str(execution_no or "").strip(),
        "expected_qty": position_qty,
...
def _log_standard_sell_partial_execution(
    ...
    *,
    ...
    persist_sell_receipt_recovery(target_stock)
...
def prune_sell_receipt_recovery_files(
    *,
    ...
    protected_target_ids = {
        int(value)
        for value in (active_target_ids or set())
...
def load_sell_receipt_recovery(
    *,
    ...
    try:
        ...
        if (
            max(0, _safe_int(payload.get("position_qty"), 0))
            != expected_position_qty
            or max(0, _safe_int(state.get("position_qty"), 0))
            != expected_position_qty
        ):
...
def clear_sell_receipt_recovery(target_id: Any) -> bool:
    ...
    try:
        ...
        log_error(
            f"[SELL_RECEIPT_RECOVERY_CLEAR_FAILED] id={target_id}: {exc}"
        )
        return False
...
def _receipt_nested_map(
    target_stock: dict[str, Any], key: str
) -> dict[str, Any]:
    ...
    *,
    ...
    for ledger_key in ledger_keys:
        ...
        known.update(
            ...
            and str(raw_key).strip() != missing_order_key
        )
    return known
    ...
    *,
    ...
    exact_pending_order = next(
        (
            pending_order
            for pending_order in pending_orders
            if str(pending_order.get("ord_no", "") or "").strip()
            == normalized_order_no
        ),
        None,
    )
    terminal_order = get_terminal_entry_order(normalized_order_no)
    terminal_code = str((terminal_order or {}).get("stock_code", "") or "").strip()[
        :6
    ]
    terminal_target_id = str(
        (terminal_order or {}).get("target_id", "") or ""
    ).strip()
    ...
    blank_candidates = [
        ...
        and (
            ...
            or max(0, int(pending_order.get("qty", 0) or 0))
            == official_order_qty
        )
    ]
...
def _resolve_cumulative_buy_order_receipt(
    *,
    ...
    final = bool(
        expected_qty > 0
        and raw_qty == expected_qty
        and official_remaining_qty == 0
    )
    return {
        "status": "final" if final else "partial",
        "reason": (
            "buy_receipt_full_order_reconciled"
            if final
            else "buy_receipt_partial_fill"
        ),
        "final": final,
        "requested_qty": expected_qty,
        "remaining_qty": (
...
def _resolve_fast_sell_execution_receipt(
    ...
    *,
    ...
    next_ledgers[normalized_order_no] = {
        ...
        "cumulative_amount": max(
            0, _safe_int(receipt.get("cumulative_amount"), 0)
        ),
...
def _resolve_entry_effective_fill_qty(
    *,
    ...
    if receipt.get("status") != "duplicate":
        ...
        bundle_filled_qty = max(
            0, _safe_int(target_stock.get("entry_filled_qty"), 0)
        )
        if (
...
def _finalize_standard_sell_execution(
    *,
    ...
    sell_receipt_snapshot.update(
        {
            ...
            "sell_execution_cumulative_amount": int(
                sell_receipt["cumulative_amount"]
            ),
            ...
            "sell_execution_final_leg_price": float(
                sell_receipt["incremental_price"]
            ),
    ...
    if not persist_sell_receipt_recovery(target_stock):
        log_error(
    ...
    try:
        with DB.get_session() as session:
            ...
            if position_buy_qty <= 0 or completed_sell_qty != position_buy_qty:
                ...
                return False
            completed_sell_amount = int(
                sell_receipt.get("cumulative_amount") or 0
            )
            position_weighted_sell_price = int(
                round(completed_sell_amount / completed_sell_qty)
            )
            realized_pnl_krw = float(
                sell_receipt.get("cumulative_net_pnl_krw") or 0.0
            )
            profit_rate = (
    ...
    if exec_type == "SELL" and normalized_order_no:
        ...
        if len(submit_pending_candidates) == 1:
            ...
            requested_qty = _safe_int(
                candidate.get("sell_submit_requested_qty"), 0
            )
            official_qty = _safe_int(order_qty, 0)
            official_remaining = (
                _safe_int(remaining_qty, -1)
            ...
            update_fields: dict[str, Any] = {
                ...
                "avg_down_count": int(
                    receipt_snapshot.get(
                        "avg_down_count", record.avg_down_count or 0
                    )
                    or 0
                ),
                "pyramid_count": int(
                    receipt_snapshot.get(
                        "pyramid_count", record.pyramid_count or 0
                    )
                    or 0
                ),
...
def _update_db_for_sell(
    ...
    try:
        with DB.get_session() as session:
            ...
            if (
                position_runner_qty <= 0
                or completed_runner_qty != position_runner_qty
            ):
                log_error(
...
def _handle_add_buy_execution(
    *,
    ...
    reconciled_before_ordno_bind = bool(
        add_receipt.get("reconciled_before_ordno_bind")
    )
    ...
    pre_add_qty = _safe_int(
        target_stock.get("pending_add_initial_buy_qty"), old_qty
    )
    ...
    _log_holding_pipeline(
        ...
        receipt_economics_complete=bool(
            add_receipt.get("economics_complete")
        ),
        ...
        receipt_unit_qty_matches_delta=add_receipt.get(
            "unit_qty_matches_delta"
        ),
        **_broker_execution_provenance_fields(target_stock),
        **split_leg_fields,
...
def _handle_entry_buy_execution(
    *,
    ...
    _log_holding_pipeline(
        ...
        receipt_unit_qty_matches_delta=entry_receipt.get(
            "unit_qty_matches_delta"
        ),
        cum_filled_qty=int(cum_filled_qty or 0),
...
def handle_real_execution(exec_data):
    ...
    cumulative_exec_amount = _optional_abs_int(
        exec_data.get("cumulative_exec_amount")
    )
        ...
        with state["lock"]:
            ...
            if exec_type == "BUY":
                if order_no and order_no == str(state.get("buy_ord_no", "")):
                    ...
                    buy_executions = (
                        dict(buy_executions)
                        if isinstance(buy_executions, dict)
                        else {}
                    )
                    ...
                    fast_buy_receipt = (
                        ...
                        if execution_conflict
                            ...
                            raw_cumulative_qty=exec_qty,
                            requested_qty=max(
                                0, int(state.get("req_buy_qty", 0) or 0)
                            ),
                    ...
                    if fast_buy_receipt.get("status") == "invalid":
                        state["buy_receipt_source_gap"] = str(
                            fast_buy_receipt.get("reason")
                            or "fast_buy_receipt_invalid"
                        )
                        log_error(
    ...
    with _active_state_lock():
        ...
        if exec_type == "BUY":
            ...
            if not sell_context:
                return
            record, safe_buy_price, profit_rate, strategy, is_scalp_revive = sell_context
