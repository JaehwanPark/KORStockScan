...
def runtime_manifest_path(target_date: str) -> Path:
    if target_date >= RUNTIME_BOOTSTRAP_CUTOVER_DATE:
        return RUNTIME_BOOTSTRAP_DIR / f"runtime_policy_bootstrap_{target_date}.json"
    return RUNTIME_ENV_DIR / f"threshold_runtime_env_{target_date}.json"


def runtime_env_path(target_date: str) -> Path:
    if target_date >= RUNTIME_BOOTSTRAP_CUTOVER_DATE:
        return RUNTIME_BOOTSTRAP_DIR / f"runtime_policy_bootstrap_{target_date}.env"
    return RUNTIME_ENV_DIR / f"threshold_runtime_env_{target_date}.env"


def runtime_verify_path(target_date: str) -> Path:
    if target_date >= RUNTIME_BOOTSTRAP_CUTOVER_DATE:
        return RUNTIME_BOOTSTRAP_DIR / f"runtime_policy_bootstrap_verify_{target_date}.json"
...
def _env_file_bytes(
    ...
    lines = [
        "# Generated by ai_multi_timeframe_context_promotion.py",
...
def main(argv: list[str] | None = None) -> int:
    ...
    runtime_verify_file = (
        ...
        else runtime_verify_path(args.date)
    )
