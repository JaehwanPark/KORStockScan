"""Official-contract Kiwoom SOR gateway for the afternoon one-share machine."""
...
from src.trading.samsung_afternoon_one_share.policy import MinuteBar
...
class KiwoomAfternoonOneShareGateway:
    ...
    def _require_write_authority(self) -> None:
        if not self.order_authority:
            raise PermissionError("afternoon_one_share_order_authority_disabled")
        if self.base_url != "https://api.kiwoom.com":
            raise PermissionError(
                "afternoon_one_share_orders_require_production_endpoint"
            )
    ...
    def completed_sor_minute_bars(
        ...
        for row in rows:
            ...
            if (
                ...
                or not (
                    datetime.strptime("0900", "%H%M").time()
                    <= timestamp.time()
                    < datetime.strptime("1530", "%H%M").time()
                )
    ...
    def entry_execution_velocity_snapshot(
        ...
        try:
            ...
            payload = kiwoom_utils.get_tick_history_ka10003(
                ...
                request_owner="samsung_afternoon_one_share_entry_velocity",
                request_class="execution_critical",
            )
        except Exception as exc:
    ...
    def execution_snapshot(
        ...
        for _ in range(3):
            ...
            pages.append(body)
            cont_yn, next_key = (
                str(response.headers.get("cont-yn", "N") or "N").upper(),
                str(response.headers.get("next-key", "") or "").strip(),
            )
        ...
        order_qty, filled_qty = _positive_int(row.get("ord_qty")), _positive_int(
            row.get("cntr_qty")
        )
