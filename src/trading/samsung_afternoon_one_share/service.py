"""CLI for the independently scheduled Samsung afternoon two-leg machine."""
...
ENABLE_ENV = "KORSTOCKSCAN_SAMSUNG_AFTERNOON_ONE_SHARE_ENABLED"
LIVE_CONFIRMATION = "005930_AFTERNOON_TWO_LEG_LIVE"
...
def main(argv: list[str] | None = None) -> int:
    ...
    if live_enabled:
        ...
        applied, applied_hash, applied_reason = load_applied_machine_policy(
            "afternoon", target_date=target_date
        )
        if applied is None:
    ...
    state_path = args.state_path or (
        ...
        if live_enabled
            ...
            "samsung_afternoon_one_share_dry_run_state.json"
        )
    )
    ...
    if lock_handle is None:
        return 3
    gateway = KiwoomAfternoonOneShareGateway(order_authority=live_enabled)
