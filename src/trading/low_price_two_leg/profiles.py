...
SAMSUNG_EA_AFTERNOON_20260907_WINDOW = (time(14, 20), time(14, 34))
...
PROFILE_REVISION_20260911_EFFECTIVE_DATE = date(2026, 9, 11)
...
# The 2026-09-04 postclose recommendations become a separate exact-date
# generation on 2026-09-07. Prior-date states retain their original profile
# snapshot, so neither held custody nor an existing target order is rewritten.
PROFILES_20260907_PRIOR = dict(PROFILES)
_REVISION_20260907_SOURCE = (
    "clean_baseline_48d_calibration_16d_holdout_user_approved_v9"
)


def _revise_20260907(
    profile_id: str,
    *,
    window: tuple[time, time] | None = None,
    lookback_bars: int,
    ...
    prior = PROFILES_20260907_PRIOR[profile_id]
    selected_window = window or (prior.policy.scan_start, prior.policy.scan_last_bar)
    return replace(
        prior,
        policy=replace(
...
PROFILES = dict(PROFILES_20260907_PRIOR)
PROFILES.update(
    {
        "sd_biosensor_midday": _revise_20260907(
            "sd_biosensor_midday",
            lookback_bars=20,
            drawdown_pct=0.75,
            near_low_pct=0.20,
        ...
        "hanwha_ocean_late_morning": _revise_20260907(
            "hanwha_ocean_late_morning",
            lookback_bars=45,
            drawdown_pct=0.75,
            near_low_pct=0.75,
        ...
        "samsung_ea_afternoon": _revise_20260907(
            "samsung_ea_afternoon",
            window=SAMSUNG_EA_AFTERNOON_20260907_WINDOW,
            lookback_bars=20,
            drawdown_pct=0.75,
            near_low_pct=0.35,
            target_ticks=4,
...
PROFILES.update(
    {
        ...
        for profile in (
            _profile(
                ...
                "삼성중공업",
                "late_morning",
                window=SAMSUNG_HEAVY_LATE_MORNING_20260907_WINDOW,
                lookback_bars=30,
                drawdown_pct=0.75,
                near_low_pct=0.35,
                target_ticks=2,
            ...
            _profile(
                ...
                "팬오션",
                "afternoon",
                window=FAN_OCEAN_AFTERNOON_20260907_WINDOW,
                lookback_bars=30,
                drawdown_pct=0.75,
                near_low_pct=0.20,
                target_ticks=2,
...
# Explicit 9/11 approval; preserve earlier dates and existing custody.
PROFILES_20260911_PRIOR = dict(PROFILES)
PROFILES = dict(PROFILES_20260911_PRIOR)
PROFILES["lotte_chemical_afternoon"] = _profile(
    "lotte_chemical_afternoon",
    "011170",
    "롯데케미칼",
    "afternoon",
    window=(time(14, 25), time(14, 34)),
    lookback_bars=15,
    drawdown_pct=0.5,
    near_low_pct=0.05,
    entry_offsets_ticks=(-1, -2),
    entry_valid_completed_bars=5,
    target_ticks=4,
    runtime_policy_source="user_directed_profile_revision_20260911",
)
PROFILES["lotte_chemical_midday"] = _profile(
    "lotte_chemical_midday",
    "011170",
    "롯데케미칼",
    "midday",
    window=(time(13, 15), time(13, 34)),
    lookback_bars=45,
    drawdown_pct=0.75,
    near_low_pct=0.75,
    entry_offsets_ticks=(0, -1),
...
PROFILES["lx_semicon_morning"] = _profile(
    "lx_semicon_morning",
    "108320",
    "LX세미콘",
    "morning",
    window=(time(9, 45), time(9, 54)),
    lookback_bars=15,
    drawdown_pct=1.25,
    near_low_pct=0.75,
    entry_offsets_ticks=(0, -1),
...
def profiles_for_target_date(target_date: date) -> dict[str, MachineProfile]:
    ...
    if target_date < PROFILE_REVISION_20260911_EFFECTIVE_DATE:
        return PROFILES_20260911_PRIOR
    return PROFILES
