...
class KiwoomLowPriceTwoLegGateway:
    ...
    def _post(
        ...
        *,
        ...
        if not fresh and api_id in {KA10075_API_ID, KT00007_API_ID}:
            kwargs.update(
                {
                    "cache": (
        ...
        return post_kiwoom_episode_read(
            ...
            request_owner="low_price_two_leg_episode_read",
            **kwargs,
        )
    ...
    def completed_sor_minute_bars(
        ...
        try:
            selected = selected_completed_bar_payload(f"{self.symbol}_AL", now=now, consumer="episode", seed_fetch=seed_fetch,
        ...
        snapshot = MinuteBarsSnapshot(True, bars, source_receipt=(selected or annotate_completed_bar_rest(body, f"{self.symbol}_AL", selection_receipt)).get("_completed_bar_source"))
    ...
    def entry_liquidity_snapshot(self, *, route: str = "SOR") -> EntryLiquiditySnapshot:
        selected = selected_entry_snapshot(symbol=self.symbol, route=route, kind="0D")
        if selected is not None:
            return selected
        try:
            ...
            payload = kiwoom_utils.get_stock_orderbook_ka10004(
                self._token(),
                request_code,
                request_owner="low_price_two_leg_entry_liquidity",
    ...
    def entry_execution_velocity_snapshot(
        ...
        selected = selected_entry_snapshot(symbol=self.symbol, route=route, kind="0B")
        if selected is not None:
            return selected
        try:
            request_code = entry_liquidity_request_code(self.symbol, route)
