"""Official-contract Kiwoom SOR gateway for the midday one-share machine."""
...
from src.trading.samsung_midday_one_share.policy import MinuteBar
...
class KiwoomMiddayOneShareGateway:
    ...
    def _require_write_authority(self) -> None:
        if not self.order_authority:
            raise PermissionError("midday_one_share_order_authority_disabled")
        if self.base_url != "https://api.kiwoom.com":
            raise PermissionError("midday_one_share_orders_require_production_endpoint")
    ...
    def entry_liquidity_snapshot(self, *, route: str = "SOR") -> EntryLiquiditySnapshot:
        try:
            ...
            payload = kiwoom_utils.get_stock_orderbook_ka10004(
                self._token(),
                request_code,
                request_owner="samsung_midday_entry_liquidity",
