"""Persistent state machine for one Samsung midday one-share episode."""
...
class SamsungMiddayOneShareMachine:
    """Owns exact orders; morning, afternoon, and widget positions are external."""

    def __init__(
        self,
        *,
        gateway: MiddayGateway,
    ...
    def _load_state(self) -> dict:
        ...
        if (
            ...
            or payload.get("schema") != "samsung_midday_one_share_state_v1"
        ):
            return {
                "schema": "samsung_midday_one_share_state_v1",
                "trade_date": "",
    ...
    def _submit_target(self, now: datetime) -> dict:
        ...
        if not self._owns_order(str(self._state.get("buy_order_no") or "")):
            return self._block(now, "filled_buy_not_owned_by_midday_machine")
    ...
    def _reconcile_target(self, now: datetime) -> dict:
        ...
        if not self._owns_order(order_no):
            return self._block(now, "target_order_not_owned_by_midday_machine")
    ...
    def _reconcile_buy(self, now: datetime) -> dict:
        ...
        if not self._owns_order(order_no):
            return self._block(now, "buy_order_not_owned_by_midday_machine")
        snapshot = self._execution(
    ...
    def _consider_entry(self, now: datetime) -> dict:
        ...
        if latest.timestamp.time() > self.policy.scan_last_bar:
            ...
            self._record(now, "midday_scan_window_closed")
            return self.snapshot()
        if latest.timestamp.time() < self.policy.scan_start:
            self._state.update(
                {
                    "last_action": "waiting_for_midday_scan_window",
                    "blocked_reason": "",
                }
            )
            self._save()
