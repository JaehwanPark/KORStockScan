...
def test_profile_revision_is_exact_date_preopen_transition(tmp_path):
    ...
    assert set(monday_0831_generation["profiles"]) == set(PROFILES_20260907_PRIOR)
...
def test_all_thirteen_20260904_recommendations_bind_exact_next_profiles():
    evidence_path = (
        Path(__file__).resolve().parents[2]
...
def test_exact_execution_snapshot_rebinds_episode_owner_after_submit_bind_failure(
    ...
    assert owner["position_id"] == (
        f"episode:{profile.profile_id}:{profile.symbol}:{machine._state['trade_date']}"
    )
...
def test_episode_velocity_wait_cannot_renew_original_liquidity_clock(
    tmp_path, monkeypatch
):
    ...
    machine = LowPriceTwoLegMachine(
        profile=profile,
        gateway=gateway,
        state_path=tmp_path / "delayed-prints.json",
        live_enabled=True,
        ownership_source=lambda code: "manual_operator",
    )
...
def test_terminal_partial_fill_does_not_report_whole_episode_as_unfilled(tmp_path):
    ...
    assert (
        state["legs"][0]["adaptive_exit_first_fill_observation"]["first_observed_at"]
        == (started_at + timedelta(minutes=3)).isoformat()
    )
    target_order = state["legs"][0]["target_order_no"]
    target_price = state["legs"][0]["target_price"]
    receipt = next(iter(state["legs"][0]["adaptive_exit_target_observations"].values()))
    assert receipt["scope"]["profile"] == profile.profile_id
    assert receipt["target"]["order_no"] == target_order
    assert receipt["target"]["quantity"] == 4
    assert receipt["entries"][0]["requested_quantity"] == 10
