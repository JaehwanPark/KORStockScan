...
def test_collector_uses_cached_token_and_auxiliary_read_only_market_requests(
    ...
    if source_mode == "ws":
        ...
        def read_quote(context, **kwargs):
            ...
            return {**_adoptable_widget_transport(now, "042660", 98500, 98000),
                    "evaluated_at_epoch": now.timestamp()}
        monkeypatch.setattr(hanwha_ocean, "read_shared_widget_quote", read_quote)
    ...
    if clock_mode != "explicit":
        ...
        monkeypatch.setattr(hanwha_ocean, "_now_kst", lambda: next(clocks))
    ...
    if source_mode == "ws":
        assert not any(api in {"ka10001", "ka10003", "ka10004"} and body.get("stk_cd") == "042660"
