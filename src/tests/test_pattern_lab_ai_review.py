...
def test_pattern_lab_ai_review_resolves_code_improvement_workorder_self_review_gap(tmp_path, monkeypatch):
    report_dir = tmp_path / "data" / "report"
    monkeypatch.setattr(mod, "REPORT_DIR", report_dir)
    _write_json(
        report_dir / "pattern_lab_currentness_audit" / "pattern_lab_currentness_audit_2026-05-15.json",
...
def test_pattern_lab_ai_review_resolves_current_pattern_lab_self_pending_workorders(tmp_path, monkeypatch):
    report_dir = tmp_path / "data" / "report"
    monkeypatch.setattr(mod, "REPORT_DIR", report_dir)
    _write_json(
        report_dir / "pattern_lab_currentness_audit" / "pattern_lab_currentness_audit_2026-05-15.json",
    ...
    _write_json(
        report_dir / "code_improvement_workorder" / "code_improvement_workorder_2026-05-15.json",
        {
            "status": "pass",
            "runtime_effect": False,
            "allowed_runtime_apply": False,
...
def test_pattern_lab_ai_review_resolves_code_improvement_workorder_ai_review_gap(tmp_path, monkeypatch):
    report_dir = tmp_path / "data" / "report"
    monkeypatch.setattr(mod, "REPORT_DIR", report_dir)
    _write_json(
        report_dir / "pattern_lab_currentness_audit" / "pattern_lab_currentness_audit_2026-05-15.json",
        {
            "status": "pass",
            "summary": {"consumed_feedback_source_count": 6, "missing_feedback_source_count": 0},
            "checks": [
                {
                    "check_id": "pattern_lab_ai_review_contract",
                    "status": "pass",
...
def test_pattern_lab_ai_review_resolves_classified_source_quality_warning_gaps(tmp_path, monkeypatch):
    report_dir = tmp_path / "data" / "report"
    monkeypatch.setattr(mod, "REPORT_DIR", report_dir)
    _write_json(
        report_dir / "pattern_lab_currentness_audit" / "pattern_lab_currentness_audit_2026-05-15.json",
    ...
    _write_json(
        report_dir / "swing_strategy_discovery_ev" / "swing_strategy_discovery_ev_2026-05-15.json",
        {
            "runtime_effect": False,
            "allowed_runtime_apply": False,
...
def test_pattern_lab_ai_review_resolves_generic_source_report_warning_ids(tmp_path, monkeypatch):
    report_dir = tmp_path / "data" / "report"
    monkeypatch.setattr(mod, "REPORT_DIR", report_dir)
    _write_json(
        report_dir / "pattern_lab_currentness_audit" / "pattern_lab_currentness_audit_2026-05-15.json",
...
def test_pattern_lab_ai_review_resolves_swing_ai_review_missing_when_no_sim_auto_candidates(tmp_path, monkeypatch):
    ...
    conclusion = report["ai_two_pass_review"]["final_conclusions"][0]
    assert conclusion["final_state"] == "source_only_keep_collecting"
    assert conclusion["source_context_resolution"]["status"] == "resolved_by_source_only_sim_auto_review_contract"
...
def test_pattern_lab_ai_review_resolves_current_source_only_warning_set(tmp_path, monkeypatch):
    report_dir = tmp_path / "data" / "report"
    monkeypatch.setattr(mod, "REPORT_DIR", report_dir)
    _write_json(
        report_dir / "pattern_lab_currentness_audit" / "pattern_lab_currentness_audit_2026-05-15.json",
    ...
    _write_json(
        report_dir / "lifecycle_decision_matrix" / "lifecycle_decision_matrix_2026-05-15.json",
        {"status": "pass", "runtime_effect": False, "allowed_runtime_apply": False},
    )
    _write_json(
        report_dir / "lifecycle_bucket_discovery" / "lifecycle_bucket_discovery_2026-05-15.json",
        {
            ...
            "allowed_runtime_apply": False,
            "warnings": ["source_contract_drift_warning"],
            "summary": {"source_contract_status": "warning", "status": "pass", "code_patch_required_count": 0},
        },
    )
    _write_json(
        report_dir / "swing_strategy_discovery_ev" / "swing_strategy_discovery_ev_2026-05-15.json",
    ...
    _write_json(
        report_dir / "swing_lifecycle_decision_matrix" / "swing_lifecycle_decision_matrix_2026-05-15.json",
        {
            "runtime_effect": False,
            "allowed_runtime_apply": False,
            "warnings": [
    ...
    _write_json(
        report_dir / "code_improvement_workorder" / "code_improvement_workorder_2026-05-15.json",
        {"status": "pass", "runtime_effect": False, "allowed_runtime_apply": False, "orders": []},
    )
    raw_response = {
        "schema_version": 1,
        "interpretation": {"review_items": [], "source_feedback_status": "warning"},
    ...
    assert report["summary"]["audit_status"] == "pass"
    assert report["code_improvement_orders"] == []
    assert {item["final_state"] for item in report["ai_two_pass_review"]["final_conclusions"]} == {
        "source_only_keep_collecting"
    }


def test_pattern_lab_ai_review_marks_same_run_late_bound_sources_as_source_only(tmp_path, monkeypatch):
    report_dir = tmp_path / "data" / "report"
    monkeypatch.setattr(mod, "REPORT_DIR", report_dir)
    _write_json(
        report_dir / "pattern_lab_currentness_audit" / "pattern_lab_currentness_audit_2026-05-15.json",
    ...
    assert report["summary"]["audit_status"] == "pass"
    assert report["code_improvement_orders"] == []
    assert set(report["feedback_handoff_summary"]["missing_late_bound_source_labels"]) == {
        "threshold_cycle_ev",
        "code_improvement_workorder",
    }
    assert {item["final_state"] for item in report["ai_two_pass_review"]["final_conclusions"]} == {
...
def test_pattern_lab_ai_review_normalizes_keep_decision_gap_state(tmp_path, monkeypatch):
    report_dir = tmp_path / "data" / "report"
    monkeypatch.setattr(mod, "REPORT_DIR", report_dir)
    _write_json(
        report_dir / "pattern_lab_currentness_audit" / "pattern_lab_currentness_audit_2026-05-15.json",
        {"status": "pass", "summary": {"consumed_feedback_source_count": 0, "missing_feedback_source_count": 0}},
    )
    raw_response = {
        "schema_version": 1,
...
def test_pattern_lab_ai_review_resolves_generic_instrumentation_gap_with_workorder_ledger(
    ...
    _write_json(
        ...
        {
            "status": "pass",
            "summary": {"consumed_feedback_source_count": 8, "missing_feedback_source_count": 0},
        },
    )
    for name in (
        "scalping_pattern_lab_automation",
    ...
    raw_response = {
        "schema_version": 1,
        "interpretation": {"review_items": [], "source_feedback_status": "warning"},
        "audit": {"status": "correction_required", "issues": ["code_patch_required:1"], "forbidden_use_violations": []},
        "final_conclusions": [
            {
...
def test_pattern_lab_ai_review_marks_swing_partial_ai_gap_as_implemented_source_only(tmp_path, monkeypatch):
    report_dir = tmp_path / "data" / "report"
    monkeypatch.setattr(mod, "REPORT_DIR", report_dir)
    ...
    order = next(
        ...
        if item["order_id"] == "order_pattern_lab_ai_review_swing_lifecycle_bucket_discovery_ai_two_pass_partial"
    )
...
def test_pattern_lab_ai_review_marks_swing_ai_two_pass_incomplete_as_implemented_source_only(tmp_path, monkeypatch):
    report_dir = tmp_path / "data" / "report"
    monkeypatch.setattr(mod, "REPORT_DIR", report_dir)
    ...
    order = next(
        item
        for item in report["code_improvement_orders"]
        if item["order_id"] == "order_pattern_lab_ai_review_swing_ai_two_pass_review_incomplete"
    )
    assert order["implementation_status"] == "implemented"
    assert order["implementation_provenance"]["implementation_type"] == (
