...
def test_durable_writer_to_independent_process_reader(tmp_path, monkeypatch):
    import subprocess
    import sys
    from src.engine.scalping.micro_reversion import completed_bars
    now = BASE + timedelta(hours=1)
    monkeypatch.setattr(completed_bars.time, "time", lambda: now.timestamp())
    p=CompletedBarProjection(tmp_path)
