...
def test_currentness_audit_passes_when_contracts_and_guards_exist(
    ...
    assert "scalping" not in report["feedback_sources"]
    assert report["feedback_sources"]["swing"]["available_feedback_sources"]
...
def test_currentness_audit_excludes_disabled_swing_sources(tmp_path, monkeypatch):
    ...
    assert not (report_dir / "pattern_lab_currentness_audit").exists()
...
def test_currentness_audit_emits_runtime_false_orders_for_gaps(tmp_path, monkeypatch):
    ...
    (
        project_root / "analysis" / "deepseek_swing_pattern_lab" / "README.md"
    ).write_text(
