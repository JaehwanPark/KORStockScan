...
def test_pyramid_quality_calibration_holds_when_sample_floor_missing(tmp_path, monkeypatch):
...
def test_pyramid_quality_calibration_reversal_cluster_tightens_candidate(tmp_path, monkeypatch):
    monkeypatch.setattr(mod, "INPUT_REPORT_DIR", tmp_path / "input")
    mod.INPUT_REPORT_DIR.mkdir(parents=True)
    ...
    path = _feedback(mod.INPUT_REPORT_DIR / "scalping_pyramid_intraday_feedback_2026-07-03.json", rows)
    ...
    assert candidate["recommended_values"]["min_ai_score"] == candidate["current_values"]["min_ai_score"] + 5.0
...
def test_pyramid_quality_calibration_recovery_cluster_loosens_candidate(tmp_path, monkeypatch):
    monkeypatch.setattr(mod, "INPUT_REPORT_DIR", tmp_path / "input")
    mod.INPUT_REPORT_DIR.mkdir(parents=True)
    ...
    assert candidate["allowed_runtime_apply"] is True
    assert candidate["recommended_values"]["min_ai_score"] == candidate["current_values"]["min_ai_score"] - 5.0
    assert candidate["recommended_values"]["max_spread_bps"] == candidate["current_values"]["max_spread_bps"] + 10.0
...
def test_pyramid_quality_calibration_uses_all_one_share_rows_for_thresholds(tmp_path, monkeypatch):
    ...
    for index, row in enumerate(one_share_rows):
        ...
        row["decision_authority"] = "source_only_one_share_pyramid_opportunity_backtest_no_runtime_mutation"
    path = _feedback(
        mod.INPUT_REPORT_DIR / "scalping_pyramid_intraday_feedback_2026-07-03.json",
        legacy_rows,
    ...
    assert candidate["source_metrics"]["calibration_source_scope"] == "one_share_event_opportunity"
...
def test_pyramid_quality_calibration_profit_grid_sets_one_step_min_profit(tmp_path, monkeypatch):
    monkeypatch.setattr(mod, "INPUT_REPORT_DIR", tmp_path / "input")
    mod.INPUT_REPORT_DIR.mkdir(parents=True)
    one_share_rows = [
    ...
    one_share_rows.extend(
        _row(100 + i, "pyramid_overheat_or_reversal_risk", max_profit_seen=2.0, final_profit_rate=0.2)
        for i in range(6)
    )
    ...
    assert candidate["recommended_values"]["min_profit_pct"] == grid_decision["selected_min_profit_pct"]
    assert grid_decision["selected_min_profit_pct"] < candidate["current_values"]["min_profit_pct"]
    assert grid_decision["selected_row"]["eligible_count"] >= 20


def test_pyramid_quality_calibration_does_not_fallback_when_one_share_floor_missing(tmp_path, monkeypatch):
    monkeypatch.setattr(mod, "INPUT_REPORT_DIR", tmp_path / "input")
    mod.INPUT_REPORT_DIR.mkdir(parents=True)
    ...
    assert "rolling_closed_one_share_pyramid_rows_lt_20" in candidate["calibration_reason"]
