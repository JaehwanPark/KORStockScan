...
def test_discovery_floor_reuses_strict_decode_for_read_only_floor_facts(
    tmp_path, monkeypatch, compressed
):
    logical = tmp_path / "floor.json"
    physical = logical.with_name(logical.name + ".gz") if compressed else logical
    payload = {"included_artifacts": [{"parent_count": 4}]}
    raw = json.dumps(payload).encode()
    physical.write_bytes(gzip.compress(raw) if compressed else raw)
...
def test_discovery_floor_never_reuses_corrected_or_invalid_generation(tmp_path, change):
    ...
    if change == "rewrite":
        ...
        os.utime(logical, ns=(previous.st_atime_ns, previous.st_mtime_ns))
    elif change == "replace":
...
def test_historical_floor_discovery_and_selection_share_decode_with_full_parity(
    tmp_path, monkeypatch
):
    prior_body = {
