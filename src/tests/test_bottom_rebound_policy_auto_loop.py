...
def _write_sources(tmp_path: Path, *, ev: float = 1.02, sample_count: int = 8) -> dict[str, Path]:
    paths = {
    ...
    paths["bottom_rebound_research"].write_text(__import__("json").dumps(_research()), encoding="utf-8")
    paths["candidate_source"].write_text(__import__("json").dumps(_candidate_source()), encoding="utf-8")
    paths["swing_strategy_discovery_ev"].write_text(__import__("json").dumps(_ev_report(ev, sample_count)), encoding="utf-8")
    return paths


def test_policy_auto_loop_promotes_sim_only_when_tier2_ai_passes_and_one_percent_better(tmp_path: Path) -> None:
    paths = _write_sources(tmp_path, ev=1.02, sample_count=8)
    ai_payload = {
        "schema_version": 1,
        "interpretation": {"policy_edge_state": "candidate_policy_better"},
        "audit": {"status": "pass", "explicit_gaps": [], "forbidden_use_violations": []},
        "final_conclusion": {
...
def test_policy_auto_loop_refreshes_control_tower_when_written_to_default_report_dir(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("src.engine.swing.sim_auto_approval_control_tower.SIM_AUTO_APPROVAL_DIR", approval_dir)
    monkeypatch.setattr("src.engine.swing.sim_auto_approval_control_tower.SWING_SIM_POLICY_DIR", policy_dir)
    ai_payload = {
        "schema_version": 1,
        "interpretation": {"policy_edge_state": "candidate_policy_better"},
        "audit": {"status": "pass", "explicit_gaps": [], "forbidden_use_violations": []},
    ...
    approval = __import__("json").loads((approval_dir / "swing_sim_auto_approval_2026-05-22.json").read_text())
...
def test_policy_auto_loop_keeps_collecting_when_sample_floor_not_met(tmp_path: Path) -> None:
    paths = _write_sources(tmp_path, ev=1.05, sample_count=2)
    ai_payload = {
        "schema_version": 1,
        "interpretation": {"policy_edge_state": "candidate_policy_better"},
        "audit": {"status": "correction_required", "explicit_gaps": ["sample_floor_not_met"], "forbidden_use_violations": []},
        "final_conclusion": {
    ...
    report = build_policy_auto_loop_report("2026-05-22", ai_raw_response=ai_payload, source_paths=paths)
