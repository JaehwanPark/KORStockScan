...
def _price_ready_row(
    ...
    *,
    ...
    _, fields = compose_entry_execution_sizing_plan(
        [
            {
                ...
                **(
                ...
                "entry_price_captured_at": 1789603200.0,
            }
        ],
        ...
        quantity_policy_version="qty-current",
        split_policy_version="split-current",
    )
    ...
    return {
        ...
        "stock_name": "Samsung",
        "record_id": "record-1",
        "emitted_at": f"2026-09-17T10:00:{second}+09:00",
        "emitted_date": "2026-09-17",
...
def test_price_ready_attempts_are_separate_with_same_legacy_record_and_second(
    monkeypatch, tmp_path
):
    monkeypatch.setattr(report_mod, "DATA_DIR", tmp_path)
    rows = [
        _price_ready_row(monkeypatch, attempt=attempt)
        for attempt in ("eval-1", "eval-2")
    ]
...
def test_repeated_price_ready_plan_keeps_first_anchor_and_exact_attempt(
    monkeypatch, tmp_path
):
    monkeypatch.setattr(report_mod, "DATA_DIR", tmp_path)
...
def test_conflicting_valid_price_ready_plans_are_not_last_writer_wins(
    monkeypatch, tmp_path
):
    monkeypatch.setattr(report_mod, "DATA_DIR", tmp_path)
    rows = [
        _price_ready_row(monkeypatch, second="00"),
        _price_ready_row(monkeypatch, second="01", price=10100),
    ]
    _write_pipeline_events(tmp_path, "2026-09-17", rows)
    events = report_mod._load_entry_events("2026-09-17")
    assert all(report_mod._price_ready_plan(event) for event in events)
    assert report_mod._buy_intent_events(events) == (
        [],
        "conflicting_price_ready_plans",
    )
    assert report_mod._build_buy_attempts("2026-09-17", events=events) == []


def test_price_ready_full_scope_identity_and_interleaved_conflicts(
    monkeypatch, tmp_path
):
    monkeypatch.setattr(report_mod, "DATA_DIR", tmp_path)
    rows = [
        _price_ready_row(monkeypatch),
        _price_ready_row(monkeypatch, second="01", bundle="c"),
    ]
    _write_pipeline_events(tmp_path, "2026-09-17", rows)
    attempts = report_mod._build_buy_attempts(
        "2026-09-17", events=report_mod._load_entry_events("2026-09-17")
    ...
    assert (
        report["meta"]["input_streaming"]["price_ready_conflicting_identity_count"] == 1
    )
...
def test_probe_residual_future_price_is_not_fabricated(monkeypatch, tmp_path):
    ...
    _write_pipeline_events(
        tmp_path, "2026-09-17", [_price_ready_row(monkeypatch, deferred=True)]
    )
    attempts = report_mod._build_buy_attempts(
        "2026-09-17", events=report_mod._load_entry_events("2026-09-17")
    )
